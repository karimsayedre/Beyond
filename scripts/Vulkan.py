...
BEYOND_REQUIRED_VULKAN_VERSION = '1.4.' # Any 1.3 version is fine
BEYOND_INSTALL_VULKAN_VERSION = '1.4.313.2' # Specifically install this one if no 1.3 version is present
...
def CheckVulkanSDK():
    if (VULKAN_SDK is None):
        ...
        InstallVulkanPrompt()
        return False
    elif (BEYOND_REQUIRED_VULKAN_VERSION not in VULKAN_SDK):
        print(f"Located Vulkan SDK at {VULKAN_SDK}")
        print(f"{Style.BRIGHT}{Back.RED}You don't have the correct Vulkan SDK version! (Beyond requires {BEYOND_REQUIRED_VULKAN_VERSION}){Style.RESET_ALL}")
        InstallVulkanPrompt()
        return False
