...
#include "Beyond/Audio/AudioEvents/CommandID.h"
...
namespace Beyond {
	...
	static bool PropertyDropdownNoLabel(const char* label, const std::vector<eastl::string>& options, int32_t optionCount, int32_t* selected)
	{
		const char* current = options[*selected].c_str();

		bool changed = false;

		eastl::string id = "##" + eastl::string(label);
	...
	bool AudioEventsEditor::OnKeyPressedEvent(KeyPressedEvent& e)
	{
		...
		//if (Input::IsKeyPressed(BEY_KEY_LEFT_SHIFT))
		//{
		//	switch (e.GetKeyCode())
		//	{
		//	case KeyCode::Up:
	...
	bool AudioEventsEditor::OnImGuiRender(bool& isOpen)
	{
		bool changed = false;
		if (!isOpen)
		{
	...
	void AudioEventsEditor::OnEntryRenamed(CommandsTree::Node* treeNode, const char* newName)
	{
		...
		if (AudioCommandRegistry::AddNewCommand<TriggerCommand>(newName))
		{
			...
			BEY_CORE_ASSERT(inserted);
		}
		else
		{
			BEY_CORE_ERROR("Trigger with the name {0} already exists! Audio command names must be unique.", newName);
		}
	...
	bool Beyond::AudioEventsEditor::IsRenaming()
	{
	...
	std::optional<std::string> AudioEventsEditor::CheckNameConflict(const std::string& name, CommandsTree::Node* parentNode, std::function<void()> onReplaceDestination, std::function<void()> onCancel)
	{
		...
		if (ImGui::BeginPopupModal("Name Conflict", nullptr, ImGuiWindowFlags_AlwaysAutoResize))
		{
			...
			auto nameTaken = [parentNode](const std::string& name)
			{
				return parentNode->Children.contains(name);
			};
	...
	void AudioEventsEditor::SerializeTree(YAML::Emitter& out)
	{
		...
		auto serializeNode = [&out](CommandsTree::Node* treeNode, auto serializeNode) ->void
		{
			...
			else
			{
				...
				BEY_SERIALIZE_PROPERTY(Name, treeNode->Name, out);
				BEY_SERIALIZE_PROPERTY(CommandID, (uint32_t)(*treeNode->Value), out);
				out << YAML::EndMap;
	...
	void AudioEventsEditor::DeserializeTree(std::vector<YAML::Node>& data)
	{
		...
		if (it == data.end())
		{
			BEY_CORE_ERROR("Invalid, or missing TriggersTreeView cache.");
			return;
		}
		...
		auto deserializeNode = [](YAML::Node& node, CommandsTree::Node* parentFolder, auto deserializeNode) ->void
		{
			...
			if (isFolder)
			{
				...
				folder->Tree = &s_Tree;
				BEY_DESERIALIZE_PROPERTY(Name, folder->Name, node, std::string(""));
			...
			else
			{
				...
				trigger->Tree = &s_Tree;
				BEY_DESERIALIZE_PROPERTY(Name, trigger->Name, node, std::string(""));
