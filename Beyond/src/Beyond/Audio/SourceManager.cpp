    ...
    bool SourceManager::InitializeSource(uint32_t sourceID, const Ref<SoundConfig>& sourceConfig)
    {
        ...
        if (type == AssetType::SoundGraphSound) // TODO: !!! Replace this with the new Sound Graph
        {
			...
			if (!prototype)
			{
				...
				if (name.find("sound_graph_cache_") != eastl::string::npos)
					name = name.substr(18);
            ...
            {
				BEY_CONSOLE_LOG_ERROR("SourceManager::InitializeSource. Failed to initialize source, SoundGraph has to be built.");
                return false;
            }
            ...
            if (soundSource->InitializeDataSource(sourceConfig, &m_AudioEngine, graphInstance))
            {
                ...
                if (!node)
                {
                    BEY_CORE_ASSERT(false, "Failed to initialize audio data soruce.");
                    return false;
                }
