...
namespace Beyond
{
	...
	namespace Utils
	{
		...
		static inline void CopyArrayData(const choc::value::ValueView& destCustomArrayObject, const choc::value::ValueView& sourceArrayData)
		{
			...
			BEY_CORE_ASSERT(destination.size() >= sourceArrayData.size());
			BEY_CORE_ASSERT(currentDataSize >= newDataSize);
	...
	SoundGraphPatchPreset::Parameter* Beyond::SoundGraphPatchPreset::begin() { return m_Parameters.begin(); }
	SoundGraphPatchPreset::Parameter* Beyond::SoundGraphPatchPreset::end() { return m_Parameters.end(); }
	...
	void SoundGraphPatchPreset::TranslateParameters()
	{
		...
		for (auto& parameter : m_Parameters)
		{
			...
			else if (value.isArray())
			{
				...
				//BEY_CORE_ASSERT(arrayMaxSize == value.size());
	...
	bool SoundGraphPatchPreset::SetParameter(std::string_view name, const choc::value::ValueView& value)
	{
		...
		{
			BEY_CORE_ASSERT(false, "Dynamically changing SoundGraph Wave asset arrays is not implemented yet!");
			return false;
		}
