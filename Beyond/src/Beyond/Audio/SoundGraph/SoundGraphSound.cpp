...
namespace Beyond
{
    ...
    #define LOG_PLAYBACK(...) BEY_CORE_INFO(__VA_ARGS__)
#else
    #define LOG_PLAYBACK(...)
#endif
	...
	bool Beyond::SoundGraphSound::InitializeAudioCallback()
	{
		bool result = true;
    ...
    {
        BEY_CORE_ASSERT(!IsPlaying());
        BEY_CORE_ASSERT(!bIsReadyToPlay);
		...
		{
			BEY_CORE_ASSERT(false, "Failed to initialize Audio Callback for SoundGraphSound.");
			return false;
		}
    ...
    bool SoundGraphSound::Play()
    {
        ...
        bFinished = false;
		BEY_CORE_ASSERT(!m_Source->IsFinished());
    ...
    {
        BEY_CORE_ASSERT(milliseconds > 0);
    ...
    void SoundGraphSound::Update(Beyond::Timestep ts)
    {
        BEY_PROFILE_FUNC();
        ...
        case EPlayState::Starting:
            if (graph)
            {
                ...
                {
                    BEY_CORE_ASSERT("Failed to post Play event to SoundGraph instance.");
                }
            }
#if 0
            if (isNodePlaying())
        ...
        case EPlayState::Playing:
        {
            ...
            if (time >= step)
            {
                ...
                if (!m_Source->SetParameter("VolumePar", choc::value::Value(volume)))
                    BEY_CORE_ERROR("Failed to set VolumePar");
            }*/
    ...
    int SoundGraphSound::StopNow(bool notifyPlaybackComplete /*= true*/, bool resetPlaybackPosition /*= true*/)
    {
        ...
        if (resetPlaybackPosition)
        {
            ...
            BEY_CORE_ASSERT(m_PlayState != EPlayState::Starting);
            m_NextPlayState = EPlayState::Stopped;
        }
