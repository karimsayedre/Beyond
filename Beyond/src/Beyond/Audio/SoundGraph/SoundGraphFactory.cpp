	...
	NodeProcessor* Factory::Create(Identifier nodeTypeID, UUID nodeID)
	{
		if (!NodeProcessors.count(nodeTypeID))
		{
			...
			if (!nodeTypeID.GetDBGName().empty())
				BEY_CORE_ERROR("SoundGraph::Factory::Create - Node with type ID {} is not in the registry", nodeTypeID.GetDBGName());
			else
				BEY_CORE_ERROR("SoundGraph::Factory::Create - Node with type ID {} is not in the registry", (uint32_t)nodeTypeID);
