...
#include "Beyond/Audio/SoundGraph/WaveSource.h"
...
#if LOG_DBG_MESSAGES
#define DBG(...) BEY_CORE_WARN(__VA_ARGS__)
#else
#define DBG(...)
#endif
...
namespace Beyond::SoundGraph
{
	...
	private:
		...
		void StartPlayback()
		{
			if (!isInitialized)
			{
				BEY_CORE_WARN("!WavePlayer::Play() - Wave Player is not initialized.");
				return;
			}

			startSample = int64_t(*in_StartTime * sampleRate /*currentBuffer.sampleRate*/);
			BEY_CORE_ASSERT(startSample >= 0);
			...
			if (!buffer.WaveHandle)
			{
				...
				//BEY_CORE_WARN("!WavePlayer::Play() - invalid wave source id.");
