...
#include "Beyond/Audio/SoundGraph/Nodes/NodeTypes.h"
#include "Beyond/Audio/SoundGraph/NodeProcessor.h"
#include "Beyond/Audio/SoundGraph/SoundGraphFactory.h"
...
#include "Beyond/Serialization/MemoryStream.h"
...
namespace Beyond::SoundGraph {
	...
	struct Parser
	{
		...
		{
			BEY_CORE_ASSERT(Options.Model);
			BEY_CORE_ASSERT(Options.Graph);
			BEY_CORE_ASSERT(OutSoundGraph);
		}
		...
		*/
		...
		void ParseInputParameters()
		{
			...
			const Beyond::Utils::PropertySet& inputs = Graph->GraphInputs;
		...
		void ParseNodes()
		{
			...
			const Beyond::Utils::PropertySet& graphInputs = Graph->GraphInputs;
		...
		NodeProcessor* CreateNode(std::string_view nodeName, UUID nodeID)
		{
			...
			if (!node)
			{
				BEY_CORE_ERROR("[SoundGraph] Parser. Node type '{}' is not implemented!", nodeName);
					...
					*/
					...
					else // Local variable plug connection
					{
						...
						else
						{
							...
							BEY_CORE_ERROR("We can't have default plugs for Local Variables of function type!");
						}
			...
			for (const auto& node : Graph->Nodes)
			{
				for (const auto& input : node->Inputs)
				{
					...
					if (Utils::IsAssetHandle<AssetType::Audio>(value))
					{
						...
						{
							//BEY_CORE_WARN("ParseWaveReferences() - Found Wave Asset reference pin!");
	...
	Ref<Prototype> ConstructPrototype(GraphGeneratorOptions options, std::vector<UUID>& waveAssetsToLoad)
	{
		...
		if (!errors.empty())
		{
			...
			BEY_CONSOLE_LOG_ERROR("Failed to construct Sound Graph Prototype! " + choc::text::joinStrings(errors, "\n"));
			return nullptr;
		}
