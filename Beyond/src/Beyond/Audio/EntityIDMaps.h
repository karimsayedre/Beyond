...
namespace Beyond {
	...
	*/
	...
	struct EntityIDMap
	{
		...
		{
			std::scoped_lock lock{ m_Mutex };
			BEY_CORE_ASSERT(m_EntityIDMap.at(sceneID).find(entityID) != m_EntityIDMap.at(sceneID).end(),
...
namespace std {
	template <>
	struct hash<Beyond::Audio::EventID>
	{
		std::size_t operator()(const Beyond::Audio::EventID& id) const noexcept
		{
...
//namespace std {
//	// Implementation
//	 std::size_t hash<Beyond::Audio::EventID>::operator()(const Beyond::Audio::EventID& id) const noexcept { return hash<uint32_t>()(id.m_ID); }
//}
