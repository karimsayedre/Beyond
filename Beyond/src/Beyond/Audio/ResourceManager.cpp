...
namespace Beyond::Audio
{
	...
	void ResourceManager::Initialize()
	{
		...
		if (std::filesystem::exists(bankPath) && std::filesystem::is_regular_file(bankPath))
		{
			...
			BEY_CORE_ASSERT(m_SoundBank->IsLoaded());
		}
	}
	...
	void ResourceManager::ReleaseResources()
	{
		...
		if (!bankFilepath.empty())	BEY_CONSOLE_LOG_INFO("Sound Bank '{0}' has been unloaded.", bankFilepath);
		else if (anyUnloaded)		BEY_CONSOLE_LOG_INFO("Sounds loaded with Sound Bank has been unloaded.");
		else						BEY_CONSOLE_LOG_INFO("No sounds were unloaded with Sound Bank because none has been loaded.");
	}

	std::vector<AssetHandle> ResourceManager::CollectWaveHandlesForPlayAction(const TriggerAction& playAction) const
	{
		BEY_CORE_ASSERT(playAction.Type == EActionType::Play);
	...
	Ref<SoundBank> ResourceManager::BuildSoundBank()
	{
		...
		if (waveAssets.empty())
		{
			BEY_CONSOLE_LOG_INFO("No referenced audio source assets detected, Sound Bank generation aborted.");
			return nullptr;
		}
		...
		if (m_SoundBank)
		{
			...
			BEY_CONSOLE_LOG_INFO("Successfully packaged Sound Bank. Number of audio files packaged {0}, Sound Bank size {1}, file path {2}",
	...
	{
		BEY_CORE_ASSERT(!std::filesystem::exists(sourceFile));
