...
#include "Beyond/Audio/Editor/OrderedVector.h"
...
namespace Beyond::Audio
{
	...
	struct TriggerCommand : public CommandBase
	{
		...
		Beyond::OrderedVector<TriggerAction> Actions;
...
namespace Beyond::Utils {
	...
	inline Audio::EActionType AudioActionTypeFromString(const std::string& actionType)
	{
		...
		BEY_CORE_ASSERT(false, "Unknown Action Type");
	...
	inline Audio::EActionContext AudioActionContextFromString(const std::string& context)
	{
		...
		BEY_CORE_ASSERT(false, "Unknown Context Type");
