...
namespace Beyond
{
	...
	void AudioCommandRegistry::Init(WeakRef<AudioCommandRegistry>& instance)
	{
		if (!instance)
		{
			BEY_CORE_ASSERT(false, "Invalid instance of AudioCommandsRegistry.");
			return;
		}
	...
	void AudioCommandRegistry::LoadCommandRegistry(const std::filesystem::path& filepath)
	{
		...
		*/
		...
		if (!triggers)
		{
			BEY_CORE_ERROR("CommandRegistry appears to be corrupted!");
			return;
		}
		...
		for (auto entry : triggers)
		{
			...
			BEY_DESERIALIZE_PROPERTY(DebugName, trigger.DebugName, entry, std::string(""));
			
			if (trigger.DebugName.empty())
			{
				BEY_CORE_ERROR("Tried to load invalid command. Command must have a DebugName, or an ID!");
				continue;
			}
			...
			for (auto actionEntry : actions)
			{
				...
				BEY_DESERIALIZE_PROPERTY(Type, type, actionEntry, std::string(""));
				BEY_DESERIALIZE_PROPERTY_ASSET(Target, action.Target, actionEntry, SoundConfig);
				BEY_DESERIALIZE_PROPERTY(Context, context, actionEntry, std::string(""));
	...
	void AudioCommandRegistry::WriteRegistryToFile(const std::filesystem::path& filepath)
	{
		...
		for (auto& [commandID, trigger] : sortedTriggers)
		{
			...
			BEY_SERIALIZE_PROPERTY(DebugName, trigger.DebugName, out);

			out << YAML::Key << "Actions" << YAML::BeginSeq;
			for (auto& action : trigger.Actions.GetVector())
			{
				out << YAML::BeginMap;
				BEY_SERIALIZE_PROPERTY(Type, Utils::AudioActionTypeToString(action.Type), out);
				BEY_SERIALIZE_PROPERTY_ASSET(Target, action.Target, out);
				BEY_SERIALIZE_PROPERTY(Context, Utils::AudioActionContextToString(action.Context), out);
				out << YAML::EndMap;
			}
