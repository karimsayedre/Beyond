...
namespace Beyond
{
		...
		: m_Path(path)
	{
		...
		BEY_CONSOLE_LOG_TRACE("Audio: SoundBank: Successfully loaded SoundBank. Number of files in the SoundBank - {}", m_File.Header.AudioFileCount);
	}
	...
	{
		BEY_CORE_VERIFY(!Application::IsRuntime());
		...
		// Error checking
		{
			...
			if (!errorMsg.empty())
			{
				BEY_CONSOLE_LOG_ERROR("Failed to create Sound Bank. {}", errorMsg);
				return nullptr;
			}
		}
		...
		for (const auto& handle : waveAssets)
		{
			...
			// Create file info entry
			{
				...
				BEY_CORE_WARN("SoundBank - {}", serializedPathString);
