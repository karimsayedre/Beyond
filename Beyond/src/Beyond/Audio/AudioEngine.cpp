...
#define LOG_VOICES(...) BEY_CORE_INFO_TAG("Audio", __VA_ARGS__)
#else
#define LOG_VOICES(...)
#endif

#if 0 // Enable logging for Events handling
#define LOG_EVENTS(...) BEY_CORE_INFO_TAG("Audio", __VA_ARGS__)
#else
#define LOG_EVENTS(...)
#endif
...
namespace Beyond {
	...
	void MiniAudioEngine::ExecuteOnAudioThread(EIfThisIsAudioThread policy, Audio::AudioThreadCallbackFunction func, const char* jobID)
	{
		if (IsAudioThread())
		{
			...
			{
				default:
				case Beyond::MiniAudioEngine::ExecuteAsync: AudioThread::AddTask(hnew AudioFunctionCallback(std::move(func), jobID)); break;
				case Beyond::MiniAudioEngine::ExecuteNow: func(); break;
			}
		}
		else
		{
...
#ifdef LOG_MEMORY
		BEY_CORE_INFO_TAG("Audio", "Freed mem KB : {0}", *sizeBox / 1000.0f);
#endif
...
#ifdef LOG_MEMORY
		BEY_CORE_INFO_TAG("Audio", "Allocated mem KB : {0}", *sizeBox / 1000.0f);
#endif
...
#ifdef LOG_MEMORY
		BEY_CORE_INFO_TAG("Audio", "Reallocated mem KB : {0}", sz / 1000.0f);
#endif
	...
	{
		eastl::string message = fmt::eastl_format("{0}: {1}", eastl::string(ma_log_level_to_string(level)), pMessage);
		message.erase(std::remove(message.begin(), message.end(), '\n'), message.end());

		switch (level)
		{
			case MA_LOG_LEVEL_INFO:
				BEY_CORE_TRACE_TAG("miniaudio", message);
				break;
			case MA_LOG_LEVEL_WARNING:
				BEY_CORE_INFO_TAG("miniaudio", message);
				break;
			case MA_LOG_LEVEL_ERROR:
				BEY_CORE_ERROR_TAG("miniaudio", message);
				break;
			default:
				BEY_CORE_TRACE_TAG("miniaudio", message);
		}
	...
	{
		BEY_CORE_ASSERT(s_Instance == nullptr, "Audio Engine already initialized.");
		MiniAudioEngine::s_Instance = hnew MiniAudioEngine();
	}

	void MiniAudioEngine::Shutdown()
	{
		BEY_CORE_ASSERT(s_Instance, "Audio Engine was not initialized.");
	...
	bool MiniAudioEngine::Initialize()
	{
        ...
        // TODO: These callbacks cause heap corruption
		...
		BEY_CORE_ASSERT(result == MA_SUCCESS, "Failed to initialize miniaudio logger.");
		...
		BEY_CORE_ASSERT(result == MA_SUCCESS, "Failed to register miniaudio log callback.");
		...
		{
			BEY_CORE_ASSERT(false, "Failed to initialize Resource Manager VFS.");
			return false;
		}
		...
		if (result != MA_SUCCESS)
		{
			BEY_CORE_ASSERT(false, "Failed to initialize audio engine.");
			return false;
		}
		...
		BEY_CORE_INFO_TAG("Audio", R"(Audio Engine: engine initialized.
                    -----------------------------
	...
	bool MiniAudioEngine::Uninitialize()
	{
		...
		BEY_CORE_INFO_TAG("Audio", "Audio Engine: engine un-initialized.");
	...
	void MiniAudioEngine::OnProjectLoaded()
	{
		...
		{
			BEY_CORE_ERROR("Could not load SoundBank OnProjectLoaded!");
		}
	...
	void MiniAudioEngine::InvokeOnActiveSources(Audio::EventID playingEvent, Function functionToInvoke)
	{
		if (m_EventsManager->GetNumberOfActiveSources(playingEvent) <= 0)
		{
			...
			BEY_CONSOLE_LOG_WARN("Audio. Trying to set parameter for an event that's not in the active events registry.");
			return;
		}
	...
	SoundObject* MiniAudioEngine::FreeLowestPrioritySource()
	{
		...
		BEY_CORE_ASSERT(m_SourceManager->m_FreeSourcIDs.empty());
		...
		BEY_CORE_ASSERT(releasedSoundSource);
...
#if DBG_VOICES
		BEY_CORE_ASSERT(releasedSoundSource->m_PlayState == Sound::ESoundPlayState::Stopped);
#endif
		BEY_CORE_ASSERT(!m_SourceManager->m_FreeSourcIDs.empty());
	...
	bool MiniAudioEngine::SubmitStartPlayback(uint64_t audioEntityID)
	{
		...
		{
			BEY_CORE_ERROR_TAG("Audio", "AudioEntity with ID {0} doesn't exist!", audioEntityID);
			return false;
		}
	}
	...
	int MiniAudioEngine::Action_StartPlayback(UUID objectID, EventID playingEvent, Ref<SoundConfig> target)
	{
		...
		//BEY_CORE_ASSERT(m_ActiveSounds.size() < m_NumSources && m_SoundsToStart.size() < m_NumSources);
	...
	void MiniAudioEngine::Action_ExecuteOnSources(EActionTypeOnPlayingEvent action, const std::vector<SourceID>& sources)
	{
		...
		// TODO
		auto break_ = [](SoundObject* sound) { BEY_CORE_ASSERT(false); };
		auto releaseEnvelope = [](SoundObject* sound) { BEY_CORE_ASSERT(false); };
	...
	void MiniAudioEngine::UpdateSources()
	{
		...
		*/
		...
		//BEY_CORE_ASSERT(m_ActiveSounds.size() + m_SoundsToStart.size() == m_VoiceHandles.size());
		...
		{
			BEY_PROFILE_SCOPE_DYNAMIC("MiniAudioEngine::UpdateSources - USP Loop");
	...
	void MiniAudioEngine::UpdateListenerVelocity(const glm::vec3& newVelocity)
	{
		...
		BEY_CORE_ASSERT(isVelocityValid(newVelocity));
	...
	void MiniAudioEngine::ReleaseFinishedSources()
	{
		...
		//BEY_CORE_ASSERT(m_ObjectData.size() == m_EventHandles.size());
	...
	{
		BEY_PROFILE_FUNC("MiniAudioEngine::Update");
	...
	{
		BEY_CORE_ASSERT(AudioThread::IsAudioThread());
		...
		{
			BEY_CORE_ERROR_TAG("Audio", "Failed to initialize sound source!");
	...
	EventID MiniAudioEngine::PostTrigger(CommandID triggerCommandID, UUID entityID)
	{
		...
		{
			BEY_CORE_ERROR_TAG("Audio", "PostTrigger with empty triggerCommandID.");
			return EventID::INVALID;
		}

		if (!AudioCommandRegistry::DoesCommandExist<TriggerCommand>(triggerCommandID))
		{
			BEY_CONSOLE_LOG_ERROR("[Audio] PostTrigger. Audio command with ID {0} does not exist!", triggerCommandID);
			return EventID::INVALID;
		}
		...
		if (entityID == 0)
		{
			BEY_CORE_ERROR_TAG("Audio", "PostTrigger. Invalid entity ID {0}.", entityID);
			return EventID::INVALID;
		}
	...
	void MiniAudioEngine::PauseAll()
	{
		if (m_PlaybackState != EPlaybackState::Playing)
		{
			BEY_CORE_ASSERT(false, "Audio Engine is already paused.");
			return;
		}
	...
	void MiniAudioEngine::ResumeAll()
	{
		...
		{
			BEY_CORE_ASSERT(false, "Audio Engine is already active.");
			return;
		}
	...
	void MiniAudioEngine::OnRuntimePlaying(UUID sceneID)
	{
		...
		BEY_CORE_ASSERT(currentSceneID == sceneID);
		...
		//BEY_CORE_INFO_TAG("Audio", "ON RUNTIME PLAYING");
	}
	...
	void MiniAudioEngine::OnSceneDestruct(UUID sceneID)
	{
		...
		//BEY_CORE_INFO_TAG("Audio", "ON SCENE DESTRUCT");
	}
	...
	bool MiniAudioEngine::BuildSoundBank()
	{
		...
		{
			BEY_CONSOLE_LOG_WARN("Can't build Sound Bank while playing a scene!");
			return false;
		}

		BEY_CONSOLE_LOG_INFO("Packaging Sound Bank...");
	...
	bool MiniAudioEngine::UnloadCurrentSoundBank()
	{
		if (Get().GetCurrentSceneContext()->IsPlaying())
		{
			BEY_CONSOLE_LOG_WARN("Can't unload Sound Bank while playing a scene!");
			return false;
		}

		BEY_CONSOLE_LOG_INFO("Unloading Sound Bank...");
