...
#define BEY_IGNORE_EXTERNAL_ALLOCATIONS
	...
	{
		BEY_CORE_ASSERT(!s_ProfilerState, "Trying to initialize Script Profiler multiple times!");
	...
	void ScriptProfiler::OnDomainLoaded(MonoProfiler* profiler, MonoDomain* domain)
	{
		...
		// NOTE: Unsure if we have to do this every time we load a domain
...
#ifdef BEY_IGNORE_EXTERNAL_ALLOCATIONS
