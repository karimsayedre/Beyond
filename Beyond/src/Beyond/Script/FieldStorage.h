...
namespace Beyond {
	...
	public:
		...
		{
			BEY_CORE_VERIFY(sizeof(T) == m_FieldInfo->Size);
	...
	template<>
	inline std::string FieldStorage::GetValue() const
	{
		if (m_RuntimeInstance != nullptr)
		{
			Buffer valueBuffer;
	...
	template<>
	inline void FieldStorage::SetValue<std::string>(const std::string& value)
	{
		if (m_RuntimeInstance != nullptr)
		{
			SetValueRuntime(&value);
	...
	template<>
	inline std::string ArrayFieldStorage::GetValue(uint32_t index) const
	{
		return "";
	}

	template<>
	inline void ArrayFieldStorage::SetValue<std::string>(uint32_t index, const std::string& value)
	{
		if (m_RuntimeInstance != nullptr)
		{
			SetValueRuntime(index, &value);
