...
#define BEYOND_CORE_ASSEMBLY_INDEX 0
#define BEYOND_APP_ASSEMBLY_INDEX BEYOND_CORE_ASSEMBLY_INDEX + 1
#define BEYOND_MAX_ASSEMBLIES (size_t)2
...
namespace Beyond {
	...
	using AssembliesArray = std::array<Ref<AssemblyInfo>, BEYOND_MAX_ASSEMBLIES>;
	...
	public:
		...
		{
			return CreateManagedObject_Internal(ScriptCache::GetManagedClassByID(BEY_SCRIPT_CLASS_ID(className)), std::forward<TConstructorArgs>(args)...);
		}
		...
		{
			BEY_PROFILE_SCOPE_DYNAMIC(methodName.c_str());

			if (managedObject == nullptr)
			{
				BEY_CORE_WARN_TAG("ScriptEngine", "Attempting to call method {0} on an invalid instance!", methodName);
				return;
			}
			...
			if (clazz == nullptr)
			{
				BEY_CORE_ERROR_TAG("ScriptEngine", "Failed to find ManagedClass!");
				return;
			}
			...
			if (method == nullptr)
			{
				BEY_CORE_ERROR_TAG("ScriptEngine", "Failed to find a C# method called {0} with {1} parameters", methodName, argsCount);
				return;
			}
	...
	private:
		...
		static MonoObject* CreateManagedObject_Internal(ManagedClass* managedClass, TConstructorArgs&&... args)
		{
			...
			{
				BEY_CORE_ERROR_TAG("ScriptEngine", "Attempting to create managed object with a null class!");
				return nullptr;
			}

			BEY_PROFILE_SCOPE_DYNAMIC(managedClass->FullName.c_str());
			...
			if constexpr (argsCount > 0)
			{
				if (ctor == nullptr)
				{
					BEY_CORE_ERROR_TAG("ScriptEngine", "Failed to call constructor with {} parameters for class '{}'.", argsCount, managedClass->FullName);
					return obj;
				}
