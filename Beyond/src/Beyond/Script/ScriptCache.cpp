...
namespace Beyond {
	...
	{
		BEY_CORE_ASSERT(!s_Cache, "Trying to initialize ScriptCache multiple times!");
		s_Cache = hnew Cache();
	...
	void ScriptCache::CacheClass(std::string_view className, MonoClass* monoClass)
	{
		...
		if (managedClass.FullName.find("Beyond.") != std::string::npos)
		{
...
#define CACHE_BEYOND_CORE_CLASS(name) CacheClass("Beyond." name, mono_class_from_name(ScriptEngine::GetCoreAssemblyInfo()->AssemblyImage, "Beyond", name))

		CACHE_BEYOND_CORE_CLASS("ShowInEditorAttribute");
		CACHE_BEYOND_CORE_CLASS("HideFromEditorAttribute");
		CACHE_BEYOND_CORE_CLASS("ClampValueAttribute");

		CACHE_BEYOND_CORE_CLASS("AssetHandle");
		CACHE_BEYOND_CORE_CLASS("Vector2");
		CACHE_BEYOND_CORE_CLASS("Vector3");
		CACHE_BEYOND_CORE_CLASS("Vector4");
		CACHE_BEYOND_CORE_CLASS("Entity");
		CACHE_BEYOND_CORE_CLASS("IEditorRunnable");
		CACHE_BEYOND_CORE_CLASS("Prefab");
		CACHE_BEYOND_CORE_CLASS("Mesh");
		CACHE_BEYOND_CORE_CLASS("StaticMesh");
		CACHE_BEYOND_CORE_CLASS("Material");
		CACHE_BEYOND_CORE_CLASS("Shape");
		CACHE_BEYOND_CORE_CLASS("BoxShape");
		CACHE_BEYOND_CORE_CLASS("SphereShape");
		CACHE_BEYOND_CORE_CLASS("CapsuleShape");
		CACHE_BEYOND_CORE_CLASS("ConvexMeshShape");
		CACHE_BEYOND_CORE_CLASS("TriangleMeshShape");
		CACHE_BEYOND_CORE_CLASS("Collider");
		CACHE_BEYOND_CORE_CLASS("SceneQueryHit");
		CACHE_BEYOND_CORE_CLASS("PhysicsMaterial");
		CACHE_BEYOND_CORE_CLASS("RaycastHit2D");
		CACHE_BEYOND_CORE_CLASS("Texture2D");
		CACHE_BEYOND_CORE_CLASS("Scene");
	}
	...
	static void BuildClassMetadata(Ref<AssemblyInfo>& assemblyInfo, MonoClass* monoClass)
	{
		...
		if (parentClass != nullptr && parentClass != BEY_CACHED_CLASS_RAW("System.Object"))
		{
	...
	void ScriptCache::GenerateCacheForAssembly(Ref<AssemblyInfo> assemblyInfo)
	{
		...
		for (auto classID : assemblyInfo->Classes)
		{
			...
			if (mono_class_is_subclass_of(managedClass.Class, BEY_CACHED_CLASS_RAW("Beyond.Entity"), false))
			{
	...
	MonoClass* ScriptCache::GetFieldTypeClass(FieldType fieldType)
	{
		switch (fieldType)
		{
			case FieldType::Bool: return BEY_CACHED_CLASS("System.Bool")->Class;
			case FieldType::Int8: return BEY_CACHED_CLASS("System.SByte")->Class;
			case FieldType::Int16: return BEY_CACHED_CLASS("System.Int16")->Class;
			case FieldType::Int32: return BEY_CACHED_CLASS("System.Int32")->Class;
			case FieldType::Int64: return BEY_CACHED_CLASS("System.Int64")->Class;
			case FieldType::UInt8: return BEY_CACHED_CLASS("System.Byte")->Class;
			case FieldType::UInt16: return BEY_CACHED_CLASS("System.UInt16")->Class;
			case FieldType::UInt32: return BEY_CACHED_CLASS("System.UInt32")->Class;
			case FieldType::UInt64: return BEY_CACHED_CLASS("System.UInt64")->Class;
			case FieldType::Float: return BEY_CACHED_CLASS("System.Single")->Class;
			case FieldType::Double: return BEY_CACHED_CLASS("System.Double")->Class;
			case FieldType::String: return BEY_CACHED_CLASS("System.String")->Class;
			case FieldType::Vector2: return BEY_CACHED_CLASS("Beyond.Vector2")->Class;
			case FieldType::Vector3: return BEY_CACHED_CLASS("Beyond.Vector3")->Class;
			case FieldType::Vector4: return BEY_CACHED_CLASS("Beyond.Vector4")->Class;
			case FieldType::AssetHandle: return BEY_CACHED_CLASS("Beyond.AssetHandle")->Class;
			case FieldType::Prefab: return BEY_CACHED_CLASS("Beyond.Prefab")->Class;
			case FieldType::Entity: return BEY_CACHED_CLASS("Beyond.Entity")->Class;
			case FieldType::Mesh: return BEY_CACHED_CLASS("Beyond.Mesh")->Class;
			case FieldType::StaticMesh: return BEY_CACHED_CLASS("Beyond.StaticMesh")->Class;
			case FieldType::Material: return BEY_CACHED_CLASS("Beyond.Material")->Class;
			case FieldType::PhysicsMaterial: return BEY_CACHED_CLASS("Beyond.PhysicsMaterial")->Class;
			case FieldType::Texture2D: return BEY_CACHED_CLASS("Beyond.Texture2D")->Class;
			case FieldType::Scene: return BEY_CACHED_CLASS("Beyond.Scene")->Class;
		}
	...
	ManagedMethod* ScriptCache::GetManagedMethod(ManagedClass* managedClass, const std::string& name, bool ignoreParent /*= false*/)
	{
		...
		{
			BEY_CORE_ERROR_TAG("ScriptEngine", "Attempting to get method {0} from a nullptr class!", name);
			return nullptr;
		}
		...
		BEY_CORE_WARN_TAG("ScriptEngine", "Failed to find method with name: {0} in class {1}", name, managedClass->FullName);
		return nullptr;
	}
	...
	ManagedMethod* ScriptCache::GetSpecificManagedMethod(ManagedClass* managedClass, const std::string& name, uint32_t parameterCount, bool ignoreParent)
	{
		...
		if (method == nullptr)
			BEY_CORE_WARN_TAG("ScriptEngine", "Failed to find method with name: {0} and parameter count: {1} in class {2}", name, parameterCount, managedClass->FullName);
	...
	void ScriptCache::CacheClassFields(Ref<AssemblyInfo> assemblyInfo, ManagedClass& managedClass)
	{
		...
		while (currentClass != nullptr)
		{
			...
			if (classNameSpace.find("Beyond") != std::string::npos && className.find("Entity") != std::string::npos)
			{
			...
			while ((field = mono_class_get_fields(currentClass, &fieldPtr)) != NULL)
			{
				...
				if (attributes && mono_custom_attrs_has_attr(attributes, GetManagedClassByName("Beyond.ShowInEditorAttribute")->Class))
				{
					managedField.Flags &= ~(uint64_t)FieldFlag::Protected;
					managedField.Flags &= ~(uint64_t)FieldFlag::Internal;
					managedField.Flags &= ~(uint64_t)FieldFlag::Private;
					...
					MonoObject* attrib = mono_custom_attrs_get_attr(attributes, GetManagedClassByName("Beyond.ShowInEditorAttribute")->Class);
	...
	void ScriptCache::CacheClassProperties(Ref<AssemblyInfo> assemblyInfo, ManagedClass& managedClass)
	{
		...
		while ((prop = mono_class_get_properties(managedClass.Class, &propertyIter)) != NULL)
		{
			...
			if (monoType == nullptr)
			{
				BEY_CORE_ERROR_TAG("ScriptEngine", "Failed to retrieve managed type for property '{0}'", name);
				continue;
			}

			MonoCustomAttrInfo* attributes = mono_custom_attrs_from_property(managedClass.Class, prop);

			if (!TypeUtils::ContainsAttribute(attributes, "Beyond.ShowInEditorAttribute"))
				continue;
