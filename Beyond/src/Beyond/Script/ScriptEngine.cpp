...
#include "Beyond/Editor/EditorConsolePanel.h"
...
namespace Beyond {
	...
	{
		BEY_CORE_ASSERT(!s_State, "[ScriptEngine]: Trying to call ScriptEngine::Init multiple times!");
		...
		if (!LoadCoreAssembly())
		{
			...
			BEY_CORE_INFO_TAG("ScriptEngine", "Failed to load Beyond's C# core, attempting to build automatically using MSBuild");

			auto scriptCoreAssemblyFile = std::filesystem::current_path().parent_path() / "Beyond-ScriptCore" / "Beyond-ScriptCore.csproj";
			ScriptBuilder::BuildCSProject(scriptCoreAssemblyFile);

			if (!LoadCoreAssembly())
				BEY_CORE_FATAL_TAG("ScriptEngine", "Failed to build Beyond-ScriptCore! Most likely there were compile errors!");
		}

		for(const auto& x : s_State->CoreAssemblyInfo.Raw()->ReferencedAssemblies)
		{
			BEY_CORE_INFO("Loaded Core Assembly {}", x.Name);
		}

		for(const auto& x : s_State->AppAssemblyInfo.Raw()->ReferencedAssemblies)
		{
			BEY_CORE_INFO("Loaded App Assembly {}", x.Name);
		}
	}
	...
	{
		BEY_PROFILE_FUNC();
		BEY_CORE_ASSERT(s_State->SceneContext, "Tring to initialize script runtime without setting the scene context!");
		BEY_CORE_ASSERT(s_State->SceneContext->IsPlaying(), "Don't call ScriptEngine::InitializeRuntime if the scene isn't being played!");
	...
	void ScriptEngine::ShutdownRuntime()
	{
		...
		BEY_CORE_INFO_TAG("ScriptEngine", "Shutting down runtime scene {0}, with {1} scripts active.", runtimeSceneID, s_State->ScriptInstances.size());
	...
	bool ScriptEngine::ReloadAppAssembly(const bool scheduleReload)
	{
		...
		BEY_CORE_INFO_TAG("ScriptEngine", "Reloading {0}", Project::GetScriptModuleFilePath());
		...
		GCManager::CollectGarbage();
		BEY_CORE_INFO_TAG("ScriptEngine", "Done!");
	...
	bool ScriptEngine::LoadAppAssembly()
	{
		...
		{
			BEY_CORE_ERROR_TAG("ScriptEngine", "Failed to load app assembly! Invalid filepath");
			BEY_CORE_ERROR_TAG("ScriptEngine", "Filepath = {}", Project::GetScriptModuleFilePath());
			return false;
		}
		...
		{
			BEY_CORE_ERROR_TAG("ScriptEngine", "Failed to load app assembly!");
			return false;
		}
		...
		{
			return metadata.Name == "Beyond-ScriptCore";
		});

		if (coreMetadataIt == s_State->AppAssemblyInfo->ReferencedAssemblies.end())
		{
			BEY_CONSOLE_LOG_ERROR("C# project doesn't reference Beyond-ScriptCore?");
			return false;
		}
		...
		{
			BEY_CONSOLE_LOG_ERROR("C# project referencing an incompatible script core version!");
			BEY_CONSOLE_LOG_ERROR("Expected version: {0}.{1}, referenced version: {2}.{3}",
		...
		BEY_CORE_INFO_TAG("ScriptEngine", "Successfully loaded app assembly from: {0}", s_State->AppAssemblyInfo->FilePath);
	...
	void ScriptEngine::InitializeScriptEntity(Entity entity)
	{
		...
		{
			BEY_CORE_ERROR_TAG("ScriptEngine", "Tried to initialize script entity with an invalid script!");
			return;
		}
	...
	void ScriptEngine::RuntimeInitializeScriptEntity(Entity entity, bool skipInitializedEntities)
	{
		...
		if (!entity.HasComponent<ScriptComponent>())
		{
			BEY_CORE_WARN_TAG("ScriptEngine", "Trying to instantiate a script on an entity that doesn't have a ScriptComponent");
			return;
		}

		// NOTE: Intentional copy
		...
		if (!IsModuleValid(scriptComponent.ScriptClassHandle))
		{
			...
			BEY_CORE_ERROR_TAG("ScriptEngine", "Tried to instantiate an entity ({1}) with an invalid C# class '{0}'!", className, entity.Name());
			BEY_CONSOLE_LOG_ERROR("Tried to instantiate an entity ({1}) with an invalid C# class '{0}'!", className, entity.Name());
			return;
		}
	...
	void ScriptEngine::DuplicateScriptInstance(Entity entity, Entity targetEntity)
	{
		...
		if (srcScriptComp.ScriptClassHandle != dstScriptComp.ScriptClassHandle)
		{
			...
			BEY_CORE_WARN_TAG("ScriptEngine", "Attempting to duplicate instance of: {0} to an instance of: {1}", srcClass->FullName, dstClass->FullName);
			return;
		}
	...
	void ScriptEngine::InitMono()
	{
		...
		s_State->RootDomain = mono_jit_init("BeyondJITRuntime");
		BEY_CORE_ASSERT(s_State->RootDomain, "Unable to initialize Mono JIT");
		...
		BEY_CORE_INFO_TAG("ScriptEngine", "Initialized Mono");
	}
	...
	void ScriptEngine::ShutdownMono()
	{
		if (!s_State->IsMonoInitialized)
		{
			BEY_CORE_WARN_TAG("ScriptEngine", "Trying to shutdown Mono multiple times!");
			return;
		}
	...
	MonoObject* ScriptEngine::CreateManagedObject(ManagedClass* managedClass)
	{
		...
		BEY_CORE_VERIFY(monoObject, "Failed to create MonoObject!");
		return monoObject;
	}
	...
	MonoAssembly* ScriptEngine::LoadMonoAssembly(const std::filesystem::path& assemblyPath)
	{
		...
		if (status != MONO_IMAGE_OK)
		{
			...
			BEY_CORE_ERROR_TAG("ScriptEngine", "Failed to open C# assembly '{0}'\n\t\tMessage: {1}", assemblyPath, errorMessage);
			return nullptr;
		}
		...
		if (s_State->Config.EnableDebugging)
		{
			...
			if (!FileSystem::Exists(pdbPath))
			{
				...
				if (!FileSystem::Exists(pdbPath))
				{
					BEY_CORE_WARN_TAG("ScriptEngine", "Couldn't find .pdb file for assembly {0}, no debug symbols will be loaded.", assemblyPath.string());
			...
			if (loadDebugSymbols)
			{
				BEY_CORE_INFO_TAG("ScriptEngine", "Loading debug symbols from '{0}'", pdbPath.string());
	...
	MonoAssembly* ScriptEngine::LoadMonoAssemblyRuntime(Buffer assemblyData)
	{
		...
		{
			const char* errorMessage = mono_image_strerror(status);
			BEY_CORE_ERROR_TAG("ScriptEngine", "Failed to load C# assembly");
			return nullptr;
		}
	...
	{
		BEY_CORE_TRACE_TAG("ScriptEngine", "Trying to load core assembly: {}", s_State->Config.CoreAssemblyPath.string());
		if (!FileSystem::Exists(s_State->Config.CoreAssemblyPath))
		{
			BEY_CORE_ERROR_TAG("ScriptEngine", "Could not load core assembly! Path: {}", s_State->Config.CoreAssemblyPath.string());
			return false;
		}
		...
		std::string domainName = "BeyondScriptRuntime";
		...
		{
			BEY_CORE_ERROR_TAG("ScriptEngine", "Failed to load core assembly!");
			return false;
		}
		...
		BEY_CORE_INFO_TAG("ScriptEngine", "Successfully loaded core assembly from: {0}", s_State->Config.CoreAssemblyPath);
	...
	void ScriptEngine::LoadReferencedAssemblies(const Ref<AssemblyInfo>& assemblyInfo)
	{
		...
		{
			// Ignore Beyond-ScriptCore and mscorlib, since they're already loaded
			if (assemblyMetadata.Name.find("Beyond-ScriptCore") != std::string::npos)
				continue;
			...
			BEY_CORE_INFO_TAG("ScriptEngine", "Loading assembly {0} referenced by {1}", assemblyPath.string(), assemblyInfo->FilePath.filename().string());

			MonoAssembly* assembly = LoadMonoAssembly(assemblyPath);

			if (assembly == nullptr)
			{
				BEY_CORE_ERROR_TAG("ScriptEngine", "Failed to load assembly {0} referenced by {1}", assemblyMetadata.Name, assemblyInfo->FilePath);
				continue;
			}
