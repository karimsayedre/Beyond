...
#define REGISTER_METHOD_THUNK(var, method) s_MethodThunks->var.SetThunkFromMethod(method);
		...
		REGISTER_METHOD_THUNK(Entity_OnCreate, BEY_CACHED_METHOD("Beyond.Entity", "OnCreate", 0));
		REGISTER_METHOD_THUNK(Entity_OnUpdate, BEY_CACHED_METHOD("Beyond.Entity", "OnUpdate", 1));
		REGISTER_METHOD_THUNK(Entity_OnPhysicsUpdate, BEY_CACHED_METHOD("Beyond.Entity", "OnPhysicsUpdate", 1));
		REGISTER_METHOD_THUNK(Entity_OnDestroyInternal, BEY_CACHED_METHOD("Beyond.Entity", "OnDestroyInternal", 0));
		REGISTER_METHOD_THUNK(IEditorRunnable_OnInstantiate, BEY_CACHED_METHOD("Beyond.IEditorRunnable", "OnInstantiate", 0));
		REGISTER_METHOD_THUNK(IEditorRunnable_OnUIRender, BEY_CACHED_METHOD("Beyond.IEditorRunnable", "OnUIRender", 0));
		REGISTER_METHOD_THUNK(IEditorRunnable_OnShutdown, BEY_CACHED_METHOD("Beyond.IEditorRunnable", "OnShutdown", 0));
...
#undef REGISTER_METHOD_THUNK
	...
	bool ScriptUtils::CheckMonoError(MonoError& error)
	{
		...
		if (hasError)
		{
			...
			BEY_CORE_ERROR_TAG("ScriptEngine", "Mono Error!");
			BEY_CORE_ERROR_TAG("ScriptEngine", "\tError Code: {0}", errorCode);
			BEY_CORE_ERROR_TAG("ScriptEngine", "\tError Message: {0}", errorMessage);
	...
	static MonoExceptionInfo GetExceptionInfo(MonoObject* exception)
	{
		...
		auto GetExceptionString = [exception, exceptionClass](const char* stringName) -> eastl::string
		{
	...
	void ScriptUtils::HandleException(MonoObject* exception)
	{
		...
		BEY_CONSOLE_LOG_ERROR("{0}: {1}. Source: {2}, Stack Trace: {3}", exceptionInfo.TypeName, exceptionInfo.Message, exceptionInfo.Source, exceptionInfo.StackTrace);
	}
	...
	Buffer ScriptUtils::MonoObjectToValue(MonoObject* obj, FieldType fieldType)
	{
		...
		switch (fieldType)
		{
			...
			{
				eastl::string str = MonoStringToUTF8((MonoString*)obj);
	...
	MonoObject* ScriptUtils::ValueToMonoObject(const void* data, FieldType dataType)
	{
		...
		else
		{
			switch (dataType)
			{
				case FieldType::String: return (MonoObject*)UTF8StringToMono(eastl::string((const char*)data));
				case FieldType::Prefab: return ScriptEngine::CreateManagedObject("Beyond.Prefab", *(AssetHandle*)data);
				case FieldType::Entity: return ScriptEngine::CreateManagedObject("Beyond.Entity", *(UUID*)data);
				case FieldType::Mesh: return ScriptEngine::CreateManagedObject("Beyond.Mesh", *(AssetHandle*)data);
				case FieldType::StaticMesh: return ScriptEngine::CreateManagedObject("Beyond.StaticMesh", *(AssetHandle*)data);
				case FieldType::Material: return ScriptEngine::CreateManagedObject("Beyond.Material", *(AssetHandle*)data);
				case FieldType::PhysicsMaterial: return ScriptEngine::CreateManagedObject("Beyond.PhysicsMaterial", *(AssetHandle*)data);
				case FieldType::Texture2D: return ScriptEngine::CreateManagedObject("Beyond.Texture2D", *(AssetHandle*)data);
				case FieldType::Scene: return ScriptEngine::CreateManagedObject("Beyond.Scene", *(AssetHandle*)data);
			}
		}

		BEY_CORE_ASSERT(false, "Unsupported value type!");
		return nullptr;
	}
	...
	/*MonoObject* ScriptUtils::GetDefaultValueObjectForType(const ManagedType& type)
	{
		...
		switch (type.NativeType)
		{
			case UnmanagedType::Bool: return BoxValue(BEY_CACHED_CLASS("System.Boolean"), (MonoBoolean)false);
			case UnmanagedType::Int8: return BoxValue(BEY_CACHED_CLASS("System.SByte"), 0);
			case UnmanagedType::Int16: return BoxValue(BEY_CACHED_CLASS("System.Int16"), 0);
			case UnmanagedType::Int32: return BoxValue(BEY_CACHED_CLASS("System.Int32"), 0);
			case UnmanagedType::Int64: return BoxValue(BEY_CACHED_CLASS("System.Int64"), 0);
			case UnmanagedType::UInt8: return BoxValue(BEY_CACHED_CLASS("System.Byte"), 0);
			case UnmanagedType::UInt16: return BoxValue(BEY_CACHED_CLASS("System.UInt16"), 0);
			case UnmanagedType::UInt32: return BoxValue(BEY_CACHED_CLASS("System.UInt32"), 0);
			case UnmanagedType::UInt64: return BoxValue(BEY_CACHED_CLASS("System.UInt64"), 0);
			case UnmanagedType::Float: return BoxValue(BEY_CACHED_CLASS("System.Single"), 0);
			case UnmanagedType::Double: return BoxValue(BEY_CACHED_CLASS("System.Double"), 0);
			case UnmanagedType::String: return (MonoObject*)UTF8StringToMono("");
			case UnmanagedType::AssetHandle: return BoxValue(BEY_CACHED_CLASS("Beyond.AssetHandle"), 0);
			case UnmanagedType::Vector2: return BoxValue(BEY_CACHED_CLASS("Beyond.Vector2"), glm::vec2(0.0f));
			case UnmanagedType::Vector3: return BoxValue(BEY_CACHED_CLASS("Beyond.Vector3"), glm::vec3(0.0f));
			case UnmanagedType::Vector4: return BoxValue(BEY_CACHED_CLASS("Beyond.Vector4"), glm::vec4(0.0f));
	...
	FieldType ScriptUtils::GetFieldTypeFromMonoType(MonoType* monoType)
	{
		...
		switch (typeEncoding)
		{
			...
			case MONO_TYPE_VALUETYPE:
			{
				...
				if (BEY_CORE_CLASS(AssetHandle) && typeClass == BEY_CORE_CLASS(AssetHandle)->Class)
					return FieldType::AssetHandle;

				if (BEY_CORE_CLASS(Vector2) && typeClass == BEY_CORE_CLASS(Vector2)->Class)
					return FieldType::Vector2;

				if (BEY_CORE_CLASS(Vector3) && typeClass == BEY_CORE_CLASS(Vector3)->Class)
					return FieldType::Vector3;

				if (BEY_CORE_CLASS(Vector4) && typeClass == BEY_CORE_CLASS(Vector4)->Class)
			...
			{
				auto entityClass = BEY_CORE_CLASS(Entity);

				if (entityClass && mono_class_is_assignable_from(typeClass, entityClass->Class))
					return FieldType::Entity;

				if (BEY_CORE_CLASS(Prefab) && typeClass == BEY_CORE_CLASS(Prefab)->Class)
					return FieldType::Prefab;

				if (BEY_CORE_CLASS(Mesh) && typeClass == BEY_CORE_CLASS(Mesh)->Class)
					return FieldType::Mesh;

				if (BEY_CORE_CLASS(StaticMesh) && typeClass == BEY_CORE_CLASS(StaticMesh)->Class)
					return FieldType::StaticMesh;

				if (BEY_CORE_CLASS(Material) && typeClass == BEY_CORE_CLASS(Material)->Class)
					return FieldType::Material;

				if (BEY_CORE_CLASS(PhysicsMaterial) && typeClass == BEY_CORE_CLASS(PhysicsMaterial)->Class)
					return FieldType::PhysicsMaterial;

				if (BEY_CORE_CLASS(Texture2D) && typeClass == BEY_CORE_CLASS(Texture2D)->Class)
					return FieldType::Texture2D;

				if (BEY_CORE_CLASS(Scene) && typeClass == BEY_CORE_CLASS(Scene)->Class)
	...
	eastl::string ScriptUtils::MonoStringToUTF8(MonoString* monoString)
	{
	...
	MonoString* ScriptUtils::UTF8StringToMono(const eastl::string& str)
	{
	...
	eastl::string ScriptUtils::GetCurrentStackTrace()
	{
	...
	{
		BEY_CORE_VERIFY(!arrayClass.empty(), "Cannot create managed array of no type");

		ManagedClass* klass = ScriptCache::GetManagedClassByName(arrayClass);
		BEY_CORE_VERIFY(klass, "Unable to find array class");
	...
	{
		BEY_CORE_VERIFY(arrayClass, "Cannot create managed array of no type");
	...
	void ManagedArrayUtils::SetValueInternal(MonoArray* arr, uintptr_t index, void* data)
	{
		...
		if (index >= length)
		{
			BEY_CORE_WARN_TAG("ScriptEngine", "Index out of bounds in C# array!");
			return;
		}
