...
namespace Beyond
{
	...
	public:
		...
		void WriteString(const eastl::string& string);
		...
		template<typename Key, typename Value>
		void WriteMap(const eastl::unordered_map<Key, Value>& map, bool writeSize = true)
		{
			if (writeSize)
				WriteRaw<uint32_t>((uint32_t)map.size());
		...
		void WriteMap(const std::unordered_map<eastl::string, Value>& map, bool writeSize = true)
		{
			if (writeSize)
				WriteRaw<uint32_t>((uint32_t)map.size());
		...
		template<typename Value>
		void WriteMap(const eastl::unordered_map<eastl::string, Value>& map, bool writeSize = true)
		{
			if (writeSize)
				WriteRaw<uint32_t>((uint32_t)map.size());
	...
	template<>
	inline void StreamWriter::WriteArray(const std::vector<eastl::string>& array, bool writeSize)
	{
		if (writeSize)
			WriteRaw<uint32_t>((uint32_t)array.size());
