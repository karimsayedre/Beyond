...
#include "Beyond/Audio/SoundObject.h"
...
namespace Beyond {
	...
	void AssetPack::AddAsset(Ref<Asset> asset)
	{
		...
		{
			BEY_CORE_WARN_TAG("AssetPack", "Asset already present in asset pack");
			return;
		}
	...
	Buffer AssetPack::ReadAppBinary()
	{
		...
		BEY_CORE_VERIFY(m_File.Index.PackedAppBinarySize == (buffer.Size + sizeof(uint32_t)));
		return buffer;
	}
...
#define DEBUG_PRINT 1
		...
		for (const auto& [handle, metadata] : registry)
		{
			if (metadata.Type == AssetType::Scene)
			{
				...
				BEY_CORE_TRACE("Deserializing Scene: {}", metadata.FilePath);
				if (serializer.Deserialize(Project::GetAssetDirectory() / metadata.FilePath))
				{
					...
					BEY_CORE_TRACE("  Scene has {} used assets", sceneAssetList.size());
				...
				{
					BEY_CONSOLE_LOG_ERROR("Failed to deserialize Scene: {} ({})", metadata.FilePath, handle);
				}
		...
		BEY_CONSOLE_LOG_INFO("Project contains {} used assets", fullAssetList.size());

#if DEBUG_PRINT
		BEY_CORE_TRACE("Complete AssetPack:");

		for (AssetHandle handle : fullAssetList)
		{
			const auto& metadata = Project::GetEditorAssetManager()->GetMetadata(handle);
			BEY_CORE_TRACE("{}: {} ({})", Utils::AssetTypeToString(metadata.Type), metadata.FilePath, metadata.Handle);
		}
#endif
		...
		BEY_CORE_TRACE_TAG("Asset Pack", "Serialized Assets:");
		for (const auto& [handle, info] : serializedAssets)
		{
			const auto& metadata = Project::GetEditorAssetManager()->GetMetadata(handle);
			BEY_CORE_TRACE_TAG("Asset Pack", "{}: {} (offset = {}, size = {})", Utils::AssetTypeToString(metadata.Type), metadata.FilePath, info.PackedOffset, info.PackedSize);
		}
	...
	Ref<AssetPack> AssetPack::Load(const std::filesystem::path& path)
	{
		...
		{
			BEY_CORE_INFO("-----------------------------------------------------");
			BEY_CORE_INFO("AssetPack Dump {}", assetPack->m_Path);
			BEY_CORE_INFO("-----------------------------------------------------");
			std::unordered_map<AssetType, uint32_t> typeCounts;
			std::unordered_set<AssetHandle> duplicatePreventionSet;
			for (const auto& [sceneHandle, sceneInfo] : index.Scenes)
			{
				BEY_CORE_INFO("Scene {}:", sceneHandle);
				for (const auto& [assetHandle, assetInfo] : sceneInfo.Assets)
				{
					BEY_CORE_INFO("  {} - {}", Utils::AssetTypeToString((AssetType)assetInfo.Type), assetHandle);
			...
			BEY_CORE_INFO("-----------------------------------------------------");
			BEY_CORE_INFO("Summary:");
			for (const auto& [type, count] : typeCounts)
			{
				BEY_CORE_INFO("  {} {}", count, Utils::AssetTypeToString(type));
			}
			BEY_CORE_INFO("-----------------------------------------------------");
		}
#endif
