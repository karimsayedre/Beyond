...
namespace Beyond
{
	...
	public:
		...
		void ReadString(eastl::string& string);
		...
		template<typename Key, typename Value>
		void ReadMap(eastl::unordered_map<Key, Value>& map, uint32_t size = 0)
		{
			if (size == 0)
				ReadRaw<uint32_t>(size);
		...
		void ReadMap(std::unordered_map<eastl::string, Value>& map, uint32_t size = 0)
		{
			if (size == 0)
				ReadRaw<uint32_t>(size);
		...
		template<typename Value>
		void ReadMap(eastl::unordered_map<eastl::string, Value>& map, uint32_t size = 0)
		{
			if (size == 0)
				ReadRaw<uint32_t>(size);
		...
		template<typename T>
		void ReadArray(std::set<T>& set, uint32_t size = 0)
		{
			if (size == 0)
				ReadRaw<uint32_t>(size);
	...
	template<>
	inline void StreamReader::ReadArray(std::vector<eastl::string>& array, uint32_t size)
	{
		if (size == 0)
			ReadRaw<uint32_t>(size);
