...
namespace YAML {
	...
	template<>
	struct convert<Beyond::AssetHandle>
	{
		static Node encode(const Beyond::AssetHandle& rhs)
		{
			Node node;
		...
		static bool decode(const Node& node, Beyond::AssetHandle& rhs)
		{
	...
	private:
		...
		static Node encodeObject(const choc::value::ValueView& object)
		{
			...
			const bool isUUID = Beyond::Utils::IsAssetHandle(object);
			if (isUUID)
			{
				const auto& value = object["Value"];
				BEY_CORE_ASSERT(value.isInt64());
	...
	private:
		...
		static bool decodeObject(const Node& node, choc::value::Value& rhs)
		{
			...
			// So that in the readable YAML it is shown as the correct uint64_t,
			// while for choc::Value it's casted to int64_t
			const bool isUUID = Beyond::Utils::IsAssetHandle(rhs) && node["Value"];
			if (isUUID)
			{
