...
namespace Beyond::Utils {
	...
	std::vector<std::string> SplitStringAndKeepDelims(const std::string& str)
	{
		...
		for (size_t i = 0; i < str.size(); ++i)
		{
			...
			if (std::isalnum(c)) // Part of a word
			{
				current += c;
			}
			...
			else // Other delimiters (non-space)
			{
				if (!current.empty())
				{
					result.push_back(current);
	...
	std::string BytesToString(uint64_t bytes)
	{
		...
		if (bytes > GB)
			snprintf(buffer, 32, "%.4f GB", (float)bytes / (float)GB);
		else if (bytes > MB)
			snprintf(buffer, 32, "%.4f MB", (float)bytes / (float)MB);
		else if (bytes > KB)
			snprintf(buffer, 32, "%.4f KB", (float)bytes / (float)KB);
		else
			snprintf(buffer, 32, "%.4f bytes", (float)bytes);
	...
	std::string ReadFileAndSkipBOM(const std::filesystem::path& filepath)
	{
		...
		if (in)
		{
			...
			in.read(result.data() + (skippedChars != 0 ? 1 : 0), fileSize);
