...
#include "Beyond/Asset/AssetTypes.h"
...
namespace eastl {
	template <>
	struct hash<Beyond::AssetHandle>
	{
		size_t operator()(const Beyond::AssetHandle& handle) const
		{
			// Define the hash calculation for AssetHandle
			return std::hash<std::uint64_t>()(handle);
		}
	};
}
...
namespace Beyond {
	...
	public:
		...
		std::atomic_uint16_t Flags = (uint16_t)AssetFlag::None;
