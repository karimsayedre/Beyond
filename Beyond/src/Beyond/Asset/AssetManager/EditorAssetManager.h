...
#include "Beyond/Asset/AssetRegistry.h"
	...
	public:
		...
		Ref<T> CreateNewAsset(const std::string& filename, const std::string& directoryPath, Args&&... args)
		{
			...
			/*if (FileExists(metadata))
			{
				...
				{
					eastl::string nextFilePath = directoryPath + "/" + metadata.FilePath.stem().string();
		...
		AssetHandle CreateNamedMemoryOnlyAsset(const eastl::string& name, TArgs&&... args)
		{
