...
#include "Beyond/Asset/AssetExtensions.h"
...
namespace Beyond {
	...
	Ref<Asset> EditorAssetManager::GetAsset(AssetHandle assetHandle)
	{
		...
		if (IsMemoryAsset(assetHandle))
			return m_MemoryAssets.at(assetHandle);
	...
	std::filesystem::path EditorAssetManager::GetRelativePath(const std::filesystem::path& filepath)
	{
		...
		if (temp.find(Project::GetAssetDirectory().string()) != eastl::string::npos)
		{
	...
	bool EditorAssetManager::ReloadData(AssetHandle assetHandle)
	{
		...
		if (!metadata.IsValid())
		{
			BEY_CORE_ERROR("Trying to reload invalid asset");
			return false;
		}
	...
	{
		BEY_CORE_INFO("[AssetManager] Loading Asset Registry");
		...
		if (!handles)
		{
			BEY_CORE_ERROR("[AssetManager] Asset Registry appears to be corrupted!");
		...
		for (auto entry : handles)
		{
			...
			{
				BEY_CORE_WARN_TAG("AssetManager", "Mismatch between stored AssetType and extension type when reading asset registry!");
				metadata.Type = GetAssetTypeFromPath(filepath);
			}

			if (!FileSystem::Exists(GetFileSystemPath(metadata)))
			{
				BEY_CORE_WARN("[AssetManager] Missing asset '{0}' detected in registry file, trying to locate...", metadata.FilePath);
				...
				for (auto& pathEntry : std::filesystem::recursive_directory_iterator(Project::GetAssetDirectory()))
				{
					...
					if (bestScore > 0)
						BEY_CORE_WARN("[AssetManager] Multiple candidates found...");
					...
					{
						if (filepath.find(part) != eastl::string::npos)
							score++;
					}

					BEY_CORE_WARN("'{0}' has a score of {1}, best score is {2}", path.string(), score, bestScore);
				...
				{
					BEY_CORE_ERROR("[AssetManager] Failed to locate a potential match for '{0}'", metadata.FilePath);
					continue;
				}
				...
				BEY_CORE_WARN("[AssetManager] Found most likely match '{0}'", metadata.FilePath);
			}

			if (metadata.Handle == 0)
			{
				BEY_CORE_WARN("[AssetManager] AssetHandle for {0} is 0, this shouldn't happen.", metadata.FilePath);
				continue;
			}
		...
		BEY_CORE_INFO("[AssetManager] Loaded {0} asset entries", m_AssetRegistry.Count());
	}
	...
	void EditorAssetManager::WriteRegistryToFile()
	{
		...
		BEY_CORE_INFO("[AssetManager] serializing asset registry with {0} entries", sortedMap.size());
