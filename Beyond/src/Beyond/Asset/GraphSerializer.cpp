...
#include "Beyond/Editor/NodeGraphEditor/NodeEditorModel.h"
#include "Beyond/Editor/NodeGraphEditor/NodeGraphUtils.h"
...
namespace Beyond {
	...
	namespace Utils {
		...
		StorageKind StorageKindFromString(const std::string_view& storageKindStr)
		{
			...
			BEY_CORE_ASSERT(false, "Unknown Storage Kind");
		...
		NodeType NodeTypeFromString(const std::string_view& nodeTypeStr)
		{
			...
			BEY_CORE_ASSERT(false, "Unknown Node Type");
	...
	{
		BEY_CORE_VERIFY(false, "DefaultGraphSerialized should not be used.");
	...
	void DefaultGraphSerializer::SerializeNodes(YAML::Emitter& out, const std::vector<Node*>& nodes, std::function<void(YAML::Emitter&, const Node*)> nodeCallback)
	{
		...
		for (const auto node : nodes)
		{
			...
			BEY_SERIALIZE_PROPERTY(ID, node->ID, out);
			BEY_SERIALIZE_PROPERTY(Category, node->Category, out);
			BEY_SERIALIZE_PROPERTY(Name, node->Name, out);
			if (!node->Description.empty())
			{
				BEY_SERIALIZE_PROPERTY(Description, node->Description, out);
			}
			BEY_SERIALIZE_PROPERTY(Color, nodeColOut, out);
			BEY_SERIALIZE_PROPERTY(Type, Utils::NodeTypeToString(node->Type), out);
			BEY_SERIALIZE_PROPERTY(Size, nodeSizeOut, out);
			BEY_SERIALIZE_PROPERTY(Location, node->State, out);
			...
			for (auto in : node->Inputs)
			{
				...
				BEY_SERIALIZE_PROPERTY(ID, in->ID, out);
				BEY_SERIALIZE_PROPERTY(Name, in->Name, out);
				BEY_SERIALIZE_PROPERTY(Type, std::string(in->GetTypeString()), out);
				BEY_SERIALIZE_PROPERTY(Storage, Utils::StorageKindToString(in->Storage), out);
				BEY_SERIALIZE_PROPERTY(Value, in->Value, out);
			...
			for (auto outp : node->Outputs)
			{
				...
				BEY_SERIALIZE_PROPERTY(ID, outp->ID, out);
				BEY_SERIALIZE_PROPERTY(Name, outp->Name, out);

				BEY_SERIALIZE_PROPERTY(Type, std::string(outp->GetTypeString()), out);
				BEY_SERIALIZE_PROPERTY(Storage, Utils::StorageKindToString(outp->Storage), out);
				BEY_SERIALIZE_PROPERTY(Value, outp->Value, out);
	...
	void DefaultGraphSerializer::SerializeLinks(YAML::Emitter& out, const std::vector<Link>& links, std::function<void(YAML::Emitter&, const Link&)> linkCallback)
	{
		...
		for (const auto& link : links)
		{
			...
			BEY_SERIALIZE_PROPERTY(ID, link.ID, out);
			BEY_SERIALIZE_PROPERTY(StartPinID, link.StartPinID, out);
			BEY_SERIALIZE_PROPERTY(EndPinID, link.EndPinID, out);
			BEY_SERIALIZE_PROPERTY(Color, colOut, out);
	...
	[[nodiscard]] std::optional<std::vector<PinCandidate>> TryLoadInputs(const YAML::Node& inputs, const NodeCandidate& node)
	{
		...
		for (const auto& in : inputs)
		{
			...
			BEY_DESERIALIZE_PROPERTY(ID, ID, in, uint64_t(0));
			BEY_DESERIALIZE_PROPERTY(Name, pinName, in, std::string());
			BEY_DESERIALIZE_PROPERTY(Type, pinType, in, std::string());
			BEY_DESERIALIZE_PROPERTY(Storage, pinStorage, in, std::string());
			//BEY_DESERIALIZE_PROPERTY(Value, valueStr, in, std::string());
			BEY_DESERIALIZE_PROPERTY(Value, value, in, choc::value::Value());
			...
			auto parseCustomValueType = [](const std::string& valueString) -> choc::value::Value
			{
				...
				{
					BEY_CORE_ASSERT(false, "Failed to deserialize custom value type, missing \"TypeName\" property.");
					return {};
				}
				...
				{
					BEY_CORE_ASSERT(false, "Failed to load custom value type. It must be serialized as object.")
				}
			...
			if (!isCustomValueType)
			{
				if (storage != StorageKind::Array && !v.isString() && !v.isVoid()) // string can be name of connected Graph Input
				{
					switch (type)
					{
						case Beyond::EPinType::Int: v = choc::value::createInt32(v.get<int32_t>()); break;
						case Beyond::EPinType::Float: v = choc::value::createFloat32(v.get<float>()); break;
						default:
							break;
					}
				}
				else if (storage == StorageKind::Array && v.isArray() && v.size() && !v[0].isString())
				{
					for (auto& element : v)
					{
						switch (type)
						{
							case Beyond::EPinType::Int: element = choc::value::createInt32(element.get<int32_t>()); break;
							case Beyond::EPinType::Float: element = choc::value::createFloat32(element.get<float>()); break;
							default:
								break;
						}
					}
	...
	[[nodiscard]] std::optional<std::vector<PinCandidate>> TryLoadOutputs(const YAML::Node& outputs, const NodeCandidate& node)
	{
		...
		for (const auto& out : outputs)
		{
			...
			BEY_DESERIALIZE_PROPERTY(ID, ID, out, uint64_t(0));
			BEY_DESERIALIZE_PROPERTY(Name, pinName, out, std::string());
			BEY_DESERIALIZE_PROPERTY(Type, pinType, out, std::string());
			BEY_DESERIALIZE_PROPERTY(Storage, pinStorage, out, std::string());
			//BEY_DESERIALIZE_PROPERTY(Value, valueStr, out, std::string());
			BEY_DESERIALIZE_PROPERTY(Value, value, out, choc::value::Value());
	...
	void DefaultGraphSerializer::TryLoadNodes(YAML::Node& data, std::vector<Node*>& nodes, const DeserializationFactory& factory)
	{
		...
		for (auto node : data["Nodes"])
		{
			...
			BEY_DESERIALIZE_PROPERTY(ID, nodeID, node, uint64_t(0));
			BEY_DESERIALIZE_PROPERTY(Category, nodeCategory, node, std::string());
			BEY_DESERIALIZE_PROPERTY(Name, nodeName, node, std::string());
			BEY_DESERIALIZE_PROPERTY(Description, nodeDesc, node, std::string());
			if (node["Colour"])
			{
				BEY_DESERIALIZE_PROPERTY(Colour, nodeColor, node, glm::vec4(1.0f, 1.0f, 1.0f, 1.0f));
			}
			else
			{
				BEY_DESERIALIZE_PROPERTY(Color, nodeColor, node, glm::vec4(1.0f, 1.0f, 1.0f, 1.0f));
			}
			BEY_DESERIALIZE_PROPERTY(Type, nodeTypeStr, node, std::string());	// TODO: YAML serialization for enums to/from string
			BEY_DESERIALIZE_PROPERTY(Size, nodeSize, node, glm::vec2());
			BEY_DESERIALIZE_PROPERTY(Location, location, node, std::string());
			...
			if (newNode->Inputs.size() != candidate.NumInputs || newNode->Outputs.size() != candidate.NumOutputs)
			{
				...
				BEY_CONSOLE_LOG_WARN("Deserialized Node topology doesn't match factory Node '" + candidate.Name + "'.");
			}
	...
	void DefaultGraphSerializer::TryLoadLinks(YAML::Node& data, std::vector<Link>& links, std::function<void(YAML::Node&, Link&)> linkCallback)
	{
		for (auto link : data["Links"])
		{
			...
			BEY_DESERIALIZE_PROPERTY(ID, ID, link, uint64_t(0));
			BEY_DESERIALIZE_PROPERTY(StartPinID, StartPinID, link, uint64_t(0));
			BEY_DESERIALIZE_PROPERTY(EndPinID, EndPinID, link, uint64_t(0));
			if (link["Colour"])
			{
				BEY_DESERIALIZE_PROPERTY(Colour, color, link, glm::vec4(1.0f, 1.0f, 1.0f, 1.0f));
			}
			else
			{
				BEY_DESERIALIZE_PROPERTY(Color, color, link, glm::vec4(1.0f, 1.0f, 1.0f, 1.0f));
			}
	...
	std::string DefaultGraphSerializer::SerializeToYAML(Ref<SoundGraphAsset> soundGraphAsset) const
	{
		...
		for (auto& node : graph->Nodes)
		{
			...
			BEY_SERIALIZE_PROPERTY(Category, node.Category, out);
			BEY_SERIALIZE_PROPERTY(Name, node.Name, out);
			BEY_SERIALIZE_PROPERTY(Colour, nodeColOut, out);
			BEY_SERIALIZE_PROPERTY(Type, NodeTypeToString(node.Type), out);
			BEY_SERIALIZE_PROPERTY(Size, nodeSizeOut, out);
			BEY_SERIALIZE_PROPERTY(Location, node.State, out);
			...
			for (auto& in : node.Inputs)
			{
				...
				BEY_SERIALIZE_PROPERTY(ID, in.ID, out);
				BEY_SERIALIZE_PROPERTY(Name, in.Name, out);
				BEY_SERIALIZE_PROPERTY(Type, PinTypeToString(in.Type), out);
				BEY_SERIALIZE_PROPERTY(Storage, StorageKindToString(in.Storage), out);
				BEY_SERIALIZE_PROPERTY(Value, choc::json::toString(in.Value), out);
				out << YAML::EndMap; // in
			}
			out << YAML::EndSeq; // Inputs
			...
			for (auto& outp : node.Outputs)
			{
				...
				BEY_SERIALIZE_PROPERTY(ID, outp.ID, out);
				BEY_SERIALIZE_PROPERTY(Name, outp.Name, out);
				BEY_SERIALIZE_PROPERTY(Type, PinTypeToString(outp.Type), out);
				BEY_SERIALIZE_PROPERTY(Storage, StorageKindToString(outp.Storage), out);
				BEY_SERIALIZE_PROPERTY(Value, choc::json::toString(outp.Value), out);
				out << YAML::EndMap; // outp
			}
			out << YAML::EndSeq; // Outputs
		...
		for (auto& link : graph->Links)
		{
			...
			BEY_SERIALIZE_PROPERTY(StartPinID, link.StartPinID, out);
			BEY_SERIALIZE_PROPERTY(EndPinID, link.EndPinID, out);
			BEY_SERIALIZE_PROPERTY(Colour, colOut, out);
		...
		BEY_SERIALIZE_PROPERTY(GraphState, soundGraphAsset->GraphState, out);
	...
	bool DefaultGraphSerializer::DeserializeFromYAML(const std::string& yamlString, Ref<SoundGraphAsset>& soundGraphAsset) const
	{
		...
		for (auto& node : data["Nodes"])
		{
			...
			BEY_DESERIALIZE_PROPERTY(Category, nodeCategory, node, std::string());
			BEY_DESERIALIZE_PROPERTY(Name, nodeName, node, std::string());
			BEY_DESERIALIZE_PROPERTY(Colour, nodeCol, node, glm::vec4(1.0f, 1.0f, 1.0f, 1.0f));
			BEY_DESERIALIZE_PROPERTY(Type, nodeType, node, std::string());
			BEY_DESERIALIZE_PROPERTY(Size, nodeS, node, glm::vec2());
			BEY_DESERIALIZE_PROPERTY(Location, location, node, std::string());
		...
		for (auto& link : data["Links"])
		{
			...
			BEY_DESERIALIZE_PROPERTY(StartPinID, StartPinID, link, uint64_t(0));
			BEY_DESERIALIZE_PROPERTY(EndPinID, EndPinID, link, uint64_t(0));
			BEY_DESERIALIZE_PROPERTY(Colour, colour, link, glm::vec4(1.0f, 1.0f, 1.0f, 1.0f));
		...
		BEY_DESERIALIZE_PROPERTY(GraphState, soundGraphAsset->GraphState, data, std::string());
		return true;
	}
#endif
