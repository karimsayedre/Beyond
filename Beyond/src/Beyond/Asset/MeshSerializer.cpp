...
namespace Beyond {
	...
	bool MeshSourceSerializer::TryLoadData(const AssetMetadata& metadata, Ref<Asset>& asset) const
	{
		...
		//if (metadata.FilePath.extension() == ".gltf" || metadata.FilePath.extension() == ".glb")
		//{
		//	GltfMeshImporter importer(Project::GetEditorAssetManager()->GetFileSystemPathString(metadata));
		//	meshSource = importer.ImportToMeshSource();
		//}
		//else
		{
	...
	void MeshSerializer::Serialize(const AssetMetadata& metadata, const Ref<Asset>& asset) const
	{
		...
		std::string yamlString = SerializeToYAML(mesh, metadata.FilePath.string());

		std::filesystem::path serializePath = Project::GetActive()->GetAssetDirectory() / metadata.FilePath;
		BEY_CORE_WARN("Serializing to {0}", serializePath.string());
		std::ofstream fout(serializePath);

		if (!fout.is_open())
		{
			BEY_CORE_ERROR("Failed to serialize mesh to file '{0}'", serializePath);
//			BEY_CORE_ASSERT(false, "GetLastError() = {0}", GetLastError());
			return;
		}
	...
	bool MeshSerializer::SerializeToAssetPack(AssetHandle handle, FileStreamWriter& stream, AssetSerializationInfo& outInfo) const
	{
		...
		std::string yamlString = SerializeToYAML(mesh, mesh->GetMeshSource()->GetFilePath());
	...
	std::string MeshSerializer::SerializeToYAML(Ref<Mesh> mesh, const std::string& name) const
	{
		YAML::Emitter out;
		out << YAML::BeginMap;
		out << YAML::Key << "Mesh";
	...
	bool MeshSerializer::DeserializeFromYAML(const std::string& yamlString, Ref<Mesh>& targetMesh) const
	{
		...
		std::string name = rootNode["Name"].as<std::string>("Unnamed Mesh");

		auto submeshIndices = rootNode["SubmeshIndices"].as<std::vector<uint32_t>>();
		targetMesh = Ref<Mesh>::Create(meshSource, name, submeshIndices);
		return true;
	}
	...
	void StaticMeshSerializer::Serialize(const AssetMetadata& metadata, const Ref<Asset>& asset) const
	{
		...
		std::string yamlString = SerializeToYAML(staticMesh, metadata.FilePath.string());

		auto serializePath = Project::GetActive()->GetAssetDirectory() / metadata.FilePath;
		std::ofstream fout(serializePath);
		BEY_CORE_ASSERT(fout.good());
		if (fout.good())
			fout << yamlString;
	}
	...
	{
		// TODO: this needs to open up a Beyond Mesh file and make sure
	...
	bool StaticMeshSerializer::SerializeToAssetPack(AssetHandle handle, FileStreamWriter& stream, AssetSerializationInfo& outInfo) const
	{
		...
		std::string yamlString = SerializeToYAML(staticMesh, staticMesh->GetMeshSource()->GetFilePath());
		outInfo.Offset = stream.GetStreamPosition();
		stream.WriteString(yamlString);
		outInfo.Size = stream.GetStreamPosition() - outInfo.Offset;
		return true;
	...
	std::string StaticMeshSerializer::SerializeToYAML(Ref<StaticMesh> staticMesh, const std::string& name) const
	{
		YAML::Emitter out;
		out << YAML::BeginMap;
		out << YAML::Key << "Mesh";
	...
	bool StaticMeshSerializer::DeserializeFromYAML(const std::string& yamlString, Ref<StaticMesh>& targetStaticMesh) const
	{
		...
		if (!meshSource)
			return false; // TODO: feedback to the user
		std::string name = rootNode["Name"].as<std::string>("Unnamed StaticMesh");

		auto submeshIndices = rootNode["SubmeshIndices"].as<std::vector<uint32_t>>();
		targetStaticMesh = Ref<StaticMesh>::Create(meshSource, name, submeshIndices);
