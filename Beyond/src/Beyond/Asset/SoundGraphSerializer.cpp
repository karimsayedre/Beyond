...
#include "Beyond/Audio/SoundGraph/Utils/SoundGraphCache.h"
...
#include "Beyond/Editor/NodeGraphEditor/SoundGraph/SoundGraphNodes.h"
...
namespace SG = Beyond::SoundGraph;
...
namespace Beyond {
	...
	bool SoundGraphGraphSerializer::TryLoadData(const AssetMetadata& metadata, Ref<Asset>& asset) const
	{
		...
		if (!stream.is_open())
		{
			BEY_CONSOLE_LOG_ERROR("Failed to open SoundGraph asset file to deserialize: {}", filepath.string());
			return false;
		}
		...
		{
			BEY_CONSOLE_LOG_ERROR("Failed to deserialize SoundGraph: {}", filepath.string());
			return false;
		}
	...
	std::string SoundGraphGraphSerializer::SerializeToYAML(Ref<SoundGraphAsset> graph) const
	{
		...
		// Graph IO
		BEY_SERIALIZE_PROPERTY(GraphInputs, graph->GraphInputs.ToExternalValue(), out);
		BEY_SERIALIZE_PROPERTY(GraphOutputs, graph->GraphOutputs.ToExternalValue(), out);
		BEY_SERIALIZE_PROPERTY(LocalVariables, graph->LocalVariables.ToExternalValue(), out);

		// Graph State
		BEY_SERIALIZE_PROPERTY(GraphState, graph->GraphState, out);
		...
		for (const auto& wave : graph->WaveSources)
		{
			...
			{
				BEY_SERIALIZE_PROPERTY(WaveAssetHandle, wave, out);
			}
		...
		if (graph->Prototype)
		{
			...
			BEY_SERIALIZE_PROPERTY(PrototypeName, prototype->DebugName, out);
			BEY_SERIALIZE_PROPERTY(PrototypeID, prototype->ID, out);

			auto serializeEndpoint = [&out](const SG::Prototype::Endpoint& endpoint)
			{
				out << YAML::BeginMap; // endpoing
				{
					BEY_SERIALIZE_PROPERTY(EndpointID, (uint32_t)endpoint.EndpointID, out);
					BEY_SERIALIZE_PROPERTY(DefaultValue, choc::json::toString(endpoint.DefaultValue), out); // TODO: need to store type
				}
			...
			serializeSequence("OutputChannelIDs", [&]
			{
				...
				{
					out << YAML::BeginMap;
					BEY_SERIALIZE_PROPERTY(ID, (uint32_t)id, out);
					out << YAML::EndMap;
				}
			});
			...
			serializeSequence("Nodes", [&]
			{
				...
				{
					out << YAML::BeginMap;
					BEY_SERIALIZE_PROPERTY(NodeTypeID, (uint32_t)node.NodeTypeID, out);
					BEY_SERIALIZE_PROPERTY(ID, (uint64_t)node.ID, out);
			...
			serializeSequence("Connections", [&]
			{
				for (const auto& connection : prototype->Connections)
				{
					out << YAML::BeginMap; // connection
					{
						...
						out << YAML::BeginMap; // endpoing
						{
							BEY_SERIALIZE_PROPERTY(NodeID, (uint64_t)connection.Source.NodeID, out);
							BEY_SERIALIZE_PROPERTY(EndpointID, (uint32_t)connection.Source.EndpointID, out);
						}
						out << YAML::EndMap; // endpoint
						...
						out << YAML::BeginMap; // endpoing
						{
							BEY_SERIALIZE_PROPERTY(NodeID, (uint64_t)connection.Destination.NodeID, out);
							BEY_SERIALIZE_PROPERTY(EndpointID, (uint32_t)connection.Destination.EndpointID, out);
						}
						out << YAML::EndMap; // endpoint

						BEY_SERIALIZE_PROPERTY(Type, (uint32_t)connection.Type, out);
					}
		...
		BEY_SERIALIZE_PROPERTY(CachedPrototype, pathStr, out);
	...
	bool SoundGraphGraphSerializer::DeserializeFromYAML(const std::string& yamlString, Ref<SoundGraphAsset>& graph) const
	{
		...
		// Graph IO (must be deserialized before nodes to be able to spawn IO nodes)
		{
			...
			BEY_DESERIALIZE_PROPERTY(GraphInputs, graphInputsValue, data, choc::value::Value());
			BEY_DESERIALIZE_PROPERTY(GraphOutputs, graphOutputsValue, data, choc::value::Value());
			BEY_DESERIALIZE_PROPERTY(LocalVariables, graphLocalVariablesValue, data, choc::value::Value());
		...
		auto constructNode = [&graph](const NodeCandidate& candidate, const std::optional<std::vector<PinCandidate>>& inputs, const std::optional<std::vector<PinCandidate>>& outputs)
		{
			...
			if (optType.has_value())
			{
				...
				switch (*optType)
				{
					...
					default:
						BEY_CORE_ASSERT(false, "Deserialized invalid SoundGraph property type.");
						break;
				}
			}
			else
			...
			BEY_CORE_ASSERT(!newNode ^ (newNode->Type == candidate.Type));
		...
		auto deserializePin = [](const PinCandidate& candidate, Pin* factoryPin, const NodeCandidate& node)
		{
			...
			if (candidateType != factoryPin->GetType())
			{
				...
				BEY_CONSOLE_LOG_ERROR("Pin type of the deserialized Node Pin ({0} - '{1}' ({2})) does not match the Pin type of the factory Node Pin ({0} - '{3}' ({4})).",
			...
			else if (candidate.Name != factoryPin->Name)
			{
				...
				BEY_CONSOLE_LOG_ERROR("Pin name of the deserialized Node Pin ({0} - '{1}') does not match the Pin name of the factory Node Pin ({0} - '{2}').", node.Name, candidate.Name, factoryPin->Name);
				return false;
			}
			...
			auto parseCustomValueObject = [&pin](std::string_view valueString) -> choc::value::Value
			{
				...
				else
				{
					...
					BEY_CORE_ASSERT(false, "Type of the deserialized Node Pin does not match the type of the factory Node Pin.");
			...
			{
				// This can trigger if we've trying to load old version of a Node.
				BEY_CONSOLE_LOG_ERROR("Value type of the deserialized Node Pin ({0} - '{1}') does not match the Value type of the factory Node Pin ({0} - '{2}').", node.Name, candidate.Name, factoryPin->Name);
				return false;
			}
		};
		...
		// Graph State
		BEY_DESERIALIZE_PROPERTY(GraphState, graph->GraphState, data, std::string());
		...
		for (auto wave : data["Waves"])
		{
			...
			BEY_DESERIALIZE_PROPERTY(WaveAssetHandle, ID, wave, uint64_t(0));
			graph->WaveSources.push_back(ID);
		}

		// Graph Prototype
		BEY_DESERIALIZE_PROPERTY(CachedPrototype, graph->CachedPrototype, data, std::string());
		return true;
	}
