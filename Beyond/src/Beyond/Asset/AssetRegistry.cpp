...
#define BEY_ASSETREGISTRY_LOG 0
#if BEY_ASSETREGISTRY_LOG
#define ASSET_LOG(...) BEY_CORE_TRACE_TAG("ASSET", __VA_ARGS__)
#else 
#define ASSET_LOG(...)
#endif
	...
	const AssetMetadata& AssetRegistry::Get(const AssetHandle handle) const
	{
		...
		BEY_CORE_ASSERT(m_AssetRegistry.find(handle) != m_AssetRegistry.end());
