...
namespace Beyond {
	...
	std::string SerializeToYAML(Ref<AnimationGraphAsset> graph)
	{
		...
		out << YAML::BeginMap;
		{
			BEY_SERIALIZE_PROPERTY(Skeleton, graph->m_SkeletonHandle, out);
			for(const auto id : ids) {
				...
				out << YAML::BeginMap;
				{
					...
					BEY_SERIALIZE_PROPERTY(GraphState, graph->m_GraphState.at(id), out);
				}
				out << YAML::EndMap;
			}
			BEY_SERIALIZE_PROPERTY(GraphInputs, graph->Inputs.ToExternalValue(), out);
			BEY_SERIALIZE_PROPERTY(GraphOutputs, graph->Outputs.ToExternalValue(), out);
	...
	bool DeserializeFromYAML(const std::string& yamlString, Ref<AnimationGraphAsset>& graph)
	{
		...
		// Graph IO (must be deserialized before nodes to be able to spawn IO nodes)
		{
			...
			BEY_DESERIALIZE_PROPERTY(Skeleton, graph->m_SkeletonHandle, data, uint64_t(0));
		...
		auto constructNode = [&graph](const NodeCandidate& candidate, const std::optional<std::vector<PinCandidate>>& inputs, const std::optional<std::vector<PinCandidate>>& outputs)
		{
			...
			if (optType.has_value())
			{
				...
				switch (*optType)
				{
					...
					default:
						BEY_CORE_ASSERT(false, "Deserialized invalid AnimationGraph property type.");
						break;
				}
			}
			else
		...
		auto nodeCallback = [](YAML::Node& node, Node* newNode) {
			if (newNode->Name.starts_with("Blend Space"))
			{
				...
				glm::vec2 offset;
				BEY_DESERIALIZE_PROPERTY(Offset, offset, node, glm::vec2(agNode->Offset.x, agNode->Offset.y));
				agNode->Offset = { offset.x, offset.y };
				if (newNode->Name == "Blend Space")
				{
					glm::vec2 scale;
					BEY_DESERIALIZE_PROPERTY(Scale, scale, node, glm::vec2(agNode->Scale.x, agNode->Scale.y));
					agNode->Scale = { scale.x, scale.y };
					glm::vec2 lerpSecondsPerUnit;
					BEY_DESERIALIZE_PROPERTY(LerpSecondsPerUnit, lerpSecondsPerUnit, node, glm::vec2(agNode->LerpSecondsPerUnit.x, agNode->LerpSecondsPerUnit.y));
					agNode->LerpSecondsPerUnit = { lerpSecondsPerUnit.x, lerpSecondsPerUnit.y };
					BEY_DESERIALIZE_PROPERTY(ShowDetails, agNode->ShowDetails, node, false);
				}
			}
		};
		...
		auto deserializeGraph = [&graph, &factory](UUID id, YAML::Node& node) {
			...
			// Graph State
			BEY_DESERIALIZE_PROPERTY(GraphState, graph->m_GraphState[id], node, std::string());
			return true;
		};
