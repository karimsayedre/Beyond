...
#include "Beyond/Audio/AudioFileUtils.h"
#include "Beyond/Audio/SoundBank.h"
#include "Beyond/Audio/ResourceManager.h"
...
#include "Beyond/Serialization/TextureRuntimeSerializer.h"
...
namespace Beyond {
	...
	{
		asset = Texture2D::Create(TextureSpecification{ .CreateBindlessDescriptor = true, .Compress = true }, Project::GetEditorAssetManager()->GetFileSystemPathString(metadata));
		asset->Handle = metadata.Handle;

		bool result = asset.As<Texture2D>()->IsStillLoading();
	...
	{
		BEY_CORE_WARN("TextureSerializer::DeserializeFromAssetPack");
	...
	std::string MaterialAssetSerializer::SerializeToYAML(Ref<MaterialAsset> materialAsset) const
	{
		...
		out << YAML::Key << "Material" << YAML::Value;
		{
			...
			bool blend = materialAsset->GetMaterial()->GetFlag(MaterialFlag::Blend);

			BEY_SERIALIZE_PROPERTY(AlbedoColor, materialAsset->GetAlbedoColor(), out);
			BEY_SERIALIZE_PROPERTY(Emission, materialAsset->GetEmission(), out);
			BEY_SERIALIZE_PROPERTY(IOR, materialAsset->GetIOR(), out);

			BEY_SERIALIZE_PROPERTY(Specular, materialAsset->GetSpecular(), out);
			BEY_SERIALIZE_PROPERTY(SpecularColor, materialAsset->GetSpecularColor(), out);
			BEY_SERIALIZE_PROPERTY(Transmission, materialAsset->GetTransmission(), out);
			BEY_SERIALIZE_PROPERTY(AttenuationColor, materialAsset->GetAttenuationColor(), out);
			BEY_SERIALIZE_PROPERTY(AttenuationDistance, materialAsset->GetAttenuationDistance(), out);
			BEY_SERIALIZE_PROPERTY(Thickness, materialAsset->GetThickness(), out);
			BEY_SERIALIZE_PROPERTY(Clearcoat, materialAsset->GetClearcoat(), out);
			BEY_SERIALIZE_PROPERTY(ClearcoatRoughness, materialAsset->GetClearcoatRoughness(), out);
			BEY_SERIALIZE_PROPERTY(Iridescence, materialAsset->GetIridescence(), out);
			BEY_SERIALIZE_PROPERTY(IridescenceIor, materialAsset->GetIridescenceIor(), out);
			BEY_SERIALIZE_PROPERTY(IridescenceThickness, materialAsset->GetIridescenceThickness(), out);
			BEY_SERIALIZE_PROPERTY(SheenColor, materialAsset->GetSheenColor(), out);
			BEY_SERIALIZE_PROPERTY(SheenRoughness, materialAsset->GetSheenRoughness(), out);

			if (!blend)
			{
				BEY_SERIALIZE_PROPERTY(UseNormalMap, materialAsset->IsUsingNormalMap(), out);
				BEY_SERIALIZE_PROPERTY(Metalness, materialAsset->GetMetalness(), out);
				BEY_SERIALIZE_PROPERTY(Roughness, materialAsset->GetRoughness(), out);
			}

			{
				...
				BEY_SERIALIZE_PROPERTY(AlbedoMap, albedoMapHandle, out);
			}
			if (!blend)
			{
				{
					...
					BEY_SERIALIZE_PROPERTY(NormalMap, normalMapHandle, out);
				}
				{
					...
					BEY_SERIALIZE_PROPERTY(MetalnessMap, metalnessMapHandle, out);
				}
				{
					...
					BEY_SERIALIZE_PROPERTY(RoughnessMap, roughnessMapHandle, out);
				}
			}

			BEY_SERIALIZE_PROPERTY(MaterialFlags, materialAsset->GetMaterial()->GetFlags(), out);
	...
	bool MaterialAssetSerializer::DeserializeFromYAML(const std::string& yamlString, Ref<MaterialAsset>& targetMaterialAsset) const
	{
		...
		targetMaterialAsset = Ref<MaterialAsset>::Create();

		BEY_DESERIALIZE_PROPERTY(AlbedoColor, targetMaterialAsset->GetAlbedoColor(), materialNode, glm::vec4(0.8f, 0.8f, 0.8f, 1.0f));
		BEY_DESERIALIZE_PROPERTY(Emission, targetMaterialAsset->GetEmission(), materialNode, 0.0f);
		BEY_DESERIALIZE_PROPERTY(IOR, targetMaterialAsset->GetIOR(), materialNode, 1.0f);

		BEY_DESERIALIZE_PROPERTY(Specular, targetMaterialAsset->GetSpecular(), materialNode, 0.0f);
		BEY_DESERIALIZE_PROPERTY(SpecularColor, targetMaterialAsset->GetSpecularColor(), materialNode, glm::vec3(1.0f));
		BEY_DESERIALIZE_PROPERTY(Transmission, targetMaterialAsset->GetTransmission(), materialNode, 0.0f);
		BEY_DESERIALIZE_PROPERTY(AttenuationColor, targetMaterialAsset->GetAttenuationColor(), materialNode, glm::vec3(1.0f));
		BEY_DESERIALIZE_PROPERTY(AttenuationDistance, targetMaterialAsset->GetAttenuationDistance(), materialNode, 1.0f);
		BEY_DESERIALIZE_PROPERTY(Thickness, targetMaterialAsset->GetThickness(), materialNode, 1.0f);
		BEY_DESERIALIZE_PROPERTY(Clearcoat, targetMaterialAsset->GetClearcoat(), materialNode, 0.0f);
		BEY_DESERIALIZE_PROPERTY(ClearcoatRoughness, targetMaterialAsset->GetClearcoatRoughness(), materialNode, 0.0f);
		BEY_DESERIALIZE_PROPERTY(Iridescence, targetMaterialAsset->GetIridescence(), materialNode, 0.0f);
		BEY_DESERIALIZE_PROPERTY(IridescenceIor, targetMaterialAsset->GetIridescenceIor(), materialNode, 1.5f);
		BEY_DESERIALIZE_PROPERTY(IridescenceThickness, targetMaterialAsset->GetIridescenceThickness(), materialNode, 0.0f);
		BEY_DESERIALIZE_PROPERTY(SheenColor, targetMaterialAsset->GetSheenColor(), materialNode, glm::vec3(0.0f));
		BEY_DESERIALIZE_PROPERTY(SheenRoughness, targetMaterialAsset->GetSheenRoughness(), materialNode, 0.0f);


		if (!transparent)
		{
			targetMaterialAsset->SetUseNormalMap(materialNode["UseNormalMap"] ? materialNode["UseNormalMap"].as<bool>() : false);
			BEY_DESERIALIZE_PROPERTY(Metalness, targetMaterialAsset->GetMetalness(), materialNode, 0.0f);
			BEY_DESERIALIZE_PROPERTY(Roughness, targetMaterialAsset->GetRoughness(), materialNode, 0.5f);
		}

		AssetHandle albedoMap, normalMap, metalnessMap, roughnessMap;
		BEY_DESERIALIZE_PROPERTY(AlbedoMap, albedoMap, materialNode, (AssetHandle)0);
		if (!transparent)
		{
			BEY_DESERIALIZE_PROPERTY(NormalMap, normalMap, materialNode, (AssetHandle)0);
			BEY_DESERIALIZE_PROPERTY(MetalnessMap, metalnessMap, materialNode, (AssetHandle)0);
			BEY_DESERIALIZE_PROPERTY(RoughnessMap, roughnessMap, materialNode, (AssetHandle)0);
		}
		if (albedoMap)
		{
		...
		BEY_DESERIALIZE_PROPERTY(MaterialFlags, roughnessMap, materialNode, (AssetHandle)0);
		if (materialNode["MaterialFlags"])
			targetMaterialAsset->GetMaterial()->SetFlags(materialNode["MaterialFlags"].as<uint32_t>());
		targetMaterialAsset->SetBlending(transparent);
		return true;
	}
	...
	std::string SoundConfigSerializer::SerializeToYAML(Ref<SoundConfig> soundConfig) const
	{
		...
		{
			BEY_SERIALIZE_PROPERTY(AssetID, soundConfig->DataSourceAsset, out);
		}

		BEY_SERIALIZE_PROPERTY(IsLooping, (bool)soundConfig->bLooping, out);
		BEY_SERIALIZE_PROPERTY(VolumeMultiplier, soundConfig->VolumeMultiplier, out);
		BEY_SERIALIZE_PROPERTY(PitchMultiplier, soundConfig->PitchMultiplier, out);
		BEY_SERIALIZE_PROPERTY(MasterReverbSend, soundConfig->MasterReverbSend, out);
		BEY_SERIALIZE_PROPERTY(LPFilterValue, soundConfig->LPFilterValue, out);
		BEY_SERIALIZE_PROPERTY(HPFilterValue, soundConfig->HPFilterValue, out);
		...
		BEY_SERIALIZE_PROPERTY(Enabled, soundConfig->bSpatializationEnabled, out);
		BEY_SERIALIZE_PROPERTY(AttenuationModel, (int)spatialConfig->AttenuationMod, out);
		BEY_SERIALIZE_PROPERTY(MinGain, spatialConfig->MinGain, out);
		BEY_SERIALIZE_PROPERTY(MaxGain, spatialConfig->MaxGain, out);
		BEY_SERIALIZE_PROPERTY(MinDistance, spatialConfig->MinDistance, out);
		BEY_SERIALIZE_PROPERTY(MaxDistance, spatialConfig->MaxDistance, out);
		BEY_SERIALIZE_PROPERTY(ConeInnerAngle, spatialConfig->ConeInnerAngleInRadians, out);
		BEY_SERIALIZE_PROPERTY(ConeOuterAngle, spatialConfig->ConeOuterAngleInRadians, out);
		BEY_SERIALIZE_PROPERTY(ConeOuterGain, spatialConfig->ConeOuterGain, out);
		BEY_SERIALIZE_PROPERTY(DopplerFactor, spatialConfig->DopplerFactor, out);
		BEY_SERIALIZE_PROPERTY(Rollor, spatialConfig->Rolloff, out);
		BEY_SERIALIZE_PROPERTY(AirAbsorptionEnabled, spatialConfig->bAirAbsorptionEnabled, out);
		BEY_SERIALIZE_PROPERTY(SpreadFromSourceSize, spatialConfig->bSpreadFromSourceSize, out);
		BEY_SERIALIZE_PROPERTY(SourceSize, spatialConfig->SourceSize, out);
		BEY_SERIALIZE_PROPERTY(Spread, spatialConfig->Spread, out);
		BEY_SERIALIZE_PROPERTY(Focus, spatialConfig->Focus, out);
	...
	bool SoundConfigSerializer::DeserializeFromYAML(const std::string& yamlString, Ref<SoundConfig> targetSoundConfig) const
	{
		...
		if (Application::IsRuntime())
		{
			...
			if (type == AssetType::Audio)
			{
				...
				BEY_CORE_VERIFY(soundBank, "SoundBank is not loaded!");
		...
		else
		{
			if (Application::IsRuntime())
			{
				BEY_CORE_ERROR("Tried to load invalid audio source asset (UUID {0}) in SoundConfig: {1}",
		...
		BEY_DESERIALIZE_PROPERTY(IsLooping, targetSoundConfig->bLooping, data, false);
		BEY_DESERIALIZE_PROPERTY(VolumeMultiplier, targetSoundConfig->VolumeMultiplier, data, 1.0f);
		BEY_DESERIALIZE_PROPERTY(PitchMultiplier, targetSoundConfig->PitchMultiplier, data, 1.0f);
		BEY_DESERIALIZE_PROPERTY(MasterReverbSend, targetSoundConfig->MasterReverbSend, data, 0.0f);
		BEY_DESERIALIZE_PROPERTY(LPFilterValue, targetSoundConfig->LPFilterValue, data, 20000.0f);
		BEY_DESERIALIZE_PROPERTY(HPFilterValue, targetSoundConfig->HPFilterValue, data, 0.0f);
		...
		if (spConfigData)
		{
			...
			BEY_DESERIALIZE_PROPERTY(Enabled, targetSoundConfig->bSpatializationEnabled, spConfigData, false);
			...
			BEY_DESERIALIZE_PROPERTY(MinGain, spatialConfig->MinGain, spConfigData, 0.0f);
			BEY_DESERIALIZE_PROPERTY(MaxGain, spatialConfig->MaxGain, spConfigData, 1.0f);
			BEY_DESERIALIZE_PROPERTY(MinDistance, spatialConfig->MinDistance, spConfigData, 1.0f);
			BEY_DESERIALIZE_PROPERTY(MaxDistance, spatialConfig->MaxDistance, spConfigData, 1000.0f);
			BEY_DESERIALIZE_PROPERTY(ConeInnerAngle, spatialConfig->ConeInnerAngleInRadians, spConfigData, 6.283185f);
			BEY_DESERIALIZE_PROPERTY(ConeOuterAngle, spatialConfig->ConeOuterAngleInRadians, spConfigData, 6.283185f);
			BEY_DESERIALIZE_PROPERTY(ConeOuterGain, spatialConfig->ConeOuterGain, spConfigData, 0.0f);
			BEY_DESERIALIZE_PROPERTY(DopplerFactor, spatialConfig->DopplerFactor, spConfigData, 1.0f);
			BEY_DESERIALIZE_PROPERTY(Rollor, spatialConfig->Rolloff, spConfigData, 1.0f);
			BEY_DESERIALIZE_PROPERTY(AirAbsorptionEnabled, spatialConfig->bAirAbsorptionEnabled, spConfigData, true);
			BEY_DESERIALIZE_PROPERTY(SpreadFromSourceSize, spatialConfig->bSpreadFromSourceSize, spConfigData, true);
			BEY_DESERIALIZE_PROPERTY(SourceSize, spatialConfig->SourceSize, spConfigData, 1.0f);
			BEY_DESERIALIZE_PROPERTY(Spread, spatialConfig->Spread, spConfigData, 1.0f);
			BEY_DESERIALIZE_PROPERTY(Focus, spatialConfig->Focus, spConfigData, 1.0f);
		}
	...
	void ScriptFileSerializer::Serialize(const AssetMetadata& metadata, const Ref<Asset>& asset) const
	{
		...
		BEY_CORE_VERIFY(stream.is_open());

		std::ifstream templateStream("Resources/Templates/NewClassTemplate.cs");
		BEY_CORE_VERIFY(templateStream.is_open());
		...
		{
			size_t pos = 0;
			while ((pos = templateString.find(token, pos)) != eastl::string::npos)
			{
