...
namespace Beyond {
	...
	void AssetImporter::Serialize(const AssetMetadata& metadata, const Ref<Asset>& asset)
	{
		...
		{
			BEY_CORE_WARN("There's currently no importer for assets of type {0}", metadata.FilePath.stem().string());
			return;
		}
	...
	bool AssetImporter::TryLoadData(const AssetMetadata& metadata, Ref<Asset>& asset)
	{
		...
		// BEY_CORE_TRACE("AssetImporter::TryLoadData - {}", metadata.FilePath);
	...
	bool AssetImporter::SerializeToAssetPack(AssetHandle handle, FileStreamWriter& stream, AssetSerializationInfo& outInfo)
	{
		...
		{
			BEY_CORE_WARN("There's currently no serializer for assets of type {0}", metadata.FilePath.stem().string());
			return false;
		}
