...
namespace Beyond {
	...
	void ProjectSerializer::Serialize(const std::filesystem::path& filepath)
	{
		...
		out << YAML::Key << "Project" << YAML::Value;
		{
			...
			out << YAML::Key << "Audio" << YAML::Value;
			{
				...
				BEY_SERIALIZE_PROPERTY(FileStreamingDurationThreshold, userConfig.FileStreamingDurationThreshold, out);
				out << YAML::EndMap;
			}
			...
			out << YAML::Key << "Physics" << YAML::Value;
			{
				...
				if (PhysicsLayerManager::GetLayerCount() > 1)
				{
					...
					for (const auto& layer : PhysicsLayerManager::GetLayers())
					{
						...
						out << YAML::BeginMap;
						out << YAML::Key << "Name" << YAML::Value << std::string(layer.Name.c_str(), layer.Name.size());
						...
						{
							out << YAML::BeginMap;
							out << YAML::Key << "Name" << YAML::Value << std::string(collidingLayer.Name.c_str(), collidingLayer.Name.size());
							out << YAML::EndMap;
						}
						out << YAML::EndSeq;
						out << YAML::EndMap;
	...
	bool ProjectSerializer::Deserialize(const std::filesystem::path& filepath)
	{
		...
		{
			auto userConfig = MiniAudioEngine::Get().GetUserConfiguration();
			BEY_DESERIALIZE_PROPERTY(FileStreamingDurationThreshold, userConfig.FileStreamingDurationThreshold, audioNode, userConfig.FileStreamingDurationThreshold);
		...
		if (physicsNode)
		{
			...
			if (physicsLayers)
			{
				...
				{
					const auto& layerName = layer["Name"].as<std::string>();
					PhysicsLayerManager::AddLayer(eastl::string(layerName.c_str(), layer.size()), false);
				}
				...
				for (auto layer : physicsLayers)
				{
					...
					PhysicsLayer& layerInfo = PhysicsLayerManager::GetLayer(eastl::string(layerName.c_str(), layer.size()));
					...
					if (collidesWith)
					{
						...
						{
							const auto& layerName = collisionLayer["Name"].as<std::string>();
							const auto& otherLayer = PhysicsLayerManager::GetLayer(eastl::string(layerName.c_str(), layer.size()));
