...
#include "Beyond/Editor/NodeGraphEditor/AnimationGraph/AnimationGraphAsset.h"  // TODO (0x): need to separate prototype from the editor asset
...
namespace Beyond {
	...
	Scene::Scene(const eastl::string& name, bool isEditorScene, bool initalize)
	...
	void Scene::OnUpdateRuntime(Timestep ts)
	{
		...
		if (m_ShouldSimulate)
		{
			...
			{
				{
					BEY_PROFILE_FUNC("Scene::OnUpdate - C# OnUpdate");
					Timer timer;
				...
				{
					BEY_PROFILE_FUNC("Scene::OnUpdate - C# OnLateUpdate");
					Timer timer;
		...
		{	//--- Update Audio Listener ---
			...
			BEY_PROFILE_SCOPE("Scene::OnUpdate - Update Audio Listener");
		...
		{	//--- Update Audio Components ---
			...
			BEY_PROFILE_SCOPE("Scene::OnUpdate - Update Audio Components");
	...
	void Scene::OnRenderRuntime(Ref<SceneRenderer> renderer, Timestep ts)
	{
		...
		BEY_CORE_ASSERT(cameraEntity, "Scene does not contain any cameras!");
		...
		// DDGI
		{
			auto group = m_Registry.group<DDGIVolumeComponent>(entt::get<TransformComponent>);
			uint32_t DDGIVolumeIndex = 0;
			for (auto entity : group)
			{
				auto [transformComponent, volume] = group.get<TransformComponent, DDGIVolumeComponent>(entity);
				auto ent = Entity(entity, this);
				auto name = ent.HasComponent<TagComponent>() ? ent.GetComponent<TagComponent>().Tag : "NO NAME";
				auto eulerAngles = rtxgi::float3{ transformComponent.GetRotationEuler().x, transformComponent.GetRotationEuler().y, transformComponent.GetRotationEuler().z };
				m_LightEnvironment.DDGIVolumes[DDGIVolumeIndex] =
				{
					name,
					DDGIVolumeIndex,
					0,
					volume.ShowProbes,
					volume.ClearProbes, // changed to false in SceneHierarchyPanel a frame later.
					true,
					*(rtxgi::float3*)&transformComponent.Translation,
					eulerAngles,
					*(rtxgi::float3*)&transformComponent.Scale,
					*(rtxgi::int3*)&volume.ProbeCounts,
					volume.ProbeNumRays,
					volume.ProbeNumIrradianceTexels,
					volume.ProbeNumIrradianceTexels, // TODO is this correct?
					volume.ProbeNumDistanceTexels,
					volume.ProbeNumDistanceTexels, // TODO is this correct?
					volume.ProbeHysteresis,
					volume.ProbeMaxRayDistance,
					volume.ProbeDistanceExponent,
					volume.ProbeIrradianceEncodingGamma,
					volume.ProbeIrradianceThreshold,
					volume.ProbeBrightnessThreshold,
					volume.ProbeRandomRayBackfaceThreshold,
					volume.ProbeFixedRayBackfaceThreshold,
					volume.ProbeViewBias,
					volume.ProbeNormalBias,
					rtxgi::EDDGIVolumeTextureFormat::F32x4,
					rtxgi::EDDGIVolumeTextureFormat::F32x4,
					rtxgi::EDDGIVolumeTextureFormat::F16x2,
					rtxgi::EDDGIVolumeTextureFormat::F16x4,
					rtxgi::EDDGIVolumeTextureFormat::F16,
					false,
					volume.ProbeRelocationEnabled,
					volume.ClearProbes,
					volume.ProbeMinFrontfaceDistance,
					volume.ProbeClassificationEnabled,
					volume.ClearProbes,
					volume.ProbeVariabilityEnabled,
					rtxgi::EDDGIVolumeMovementType::Default, // TODO is this correct?
					rtxgi::EDDGIVolumeProbeVisType::Default, // TODO is this correct?
				};

				volume.ClearProbes = false;
		...
		renderer->BeginScene({ std::make_shared<SceneCamera>(camera), cameraViewMatrix, camera.GetPerspectiveNearClip(), camera.GetPerspectiveFarClip(), camera.GetRadPerspectiveVerticalFOV() }, ts);
		...
		// Render Static Meshes
		{
			...
			for (auto entity : group)
			{
				BEY_PROFILE_SCOPE("Scene-SubmitStaticMesh");
				...
				if (AssetManager::IsAssetHandleValid(staticMeshComponent.StaticMeshAssetHandle))
				{
					Ref<StaticMesh> staticMesh = AssetManager::GetAsset<StaticMesh>(staticMeshComponent.StaticMeshAssetHandle);
		...
		// Render Dynamic Meshes
		{
			...
			for (auto entity : view)
			{
				BEY_PROFILE_SCOPE("Scene-SubmitDynamicMesh");
				auto [transformComponent, meshComponent] = view.get<TransformComponent, MeshComponent>(entity);
				if (!meshComponent.Visible)
					continue;

				if (AssetManager::IsAssetHandleValid(meshComponent.MeshAssetHandle))
				{
					Ref<Mesh> mesh = AssetManager::GetAsset<Mesh>(meshComponent.MeshAssetHandle);
	...
	{
		BEY_PROFILE_FUNC();
		BEY_SCOPE_PERF("Scene::OnRenderEditor");
		...
		{
			auto group = m_Registry.group<DDGIVolumeComponent>(entt::get<TransformComponent>);
			m_LightEnvironment.DDGIVolumes.resize(group.size());
		...
		renderer->SetScene(this);
		renderer->BeginScene({ std::make_shared<EditorCamera>(editorCamera), editorCamera.GetViewMatrix(), editorCamera.GetNearClip(), editorCamera.GetFarClip(), editorCamera.GetVerticalFOV() }, ts);
		...
		// Render Static Meshes
		{
			...
			for (auto entity : view)
			{
				auto [transformComponent, staticMeshComponent] = view.get<TransformComponent, StaticMeshComponent>(entity);
				if (!staticMeshComponent.Visible)
					continue;

				auto staticMesh = AssetManager::GetAsset<StaticMesh>(staticMeshComponent.StaticMeshAssetHandle);
				if (staticMesh && !staticMesh->IsFlagSet(AssetFlag::Missing) && !staticMesh->IsFlagSet(AssetFlag::StillLoading))
				{
					Entity e = Entity(entity, this);
					glm::mat4 transform = GetWorldSpaceTransformMatrix(e);
	...
	void Scene::OnRenderSimulation(Ref<SceneRenderer> renderer, Timestep ts, const EditorCamera& editorCamera)
	{
		...
		// Render Static Meshes
		{
			...
			for (auto entity : group)
			{
				...
				{
					auto staticMesh = AssetManager::GetAsset<StaticMesh>(staticMeshComponent.StaticMeshAssetHandle);
					if (!staticMesh->IsFlagSet(AssetFlag::Missing) && !staticMesh->IsFlagSet(AssetFlag::StillLoading))
					{
						Entity e = Entity(entity, this);
						glm::mat4 transform = GetWorldSpaceTransformMatrix(e);
	...
	void Scene::OnAudioComponentConstruct(entt::registry& registry, entt::entity entity)
	{
		...
		BEY_CORE_ASSERT(m_EntityIDMap.find(entityID) != m_EntityIDMap.end());
	...
	std::vector<glm::mat4> Scene::GetModelSpaceBoneTransforms(const std::vector<UUID>& boneEntityIds, Ref<Mesh> mesh)
	{
		...
		if (mesh->HasSkeleton())
		{
			...
			//BEY_CORE_ASSERT(boneEntityIds.size() == skeleton.GetNumBones(), "Wrong number of boneEntityIds for mesh skeleton!");
			...
			if (anim.AnimationGraph && anim.BoneEntityIds.size() > 0)
			{
				...
				if (isRuntime)
				{
					...
					else if (m_ShouldSimulate && entity.HasComponent<RigidBodyComponent>())
					{
						...
						// NOTE: Removed this code path for now (with approval from 0x)
	...
	Entity Scene::GetMainCameraEntity()
	{
		...
		for (auto entity : view)
		{
			...
			if (comp.Primary)
			{
				BEY_CORE_ASSERT(comp.Camera.GetOrthographicSize() || comp.Camera.GetDegPerspectiveVerticalFOV(), "Camera is not fully initialized");
				return { entity, this };
			}
		}
		return {};
	...
	Entity Scene::CreateEntityWithID(UUID uuid, const std::string& name, bool shouldSort)
	{
		...
		BEY_CORE_ASSERT(m_EntityIDMap.find(uuid) == m_EntityIDMap.end());
	...
	void Scene::SubmitToDestroyEntity(Entity entity)
	{
		...
		if (!isValid)
		{
			BEY_CORE_WARN_TAG("Scene", "Trying to destroy invalid entity! entt={0}", (uint32_t)entity);
			return;
		}
	...
	void Scene::ResetTransformsToMesh(Entity entity, bool resetChildren)
	{
		if (entity.HasComponent<MeshComponent>())
		{
			...
			if (AssetManager::IsAssetHandleValid(mc.MeshAssetHandle))
			{
				Ref<Mesh> mesh = AssetManager::GetAsset<Mesh>(mc.MeshAssetHandle);
	...
	Entity Scene::DuplicateEntity(Entity entity)
	{
		...
		if (entity.HasComponent<PrefabComponent>())
		{
			...
			BEY_CORE_VERIFY(AssetManager::IsAssetHandleValid(prefabID));
		...
		if (entity.HasComponent<TagComponent>())
			newEntity = CreateEntity(entity.GetComponent<TagComponent>().Tag.c_str());
		else
			newEntity = CreateEntity();

		CopyComponentIfExists<TransformComponent>(newEntity.m_EntityHandle, m_Registry, entity);
		// NOTE: We can't use this method for copying the RelationshipComponent since we
		...
		BEY_CORE_ASSERT(foundAll, "At least one component was not duplicated - have you added a new component type and not dealt with it here?");
#endif
	...
	Entity Scene::CreatePrefabEntity(Entity entity, Entity parent, const glm::vec3* translation, const glm::vec3* rotation, const glm::vec3* scale)
	{
		...
		BEY_CORE_VERIFY(entity.HasComponent<PrefabComponent>());
	...
	void Scene::BuildMeshEntityHierarchy(Entity parent, Ref<Mesh> mesh, const MeshNode& node, bool generateColliders)
	{
		...
		Entity nodeEntity = CreateChildEntity(parent, node.Name.c_str());
		...
		else if (node.Submeshes.size() > 1)
		{
			...
			for (uint32_t i = 0; i < node.Submeshes.size(); i++)
			{
				...
				// NOTE: original implemenation use to use "mesh name" from assimp;
				//            we don't store that so use node name instead. Maybe we
				//            should store it?
				Entity childEntity = CreateChildEntity(nodeEntity, node.Name.c_str());
	...
	Entity Scene::InstantiateMesh(Ref<Mesh> mesh, bool generateColliders)
	{
		...
		Entity rootEntity = CreateEntity(assetData.FilePath.stem().string().c_str());
	...
	{
		//BEY_PROFILE_FUNC();
		BEY_CORE_VERIFY(m_EntityIDMap.find(id) != m_EntityIDMap.end(), "Invalid entity ID or entity doesn't exist in scene!");
	...
	Entity Scene::TryGetEntityWithTag(const eastl::string& tag)
	{
		...
		for (auto e : entities)
		{
			if (entities.get<TagComponent>(e).Tag.c_str() == tag)
	...
	void Scene::CopyTo(Ref<Scene>& target)
	{
		...
		// TODO: hack to prevent Box2D bodies from being created on copy via entt signals
		...
		CopyComponent<DDGIVolumeComponent>(target->m_Registry, m_Registry, enttMap);
	...
	void Scene::OnSceneTransition(AssetHandle scene)
	{
		...
		if (!m_OnSceneTransitionCallback)
		{
			BEY_CORE_WARN("Cannot transition scene - no callback set!");
		}
	}
	...
	{
		BEY_CORE_ASSERT(dst.HasComponent<AnimationComponent>(), "Destination entity does not have AnimationComponent!");
		{
	...
	static void InsertMeshMaterials(Ref<MeshSource> meshSource, std::unordered_set<AssetHandle>& assetList)
	{
		...
		{
			Ref<Texture2D> albedoTexture = material->GetBindlessTexture2D("u_MaterialUniforms.AlbedoTexIndex");
			if (albedoTexture && albedoTexture->Handle) // White texture has Handle == 0
				assetList.insert(albedoTexture->Handle);

			Ref<Texture2D> normalTexture = material->GetBindlessTexture2D("u_MaterialUniforms.NormalTexIndex");
			if (normalTexture && normalTexture->Handle)
				assetList.insert(normalTexture->Handle);

			Ref<Texture2D> metalnessTexture = material->GetBindlessTexture2D("u_MaterialUniforms.MetalnessTexIndex");
			if (metalnessTexture && metalnessTexture->Handle)
				assetList.insert(metalnessTexture->Handle);

			Ref<Texture2D> roughnessTexture = material->GetBindlessTexture2D("u_MaterialUniforms.RoughnessTexIndex");
			if (roughnessTexture && roughnessTexture->Handle)
	...
	std::unordered_set<AssetHandle> Scene::GetAssetList()
	{
		...
		std::unordered_set<AssetHandle> missingAssets; // TODO: debug only
		...
		// MeshComponent
		{
			...
			for (auto entity : view)
			{
				...
				if (mc.MeshAssetHandle)
				{
					if (AssetManager::IsMemoryAsset(mc.MeshAssetHandle))
						continue;

					if (AssetManager::IsAssetHandleValid(mc.MeshAssetHandle))
					{
						assetList.insert(mc.MeshAssetHandle);
					...
					{
						missingAssets.insert(mc.MeshAssetHandle);
					}
				}
		...
		// StaticMeshComponent
		{
			...
			for (auto entity : view)
			{
				...
				if (mc.StaticMeshAssetHandle)
				{
					if (AssetManager::IsMemoryAsset(mc.StaticMeshAssetHandle))
						continue;

					if (AssetManager::IsAssetHandleValid(mc.StaticMeshAssetHandle))
					{
						assetList.insert(mc.StaticMeshAssetHandle);

						// MeshSource is required too
						Ref<StaticMesh> mesh = AssetManager::GetAsset<StaticMesh>(mc.StaticMeshAssetHandle);
						Ref<MeshSource> meshSource = mesh->GetMeshSource();
						if (meshSource && AssetManager::IsAssetHandleValid(meshSource->Handle))
						{
							assetList.insert(meshSource->Handle);
					...
					{
						missingAssets.insert(mc.StaticMeshAssetHandle);
					}
				}
		...
		//BEY_CORE_WARN("{} assets ({} missing)", assetList.size(), missingAssets.size());
		return assetList;
	}
