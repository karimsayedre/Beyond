...
#include "Beyond/Editor/NodeGraphEditor/AnimationGraph/AnimationGraphAsset.h" // TODO (0x): separate editor from runtime
...
#include "Beyond/Script/ScriptUtils.h"
#include "Beyond/Utilities/SerializationMacros.h"
#include "Beyond/Utilities/YAMLSerializationHelpers.h"
...
namespace Beyond {
	...
	void SceneSerializer::SerializeEntity(YAML::Emitter& out, Entity entity)
	{
		...
		if (entity.HasComponent<MeshComponent>())
		{
			...
			out << YAML::Key << "AssetID" << YAML::Value << mc.MeshAssetHandle;
		...
		if (entity.HasComponent<StaticMeshComponent>())
		{
			...
			out << YAML::Key << "AssetID" << YAML::Value << smc.StaticMeshAssetHandle;
		...
		if (entity.HasComponent<DirectionalLightComponent>())
		{
			...
			out << YAML::Key << "SourceSize" << YAML::Value << directionalLightComponent.SourceSize;
		...
		if (entity.HasComponent<PointLightComponent>())
		{
			...
			out << YAML::Key << "SourceSize" << YAML::Value << lightComponent.SourceSize;
		...
		if (entity.HasComponent<DDGIVolumeComponent>())
		{
			out << YAML::Key << "DDGIVolumeComponent";
			out << YAML::BeginMap; // DDGIVolumeComponent

			const auto& lightComponent = entity.GetComponent<DDGIVolumeComponent>();
			out << YAML::Key << "Enable" << YAML::Value << lightComponent.Enable;
			out << YAML::Key << "Index" << YAML::Value << lightComponent.Index;
			out << YAML::Key << "RngSeed" << YAML::Value << lightComponent.RngSeed;
			out << YAML::Key << "InsertPerfMarkers" << YAML::Value << lightComponent.InsertPerfMarkers;
			out << YAML::Key << "ProbeRelocationEnabled" << YAML::Value << lightComponent.ProbeRelocationEnabled;
			out << YAML::Key << "ProbeClassificationEnabled" << YAML::Value << lightComponent.ProbeClassificationEnabled;
			out << YAML::Key << "ProbeVariabilityEnabled" << YAML::Value << lightComponent.ProbeVariabilityEnabled;
			out << YAML::Key << "InfiniteScrollingEnabled" << YAML::Value << lightComponent.InfiniteScrollingEnabled;
			out << YAML::Key << "ProbeSpacing" << YAML::Value << lightComponent.ProbeSpacing;
			out << YAML::Key << "ProbeCounts" << YAML::Value << lightComponent.ProbeCounts;
			out << YAML::Key << "ProbeNumRays" << YAML::Value << lightComponent.ProbeNumRays;
			out << YAML::Key << "ProbeNumIrradianceTexels" << YAML::Value << lightComponent.ProbeNumIrradianceTexels;
			out << YAML::Key << "ProbeNumDistanceTexels" << YAML::Value << lightComponent.ProbeNumDistanceTexels;
			out << YAML::Key << "ProbeHysteresis" << YAML::Value << lightComponent.ProbeHysteresis;
			out << YAML::Key << "ProbeMaxRayDistance" << YAML::Value << lightComponent.ProbeMaxRayDistance;
			out << YAML::Key << "ProbeNormalBias" << YAML::Value << lightComponent.ProbeNormalBias;
			out << YAML::Key << "ProbeViewBias" << YAML::Value << lightComponent.ProbeViewBias;
			out << YAML::Key << "ProbeIrradianceThreshold" << YAML::Value << lightComponent.ProbeIrradianceThreshold;
			out << YAML::Key << "ProbeBrightnessThreshold" << YAML::Value << lightComponent.ProbeBrightnessThreshold;
			out << YAML::Key << "ProbeVariabilityThreshold" << YAML::Value << lightComponent.ProbeVariabilityThreshold;
			out << YAML::Key << "ProbeMinFrontfaceDistance" << YAML::Value << lightComponent.ProbeMinFrontfaceDistance;
			out << YAML::Key << "ProbeDistanceExponent" << YAML::Value << lightComponent.ProbeDistanceExponent;
			out << YAML::Key << "ProbeIrradianceEncodingGamma" << YAML::Value << lightComponent.ProbeIrradianceEncodingGamma;
			out << YAML::Key << "ProbeRandomRayBackfaceThreshold" << YAML::Value << lightComponent.ProbeRandomRayBackfaceThreshold;
			out << YAML::Key << "ProbeFixedRayBackfaceThreshold" << YAML::Value << lightComponent.ProbeFixedRayBackfaceThreshold;

			out << YAML::EndMap; // DDGIVolumeComponent
		}
		...
		if (entity.HasComponent<AudioComponent>())
		{
			...
			BEY_SERIALIZE_PROPERTY(StartEvent,			 audioComponent.StartEvent,				out);
			BEY_SERIALIZE_PROPERTY(StartCommandID,		 audioComponent.StartCommandID,			out);

			BEY_SERIALIZE_PROPERTY(PlayOnAwake,			 audioComponent.bPlayOnAwake,			out);
			BEY_SERIALIZE_PROPERTY(StopIfEntityDestroyed, audioComponent.bStopWhenEntityDestroyed, out);
			BEY_SERIALIZE_PROPERTY(VolumeMultiplier,		 audioComponent.VolumeMultiplier,		out);
			BEY_SERIALIZE_PROPERTY(PitchMultiplier,		 audioComponent.PitchMultiplier,		out);
			BEY_SERIALIZE_PROPERTY(AutoDestroy,			 audioComponent.bAutoDestroy,			out);
		...
		if (entity.HasComponent<AudioListenerComponent>())
		{
			...
			BEY_SERIALIZE_PROPERTY(Active,				audioListenerComponent.Active,					out);
			BEY_SERIALIZE_PROPERTY(ConeInnerAngle,		audioListenerComponent.ConeInnerAngleInRadians, out);
			BEY_SERIALIZE_PROPERTY(ConeOuterAngle,		audioListenerComponent.ConeOuterAngleInRadians, out);
			BEY_SERIALIZE_PROPERTY(ConeOuterGain,		audioListenerComponent.ConeOuterGain,			out);
			out << YAML::EndMap; // AudioComponent
		}
	...
	void SceneSerializer::SerializeToYAML(YAML::Emitter& out)
	{
		...
		out << YAML::BeginMap;
		out << YAML::Key << "Scene";
		out << YAML::Value << m_Scene->GetName().c_str();
	...
	bool SceneSerializer::DeserializeFromYAML(const std::string& yamlString)
	{
		...
		const char* sceneName = data["Scene"].as<std::string>().c_str();
		BEY_CORE_INFO_TAG("AssetManager", "Deserializing scene '{0}'", sceneName);
	...
	void SceneSerializer::DeserializeEntities(YAML::Node& entitiesNode, Ref<Scene> scene)
	{
		for (auto entity : entitiesNode)
		{
			...
			//BEY_CORE_INFO("Deserialized Entity '{0}' with ID '{1}'", name, uuid);
			...
			if (transformComponent)
			{
				...
				// Some versions of Beyond serialized rotations as quaternions
				...
				//BEY_CORE_VERIFY(rotationNode.size() == 3, "Transform component rotation should be serialized as Euler angles. Found Quaternions!");
			...
			if (scriptComponent)
			{
				...
				if (scriptAssetHandle == 0 && !moduleName.empty())
					scriptAssetHandle = BEY_SCRIPT_CLASS_ID(moduleName);
					...
					if (sc.FieldIDs.size() > 0)
					{
						...
						if (storedFields)
						{
							for (auto field : storedFields)
							{
								...
								if (storage == nullptr)
								{
									BEY_CONSOLE_LOG_WARN("Serialized C# field {0} doesn't exist in script cache! This could be because the script field no longer exists or because it's been renamed.", name);
								}
								else
								{
				...
				{
					BEY_CORE_ERROR("Failed to deserialize ScriptComponent for entity '{0}'! Couldn't find a valid ModuleName or ClassHandle field/value!", deserializedEntity.Name());
				}
			}
			...
			if (meshComponent)
			{
				...
				if (AssetManager::IsAssetHandleValid(assetHandle))
				{
					...
					if (type == AssetType::Mesh)
						component.MeshAssetHandle = assetHandle;
#if DEPRECTATED
			...
			if (staticMeshComponent)
			{
				...
				if (AssetManager::IsAssetHandleValid(assetHandle))
					component.StaticMeshAssetHandle = assetHandle;
				...
				if (graphInputs && component.AnimationGraph)
				{
					for (auto input : graphInputs)
					{
						...
						catch (const YAML::Exception& e)
						{
							...
							BEY_CONSOLE_LOG_WARN("Input with id {0}: {1} while derializing animation component.", id, e.what());
						}
						catch (const std::out_of_range&)
						{
							...
							// Just ignore it.
							BEY_CONSOLE_LOG_WARN("Input with id {0} was not found in animation graph, while deserializing animation component.", id);
						}
					}
				}
			}
			...
			if (auto directionalLightComponent = entity["DirectionalLightComponent"]; directionalLightComponent)
			{
				...
				component.SourceSize = directionalLightComponent["SourceSize"].as<float>(0.5f);
			...
			if (auto pointLightComponent = entity["PointLightComponent"]; pointLightComponent)
			{
				...
				component.SourceSize = pointLightComponent["SourceSize"].as<float>(0.1f);
			...
			if (spotLightComponent)
			{
				...
				component.SourceSize = spotLightComponent["SourceSize"].as<float>(0.1f);
			}
			...
			if (skyLightComponent)
			{
				...
				{
					BEY_CORE_ERROR("Tried to load invalid environment map in Entity {0}", deserializedEntity.GetUUID());
				}
			}

			auto ddgiVolumeComponent = entity["DDGIVolumeComponent"];
			if (ddgiVolumeComponent)
			{
				auto& component = deserializedEntity.AddComponent<DDGIVolumeComponent>();

				component.Enable = ddgiVolumeComponent["Enable"].as<bool>();
				component.Index = ddgiVolumeComponent["Index"].as<uint32_t>();
				component.RngSeed = ddgiVolumeComponent["RngSeed"].as<uint32_t>();
				component.InsertPerfMarkers = ddgiVolumeComponent["InsertPerfMarkers"].as<bool>();
				component.ProbeRelocationEnabled = ddgiVolumeComponent["ProbeRelocationEnabled"].as<bool>();
				component.ProbeClassificationEnabled = ddgiVolumeComponent["ProbeClassificationEnabled"].as<bool>();
				component.ProbeVariabilityEnabled = ddgiVolumeComponent["ProbeVariabilityEnabled"].as<bool>();
				component.InfiniteScrollingEnabled = ddgiVolumeComponent["InfiniteScrollingEnabled"].as<bool>();
				component.ProbeSpacing = ddgiVolumeComponent["ProbeSpacing"].as<glm::vec3>();
				component.ProbeCounts = ddgiVolumeComponent["ProbeCounts"].as<glm::ivec3>();
				component.ProbeNumRays = ddgiVolumeComponent["ProbeNumRays"].as<int>();
				component.ProbeNumIrradianceTexels = ddgiVolumeComponent["ProbeNumIrradianceTexels"].as<int>();
				component.ProbeNumDistanceTexels = ddgiVolumeComponent["ProbeNumDistanceTexels"].as<int>();
				component.ProbeHysteresis = ddgiVolumeComponent["ProbeHysteresis"].as<float>();
				component.ProbeMaxRayDistance = ddgiVolumeComponent["ProbeMaxRayDistance"].as<float>();
				component.ProbeNormalBias = ddgiVolumeComponent["ProbeNormalBias"].as<float>();
				component.ProbeViewBias = ddgiVolumeComponent["ProbeViewBias"].as<float>();
				component.ProbeIrradianceThreshold = ddgiVolumeComponent["ProbeIrradianceThreshold"].as<float>();
				component.ProbeBrightnessThreshold = ddgiVolumeComponent["ProbeBrightnessThreshold"].as<float>();
				component.ProbeVariabilityThreshold = ddgiVolumeComponent["ProbeVariabilityThreshold"].as<float>();
				component.ProbeMinFrontfaceDistance = ddgiVolumeComponent["ProbeMinFrontfaceDistance"].as<float>();
				component.ProbeDistanceExponent = ddgiVolumeComponent["ProbeDistanceExponent"].as<float>();
				component.ProbeIrradianceEncodingGamma = ddgiVolumeComponent["ProbeIrradianceEncodingGamma"].as<float>();
				component.ProbeRandomRayBackfaceThreshold = ddgiVolumeComponent["ProbeRandomRayBackfaceThreshold"].as<float>();
				component.ProbeFixedRayBackfaceThreshold = ddgiVolumeComponent["ProbeFixedRayBackfaceThreshold"].as<float>();
			...
			if (meshColliderComponent)
			{
				...
				if (meshColliderComponent["ColliderHandle"])
				{
					...
					if (!colliderAsset)
					{
						...
						{
							component.ColliderAsset = AssetManager::CreateMemoryOnlyAsset<MeshColliderAsset>(deserializedEntity.GetComponent<StaticMeshComponent>().StaticMeshAssetHandle);
						}
				...
				else
				{
					...
					if (deserializedEntity.HasComponent<MeshComponent>())
						colliderMesh = deserializedEntity.GetComponent<MeshComponent>().MeshAssetHandle;
					else if (deserializedEntity.HasComponent<StaticMeshComponent>())
						colliderMesh = deserializedEntity.GetComponent<StaticMeshComponent>().StaticMeshAssetHandle;
					...
					if (AssetManager::IsAssetHandleValid(component.ColliderAsset))
						...
						BEY_CORE_WARN("MeshColliderComponent in use without valid mesh!");
			...
			// NOTE: This can probably be removed eventually
			...
			if (audioComponent)
			{
				...
				BEY_DESERIALIZE_PROPERTY(StartEvent, component.StartEvent, audioComponent, std::string(""));
				...
				BEY_DESERIALIZE_PROPERTY(PlayOnAwake, component.bPlayOnAwake, audioComponent, false);
				BEY_DESERIALIZE_PROPERTY(StopIfEntityDestroyed, component.bStopWhenEntityDestroyed, audioComponent, true);
				BEY_DESERIALIZE_PROPERTY(VolumeMultiplier, component.VolumeMultiplier, audioComponent, 1.0f);
				BEY_DESERIALIZE_PROPERTY(PitchMultiplier, component.PitchMultiplier, audioComponent, 1.0f);
				BEY_DESERIALIZE_PROPERTY(AutoDestroy, component.bAutoDestroy, audioComponent, false);
			}
	...
	bool SceneSerializer::Deserialize(const std::filesystem::path& filepath)
	{
		...
		{
			BEY_CONSOLE_LOG_ERROR("Failed to deserialize scene '{0}': {1}", filepath.string(), e.what());
			return false;
		}
		...
		// NOTE: Fix for "UntitledScene" name, hardcoded which isn't good
		if (m_Scene->GetName() == "UntitledScene")
			m_Scene->SetName(Utils::RemoveExtension(filepath.filename().string()).c_str());
