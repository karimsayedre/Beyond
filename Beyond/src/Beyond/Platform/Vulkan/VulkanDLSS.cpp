...
namespace Beyond
{
	...
	glm::uvec2 VulkanDLSS::GetOptimalSettings(uint32_t targetWidth, uint32_t targetHegiht, DLSSQualityValue quality)
	{
		...
		auto res = NGX_DLSS_GET_OPTIMAL_SETTINGS(m_Params, targetWidth, targetHegiht, dlssQuality, &m_RenderWidth, &m_RenderHeight, &m_MaxRenderWidth, &m_MaxRenderHeight, &m_MinRenderWidth, &m_MinRenderHeight, &Sharpness);
	...
	void VulkanDLSS::Evaluate(const Ref<RenderCommandBuffer>& commandBuffer,
							  uint32_t						  frame,
							  const glm::vec2&				  jitter,
							  Timestep						  timeStep,
							  Ref<Image2D>					  outputColor,
							  Ref<Image2D>					  inputColor,
							  Ref<Image2D>					  hitTImage,
							  Ref<Image2D>					  exposureImage,
							  Ref<Image2D>					  motionVectors,
							  Ref<Image2D>					  depthImage,
							  Ref<Image2D>					  albedoImage,
							  Ref<Image2D>					  metalRough,
							  Ref<Image2D>					  normals)
	{
		Renderer::Submit([commandBuffer = commandBuffer.As<VulkanRenderCommandBuffer>(),
						  instance		= Ref(this),
						  frame,
						  jitter,
						  outputColor,
						  ...
						  normals]() mutable
		{
			...
			evalParams.Feature = { inputColor.As<VulkanImage2D>()->GetNVXResourceInfo(), outputColor.As<VulkanImage2D>()->GetNVXResourceInfo(), 0.0f };
