...
namespace Beyond {
		...
		: m_Specification(specification)
	{
		...
		if (!m_Specification.ExistingFramebuffer)
		{
			for (auto& attachmentSpec : m_Specification.Attachments.Attachments)
			{
				...
				else if (Utils::IsDepthFormat(attachmentSpec.Format))
				{
					...
					spec.DebugName = fmt::eastl_format("{0}-DepthAttachment{1}", m_Specification.DebugName.empty() ? "Unnamed FB" : m_Specification.DebugName, attachmentIndex);
				...
				else
				{
					...
					spec.Width = (uint32_t)(m_Width * m_Specification.Scale);
					spec.Height = (uint32_t)(m_Height * m_Specification.Scale);
					spec.DebugName = fmt::eastl_format("{0}-ColorAttachment{1}", m_Specification.DebugName.empty() ? "Unnamed FB" : m_Specification.DebugName, attachmentIndex);
		...
		BEY_CORE_ASSERT(specification.Attachments.Attachments.size());
		...
		if (m_Framebuffer)
		{
			...
			if (!m_Specification.ExistingFramebuffer)
			{
				...
				for (Ref<VulkanImage2D> image : m_AttachmentImages)
				{
					...
					// Only destroy array images once and prevent clearing layer views on second framebuffer invalidation
	...
	void VulkanFramebuffer::Resize(uint32_t width, uint32_t height, bool forceRecreate)
	{
		...
		Renderer::Submit([instance, width, height]() mutable
		{
			...
			for (Ref<Image2D> colorImage : instance->m_Specification.ExistingImages | std::views::values)
			{
				if (instance->m_Width > colorImage->GetWidth() || instance->m_Height > colorImage->GetHeight())
					colorImage.As<VulkanImage2D>()->RT_Resize(instance->m_Width, instance->m_Height);
			}
	...
	{
		// BEY_CORE_TRACE("VulkanFramebuffer::RT_Invalidate ({})", m_Specification.DebugName);
		...
		for (const auto& attachmentSpec : m_Specification.Attachments.Attachments)
		{
			if (Utils::IsDepthFormat(attachmentSpec.Format))
			{
				...
				else if (m_Specification.ExistingImages.find(attachmentIndex) != m_Specification.ExistingImages.end())
				{
					...
					BEY_CORE_ASSERT(Utils::IsDepthFormat(existingImage->GetSpecification().Format), "Trying to attach non-depth image as depth attachment");
				...
				attachmentDescription.initialLayout = attachmentDescription.loadOp == VK_ATTACHMENT_LOAD_OP_CLEAR ? VK_IMAGE_LAYOUT_UNDEFINED : VK_IMAGE_LAYOUT_DEPTH_READ_ONLY_OPTIMAL;
				if (/*attachmentSpec.Format == ImageFormat::DEPTH24STENCIL8 || true*/false) // Separate layouts requires a "separate layouts" flag to be enabled
				{
			...
			{
				//BEY_CORE_ASSERT(!m_Specification.ExistingImage, "Not supported for color attachments");
				...
				else if (m_Specification.ExistingImages.contains(attachmentIndex))
				{
					...
					BEY_CORE_ASSERT(!Utils::IsDepthFormat(existingImage->GetSpecification().Format), "Trying to attach depth image as color attachment");
		...
		if (m_DepthAttachmentImage)
		{
			...
			{
				BEY_CORE_ASSERT(m_Specification.ExistingImageLayers.size() == 1, "Depth attachments do not support deinterleaving");
			...
			BEY_CORE_ASSERT(attachments.back());
		}
