...
namespace Beyond {
	...
	namespace Utils {
		static VkPrimitiveTopology GetVulkanTopology(PrimitiveTopology topology)
		{
			...
			BEY_CORE_ASSERT(false, "Unknown toplogy");
		...
		static VkCompareOp GetVulkanCompareOperator(const DepthCompareOperator compareOp)
		{
			...
			BEY_CORE_ASSERT(false, "Unknown Operator");
	...
	VulkanRasterPipeline::VulkanRasterPipeline(const PipelineSpecification& spec)
		: m_Specification(spec)
	{
		BEY_CORE_ASSERT(spec.Shader);
		BEY_CORE_ASSERT(spec.TargetFramebuffer);
		VulkanRasterPipeline::Invalidate();
		Renderer::RegisterShaderDependency(spec.Shader, this);
	}

	void VulkanRasterPipeline::Release()
	{
	...
	void VulkanRasterPipeline::Invalidate()
	{
		Ref<VulkanRasterPipeline> instance = this;
	...
	void VulkanRasterPipeline::RT_Invalidate()
	{
		Release();

		// BEY_CORE_WARN("[VulkanRasterPipeline] Creating pipeline {0}", m_Specification.DebugName);

		VkDevice device = VulkanContext::GetCurrentDevice()->GetVulkanDevice();
		BEY_CORE_ASSERT(m_Specification.Shader);
		Ref<VulkanShader> vulkanShader = Ref<VulkanShader>(m_Specification.Shader);
		Ref<VulkanFramebuffer> framebuffer = m_Specification.TargetFramebuffer.As<VulkanFramebuffer>();
		...
		VKUtils::SetDebugUtilsObjectName(device, VK_OBJECT_TYPE_PIPELINE_LAYOUT, fmt::eastl_format("Raster Pipeline: {} with shader: {}",
			m_Specification.DebugName, GetShader()->GetName()), m_PipelineLayout);
		...
		depthStencilState.depthCompareOp = Utils::GetVulkanCompareOperator(m_Specification.DepthOperator);
		...
		vertexInputBinding.binding = vertexBinding++;
		...
		if (instanceLayout.GetElementCount())
		{
			...
			instanceInputBinding.binding = vertexBinding++;
		...
		if (boneInfluenceLayout.GetElementCount())
		{
			...
			boneInfluenceInputBinding.binding = vertexBinding++;
		...
		VK_CHECK_RESULT(vkCreateGraphicsPipelines(device, nullptr, 1, &pipelineCreateInfo, nullptr, &m_VulkanPipeline));
		VKUtils::SetDebugUtilsObjectName(device, VK_OBJECT_TYPE_PIPELINE, m_Specification.DebugName, m_VulkanPipeline);
	}

	bool VulkanRasterPipeline::IsDynamicLineWidth() const
	{
