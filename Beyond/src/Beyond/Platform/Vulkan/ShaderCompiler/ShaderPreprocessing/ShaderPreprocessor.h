...
namespace std {
	template<>
	struct hash<Beyond::IncludeData>
	{
		size_t operator()(const Beyond::IncludeData& data) const noexcept
		{
...
namespace Beyond {
	...
	VkShaderStageFlagBits ShaderPreprocessor::PreprocessHeader(std::string& contents, bool& isGuarded, std::unordered_set<std::string>& specialMacros, const std::unordered_set<IncludeData>& includeData, const std::filesystem::path& fullPath)
	{
		...
		while (startOfShaderStage != eastl::string::npos)
		{
			...
			if (tokens[index] == "#")
			{
				...
				if (tokens[index] == "pragma")
				{
					...
					{
						BEY_CORE_VERIFY(tokens[++index] == ":", "Stage pragma is invalid");

						// Skipped ':'
						const std::string_view stage(tokens[++index]);
						BEY_CORE_VERIFY(stage == "vert" || stage == "frag" || stage == "comp", "Invalid shader type specified");
						VkShaderStageFlagBits foundStage = ShaderUtils::ShaderTypeFromString(stage);
						...
						if (isGuarded && alreadyIncluded)
							...
							BEY_CORE_WARN("\"{}\" Header does not contain a header guard (#pragma once).", fullPath);
				...
				{
					++index;
					if (tokens[index].rfind("__BEY_", 0) == 0) // Beyond special macros start with "__BEY_"
					{
				...
				else if (tokens[index] == "if" || tokens[index] == "define") // Consider "#if defined()" too?
				{
					...
					{
						if (tokens[i].rfind("__BEY_", 0) == 0) // Beyond special macros start with "__BEY_"
						{
	...
	std::map<VkShaderStageFlagBits, std::string> ShaderPreprocessor::PreprocessShader(const std::string& source, std::unordered_set<std::string>& specialMacros)
	{
		...
		BEY_CORE_ASSERT(newSource.size(), "Shader is empty!");
		...
		if constexpr (Lang == ShaderUtils::SourceLang::GLSL)
		{
			...
			BEY_CORE_VERIFY(tokens.size() >= 3 && tokens[1] == "version", "Invalid #version encountered or #version is NOT encounted first.");
			pos = newSource.find('#', pos + 1);
		}
		else if constexpr (Lang == ShaderUtils::SourceLang::HLSL)
		{
			const size_t endOfLine = newSource.find_first_of("\r\n", pos) + 1;
			const std::vector<std::string> tokens = Utils::SplitStringAndKeepDelims(newSource.substr(pos, endOfLine - pos));
			BEY_CORE_VERIFY(tokens.size() >= 5 && tokens[1] == "pragma" && tokens[2] == "stage" && tokens[3] == ":", "Stage pragma MUST be encountered first.");
			pos = newSource.find('#', pos + 1);

			stagePositions.emplace_back(ShaderUtils::ShaderTypeFromString(tokens[4]), 0);
		...
		while (pos != eastl::string::npos)
		{
			...
			if (tokens[index] == "pragma") // Parse stage. example: #pragma stage : vert
			{
				...
				if (tokens[index] == "stage")
				{
					...
					BEY_CORE_VERIFY(shaderStage & VK_SHADER_STAGE_ALL, "Invalid shader type specified");
			...
			else if constexpr (Lang == ShaderUtils::SourceLang::GLSL)
			{
				if (tokens[index] == "version")
				{
					++index;
					startOfStage = pos;
					//[[maybe_unused]] eastl::string stageStr = newSource.substr(0, pos);
				}
			}
		...
		BEY_CORE_VERIFY(stagePositions.size(), "Could not pre-process shader! There are no known stages defined in file.");
		...
		if (stagePositions.size() > 1)
		{
			...
			size_t lineCount = std::ranges::count(firstStageStr, '\n') + 1;
			...
			for (size_t i = 1; i < stagePositions.size() - 1; ++i)
			{
				...
				lineCount += std::ranges::count(stageStr, '\n') - 1;
