...
namespace Beyond {
	...
	shaderc_include_result* GlslIncluder::GetInclude(const char* requestedPath, const shaderc_include_type type, const char* requestingPath, const size_t includeDepth)
	{
		...
		if (source.empty())
		{
			...
			if (source.empty())
				BEY_CORE_ERROR("Failed to load included file named: {} requested in: {} with full path: {}.", requestedPath, 
					requestingPath, requestedFullPath.empty() ? "Unknown" : requestedFullPath);
	...
	{
		delete static_cast<std::array<eastl::string, 2>*>(data->user_data);
		delete data;
	}
