...
namespace Beyond
{
	...
	class VulkanShaderCompiler : public RefCounted
	{
	  public:
		...
		const std::map<VkShaderStageFlagBits, std::vector<uint32_t>>& GetSPIRVData() const
		{
			return m_SPIRVData;
		}
		const std::map<VkShaderStageFlagBits, std::vector<uint32_t>>& GetDebugSpirVData() const
		{
			return m_SPIRVDebugData;
		}
		const std::unordered_set<std::string>& GetAcknowledgedMacros() const
		{
			return m_AcknowledgedMacros;
		}

		static void ClearUniformBuffers();

		static Ref<VulkanShader> Compile(const RootSignature rootSignature, const std::filesystem::path& shaderSourcePath, bool forceCompile, bool disableOptimization, bool external, const std::wstring& entryPoint, const std::wstring& targetProfile, const std::vector<std::pair<std::wstring, std::wstring>>& defines);
	  ...
	  private:
		...
		void SerializeReflectionData(StreamWriter* serializer) const;
