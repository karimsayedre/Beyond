...
namespace Beyond {
	...
	public:
		...
		{
			BEY_CORE_ASSERT(index < m_Framebuffers.size());
			return m_Framebuffers[index];
		}

		VkCommandBuffer GetDrawCommandBuffer(uint32_t index)
		{
			BEY_CORE_ASSERT(index < m_CommandBuffers.size());
			return m_CommandBuffers[index].CommandBuffer;
		}

		VkSemaphore GetRenderCompleteSemaphore() { return m_Semaphores.RenderComplete[m_CurrentBufferIndex]; }
