    ...
    const ImU32 arrow_col = selected ? Colors::Theme::backgroundDark : Colors::Theme::muted;
		...
		if (display_frame)
		{
			...
			if (icon)
			{
				...
				Beyond::UI::ShiftCursorY(-frame_height + pad);
				Beyond::UI::ShiftCursorX(arrowWidth);
				Beyond::UI::Image(icon, { frame_height - pad * 2.0f, frame_height - pad * 2.0f }, ImVec2(0, 0), ImVec2(1, 1), iconTint /*selected ? colourDark : tintFloat*/);
		...
		else
		{
			...
			if (hovered || selected)
			{
				//if (held && hovered) BEY_CORE_WARN("held && hovered");
				//if(hovered && !selected && !held && !pressed && !toggled) BEY_CORE_WARN("hovered && !selected && !held");
				//else if(!selected) BEY_CORE_WARN("ImGuiCol_Header");
