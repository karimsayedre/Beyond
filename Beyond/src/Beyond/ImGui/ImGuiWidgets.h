...
#include "Beyond/Editor/EditorResources.h"
...
namespace Beyond::UI {
	...
	template<typename Item, typename Query>
		requires std::is_same_v<Item, std::string> || std::is_same_v<Item, eastl::string> || std::is_same_v<const char*, Item> ||
	std::is_same_v<Query, std::string_view> || std::is_same_v<Query, eastl::string_view>
		static bool IsMatchingSearch(const Item& item, Query searchQuery, bool caseSensitive = false, bool stripWhiteSpaces = false, bool stripUnderscores = false)
	{
		if (searchQuery.empty())
			return true;

		std::string_view itemView = item;

		if (itemView.empty())
			return false;

		std::string itemSanitized = stripUnderscores ? choc::text::replace(itemView, "_", " ") : std::string(itemView);

		if (stripWhiteSpaces)
			itemSanitized = choc::text::replace(itemSanitized, " ", "");

		std::string searchString = stripWhiteSpaces ? choc::text::replace(std::string_view(searchQuery.data(), searchQuery.size()), " ", "") : std::string(searchQuery.data(), searchQuery.size());
	...
	public:
		...
		static bool SearchWidget(StringType& searchString, const char* hint = "Search...", bool* grabFocus = nullptr)
		{
			...
			if constexpr (std::is_same_v<StringType, std::string> || std::is_same_v<StringType, eastl::string>)
			{
		...
		// See also ImGuiUtilities.h UI::DragVec3() which uses Dear ImGui's internal DragVec3
		// By contrast EditVec3() custom draws the X,Y, and Z components with red, green and blue label buttons.
		static bool EditIVec3(std::string_view label, ImVec2 size, int resetValue, bool& manuallyEdited, glm::ivec3& value, VectorAxis renderMultiSelectAxes = VectorAxis::None, float speed = 1.0f, glm::ivec3 v_min = glm::zero<glm::ivec3>(), glm::ivec3 v_max = glm::zero<glm::ivec3>(), const char* format = "%.2f", ImGuiSliderFlags flags = 0)
		{
			ImGui::BeginVertical((std::string(label) + "fr").data());
			bool changed = false;
			{
				...
				auto drawControl = [&](const std::string& label, int& value, const ImVec4& colourN, const ImVec4& colourH, const ImVec4& colourP, bool renderMultiSelect, float speed, int v_min, int v_max, const char* format, ImGuiSliderFlags flags)
				{
					{
						UI::ScopedStyle buttonFrame(ImGuiStyleVar_FramePadding, ImVec2(framePadding, 0.0f));
						UI::ScopedStyle buttonRounding(ImGuiStyleVar_FrameRounding, 1.0f);
					...
					ImGui::PushItemFlag(ImGuiItemFlags_MixedValue, renderMultiSelect);
					bool wasTempInputActive = ImGui::TempInputIsActive(ImGui::GetID(("##" + label).c_str()));
					changed |= UI::DragInt32(("##" + label).c_str(), &value, speed, v_min, v_max, format, flags);
