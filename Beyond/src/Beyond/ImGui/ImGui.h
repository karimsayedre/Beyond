...
#include "Beyond/Editor/AssetEditorPanelInterface.h"
#include "Beyond/ImGui/Colors.h"
#include "Beyond/ImGui/ImGuiUtilities.h"
#include "Beyond/ImGui/ImGuiWidgets.h"
#include "Beyond/ImGui/ImGuiFonts.h"
#include "Beyond/Utilities/StringUtils.h"
#include "Beyond/Script/ScriptCache.h"

#include "choc/text/choc_StringUtilities.h"

namespace Beyond::UI {

#define BEY_MESSAGE_BOX_OK_BUTTON BIT(0)
#define BEY_MESSAGE_BOX_CANCEL_BUTTON BIT(1)
#define BEY_MESSAGE_BOX_USER_FUNC BIT(2)
#define BEY_MESSAGE_BOX_AUTO_SIZE BIT(3)
	...
	static void ShowSimpleMessageBox(const char* title, fmt::format_string<TArgs...> content, TArgs&&... contentArgs)
	{
		auto& messageBoxData = s_MessageBoxes[title];
		messageBoxData.Title = fmt::format("{}##MessageBox{}", title, s_MessageBoxes.size() + 1).c_str();
		if constexpr (sizeof...(contentArgs) > 0)
			messageBoxData.Body = fmt::format(content, std::forward<TArgs>(contentArgs)...).c_str();
		else
			messageBoxData.Body = eastl::string{ content.get().data(), content.get().size() };
	...
	static void ShowMessageBox(const char* title, const std::function<void()>& renderFunction, uint32_t width = 600, uint32_t height = 0, uint32_t minWidth = 0, uint32_t minHeight = 0, uint32_t maxWidth = -1, uint32_t maxHeight = -1, uint32_t flags = BEY_MESSAGE_BOX_AUTO_SIZE)
	{
		auto& messageBoxData = s_MessageBoxes[title];
		messageBoxData.Title = fmt::eastl_format("{0}##MessageBox{1}", title, s_MessageBoxes.size() + 1);
		messageBoxData.UserRenderFunction = renderFunction;
		messageBoxData.Flags = BEY_MESSAGE_BOX_USER_FUNC | flags;
	...
	static void RenderMessageBoxes()
	{
		for (auto& [key, messageBoxData] : s_MessageBoxes)
		{
			...
			if (messageBoxData.Flags & BEY_MESSAGE_BOX_AUTO_SIZE)
			{
			...
			{
				if (messageBoxData.Flags & BEY_MESSAGE_BOX_USER_FUNC)
				{
					BEY_CORE_VERIFY(messageBoxData.UserRenderFunction, "No render function provided for message box!");
				...
				else
				{
					...
					if (messageBoxData.Flags & BEY_MESSAGE_BOX_OK_BUTTON)
					{
						if (ImGui::Button("Ok"))
							ImGui::CloseCurrentPopup();

						if (messageBoxData.Flags & BEY_MESSAGE_BOX_CANCEL_BUTTON)
							ImGui::SameLine();
					}

					if (messageBoxData.Flags & BEY_MESSAGE_BOX_CANCEL_BUTTON && ImGui::Button("Cancel"))
					{
						ImGui::CloseCurrentPopup();
					}
				}
	...
	static bool PropertyGridHeader(const eastl::string& name, bool openByDefault = true)
	{
		...
		open = ImGui::TreeNodeEx("##dummy_id", treeNodeFlags, Utils::ToUpper(name.c_str()).c_str());
		//UI::PopID();
		ImGui::PopID();
	...
	static bool TreeNodeWithIcon(const eastl::string& label, const Ref<Texture2D>& icon, const ImVec2& size, bool openByDefault = true)
	{
		ImGuiTreeNodeFlags treeNodeFlags = ImGuiTreeNodeFlags_Framed
			| ImGuiTreeNodeFlags_SpanAvailWidth
			| ImGuiTreeNodeFlags_AllowItemOverlap
		...
		ImGui::SameLine();
		UI::ShiftCursorY(-(size.y / 2.0f) + 1.0f);
		ImGui::TextUnformatted(Utils::ToUpper({ label.c_str(), label.size() }).c_str());
	...
	static bool PropertyButton(const char* label, const char* buttonText, const char* helpText = "")
	{
		...
		modified = ImGui::Button(fmt::format("{0}##{1}", buttonText, label).c_str());
	...
	static bool ShowWorkgroupSizeChanger(int& workGroupSize, int minSize = 1, int maxSize = 1024)
	{
		// Ensure workGroupSize is a valid power of two within the range
		workGroupSize = glm::clamp(workGroupSize, minSize, maxSize);
		workGroupSize = 1 << (int)glm::log2(workGroupSize);

		int power = (int)glm::log2(workGroupSize); // Convert size to power of two for UI
		int minPower = glm::max(1, (int)glm::log2(minSize));
		int maxPower = (int)glm::log2(maxSize);

		// ImGui slider for the power of two
		if (ImGui::SliderInt("Workgroup Size (Power of 2)", &power, minPower, maxPower))
		{
			workGroupSize = 1 << power; // Update workGroupSize to the new power of two
			return true;
		}
		...
		if (ImGui::IsItemHovered())
		{
			...
			ImGui::TextUnformatted(fmt::format("Work Group Size = {}", workGroupSize).c_str());
	...
	static bool PropertyRadio(const char* label, int& chosen, const std::map<int, const eastl::string_view>& options, const char* helpText = "", const std::map<int, const eastl::string_view>& optionHelpTexts = {})
	{
		bool modified = false;
		...
		{
			eastl::string radioLabel = fmt::format("{}##{}", option, label).c_str();
	...
	static bool Property(const char* label, glm::vec3& value, float delta = 0.1f, float min = 0.0f, float max = 0.0f, const char* helpText = "")
	{
		...
		bool modified = UI::Widgets::EditVec3(fmt::format("##{0}", label).c_str(), size, 0.0f, manuallyEdited, value, UI::VectorAxis::None, delta, glm::vec3{ min }, glm::vec3{ max });
	...
	static bool Property(const char* label, glm::ivec3& value, float delta = 0.1f, int min = 0.0f, int max = 0.0f, const char* helpText = "")
	{
		ShiftCursor(10.0f, 9.0f);
		ImGui::Text(label);
		...
		ImVec2 size(ImGui::GetContentRegionAvail().x - 8.0f, ImGui::GetFrameHeightWithSpacing());
		bool manuallyEdited = false;
		bool modified = UI::Widgets::EditIVec3(fmt::format("##{0}", label).c_str(), size, 0, manuallyEdited, value, UI::VectorAxis::None, delta, glm::ivec3{ min }, glm::ivec3{ max });
	...
	static bool PropertyPowerOfTwo(const char* label, int& value, int min = 0, int max = 0, const char* helpText = "")
	{
		ShiftCursor(10.0f, 9.0f);
		ImGui::Text(label);
		...
		bool modified = UI::ShowWorkgroupSizeChanger(value, min, max);
	...
	static bool PropertyDropdownNoLabel(const char* strID, const char** options, int32_t optionCount, int32_t* selected)
	{
		...
		const eastl::string id = "##" + eastl::string(strID);
		if (UI::BeginCombo(id.c_str(), current))
		{
			for (int i = 0; i < optionCount; i++)
			{
	...
	static bool PropertyDropdownNoLabel(const char* strID, const std::vector<eastl::string>& options, int32_t* selected, bool advanceColumn = true, bool fullWidth = true)
	{
	...
	static bool PropertyDropdown(const char* label, const std::vector<eastl::string>& options, int32_t optionCount, int32_t* selected, const char* helpText = "")
	{
		const char* current = options[*selected].c_str();
	...
	static bool PropertyAssetReference(const char* label, AssetHandle& outHandle, const char* helpText = "", PropertyAssetReferenceError* outError = nullptr, const PropertyAssetReferenceSettings& settings = {})
	{
		...
		ImVec2 originalButtonTextAlign = ImGui::GetStyle().ButtonTextAlign;
		{
			...
			if (AssetManager::IsAssetHandleValid(outHandle))
			{
				...
				if (object && !object->IsFlagSet(AssetFlag::Missing))
				{
					if (settings.ShowFullFilePath)
						buttonText = Project::GetEditorAssetManager()->GetMetadata(outHandle).FilePath.string().c_str();
					else
						buttonText = Project::GetEditorAssetManager()->GetMetadata(outHandle).FilePath.stem().string().c_str();
				}
				else
				{
					buttonText = "Missing";
			...
			eastl::string assetSearchPopupID = GenerateLabelID("ARSP");
			{
	...
	static bool PropertyEntityReference(const char* label, UUID& entityID, Ref<Scene> currentScene, const char* helpText = "")
	{
		...
		ImVec2 originalButtonTextAlign = ImGui::GetStyle().ButtonTextAlign;
		{
			...
			if (entity)
				buttonText = entity.GetComponent<TagComponent>().Tag.c_str();
	...
	static bool PropertyAssetReferenceTarget(const char* label, const char* assetName, AssetHandle& outHandle, Fn&& targetFunc, const char* helpText = "", const PropertyAssetReferenceSettings& settings = {})
	{
		...
		// - don't store return from GenerateLabelId in a const char* here. Because its pointing to an internal
		//   buffer which may get overwritten by the time you want to use it later on.
		eastl::string assetSearchPopupID = GenerateLabelID("ARTSP");
		{
			UI::ScopedColour buttonLabelColor(ImGuiCol_Text, valid ? settings.ButtonLabelColor : settings.ButtonLabelColorError);
			ImGui::Button(GenerateLabelID(buttonText), { width, itemHeight });
	...
	static bool DrawFieldValue(Ref<Scene> sceneContext, const std::string& fieldName, Ref<FieldStorage> storage)
	{
		...
		/*if (field->HasAttribute("Beyond.ClampValueAttribute"))
		{
			MonoObject* attr = field->GetAttribute("Beyond.ClampValueAttribute");
			BEY_TRY_GET_FIELD_VALUE(min, "Beyond.ClampValueAttribute", "Min", attr);
			BEY_TRY_GET_FIELD_VALUE(max, "Beyond.ClampValueAttribute", "Max", attr);
		}*/

		eastl::string id = fmt::eastl_format("{0}-{1}", fieldName, field->ID);
	...
	static bool DrawFieldArray(Ref<Scene> sceneContext, const std::string& fieldName, Ref<ArrayFieldStorage> arrayStorage)
	{
		...
		std::string arrayID = fmt::format("{0}FieldArray", fieldName).c_str();
