...
#define NODE(name) { Identifier(Beyond::Utils::CreateUserFriendlyTypeName(#name)), [](UUID nodeID) { return new name(#name, nodeID); } }
...
namespace Beyond::AnimationGraph {
	...
	NodeProcessor* Factory::Create(Identifier nodeTypeID, UUID nodeID)
	{
		if (!NodeProcessors.count(nodeTypeID))
		{
			if (!nodeTypeID.GetDBGName().empty())
				BEY_CORE_ERROR("AnimationGraph::Factory::Create - Node with type ID {} is not in the registry", nodeTypeID.GetDBGName());
			else
				BEY_CORE_ERROR("AnimationGraph::Factory::Create - Node with type ID {} is not in the registry", (uint32_t)nodeTypeID);
	...
	bool ValidateGraph(const Graph& graph)
	{
		...
		//! 2. Graph endpoints (?)
		{
			...
			// can never fail, since we always set a default value (plus for trigger inputs, no value is valid)
			// validateElements(graph.EndpointInputStreams, validateEndpoints);
	...
	bool InitStateMachine(Ref<AnimationGraph> root, StateMachine* stateMachine, const Prototype& prototype)
	{
		...
		for (auto& subnode : prototype.Nodes)
		{
			...
			{
				BEY_CORE_ASSERT(subnode.Subroutine, "State Machine node has no subroutine! (check graph compilation)");
			...
			{
				BEY_CORE_ASSERT(subnode.Subroutine, "State node has no subroutine! (check graph compilation)");
		...
		for (auto& subnode : prototype.Nodes)
		{
			if ((subnode.TypeID == IDs::Transition))
			{
				...
				BEY_CORE_ASSERT(sourceStateID != 0, "Transition node should have a source state");
				BEY_CORE_ASSERT(destStateID != 0, "Transition node should have a destination state");
				...
				BEY_CORE_ASSERT(sourceNode, "Transition node source state does not exist");
				BEY_CORE_ASSERT(destNode, "Transition node destination state does not exist");
