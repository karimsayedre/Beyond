...
namespace Beyond::AnimationGraph {
	...
	{
		BEY_CORE_ASSERT(pose0->NumBones == pose1->NumBones, "Poses have different number of bones");
	...
	{
		BEY_PROFILE_FUNC();
		BEY_CORE_ASSERT((u > 0) && (v > 0), "Both weights for Blend2 must be strictly positive"); // otherwise you aren't blending two things
		m_Vertices[i0]->EnsureAnimation();
		...
		BlendBoneTransforms(pose0, pose1, v, m_Skeleton, 0, result);
		BlendRootMotion(pose0, pose1, v, result);
		result->NumBones = pose0->NumBones;
		previousI0 = i0;
		previousI1 = i1;
		previousI2 = -1;
	...
	{
		BEY_PROFILE_FUNC();
		BEY_CORE_ASSERT((u > 0) && (v > 0) && (w > 0), "All weights for Blend3 must be strictly positive"); // otherwise you aren't blending three things
		m_Vertices[i0]->EnsureAnimation();
		m_Vertices[i1]->EnsureAnimation();
		...
		BEY_CORE_ASSERT((poseA->NumBones == poseB->NumBones) && (poseA->NumBones == poseC->NumBones), "Poses have different number of bones");
	...
	float BlendSpace::Process(float timestep)
	{
		...
		BEY_CORE_ASSERT(false); // should not be possible to get here. at least one of u, v, w must be > 0
		return 0.0f;
	}
	...
	{
		BEY_CORE_ASSERT(m_AnimationGraph, "No animation graph set");
		EndpointUtilities::InitializeInputs(this);
		m_Skeleton = skeleton;
	...
	float ConditionalBlend::Process(float timestep)
	{
		...
		else
		{
			if (*in_Additive)
			{
				AdditiveBlendBoneTransforms(in_BasePose, m_AnimationGraph->GetPose(), w, m_Skeleton, *in_BlendRootBone, result);

				if (*in_BlendRootBone == 0)
				{
					AdditiveBlendRootMotion(in_BasePose, m_AnimationGraph->GetPose(), w, result);
				}
				else
				{
			...
			{
				BlendBoneTransforms(in_BasePose, m_AnimationGraph->GetPose(), w, m_Skeleton, *in_BlendRootBone, result);

				if (*in_BlendRootBone == 0)
				{
					BlendRootMotion(in_BasePose, m_AnimationGraph->GetPose(), w, result);
				}
				else
				{
					result->RootMotion = in_BasePose->RootMotion;
	...
	float RangedBlend::Process(float timestep)
	{
		...
		Pose* result = static_cast<Pose*>(out_Pose.getRawData());
		BEY_CORE_ASSERT(m_AnimationA->GetNumTracks() == m_AnimationB->GetNumTracks());
		BlendBoneTransforms(&m_PoseA, &m_PoseB, w, m_Skeleton, 0, result);
		BlendRootMotion(&m_PoseA, &m_PoseB, w, result);
		result->AnimationDuration = weightedDuration;
		result->AnimationTimePos = glm::mix(m_PoseA.AnimationTimePos, m_PoseB.AnimationTimePos, w);
