...
namespace Beyond::AnimationGraph {
	...
	{
		BEY_CORE_ASSERT(m_ConditionGraph, "No condition graph set");
	...
	{
		BEY_PROFILE_FUNC();
		BEY_CORE_ASSERT(m_SourceState, "No source state set");
		BEY_CORE_ASSERT(m_DestinationState, "No target state set");
	...
	void TransitionNode::Blend(const choc::value::ValueView source, const choc::value::ValueView destination, float w)
	{
		...
		BEY_CORE_ASSERT(poseA->NumBones == poseB->NumBones, "Poses have different number of bones");
	...
	float StateMachine::ProcessGraph(float timestep)
	{
		while (timestep > 0.0f)
		{
			BEY_CORE_ASSERT(m_CurrentState, "No current state set");
