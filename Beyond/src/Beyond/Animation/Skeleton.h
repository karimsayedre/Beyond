...
namespace Beyond {
	...
	public:
		...
		uint32_t GetParentBoneIndex(const uint32_t boneIndex) const { BEY_CORE_ASSERT(boneIndex < m_ParentBoneIndices.size(), "bone index out of range in Skeleton::GetParentIndex()!"); return m_ParentBoneIndices[boneIndex]; }
		...
		const std::string& GetBoneName(const uint32_t boneIndex) const { BEY_CORE_ASSERT(boneIndex < m_BoneNames.size(), "bone index out of range in Skeleton::GetBoneName()!"); return m_BoneNames[boneIndex]; }
		const eastl::string_view GetBoneNameEA(const uint32_t boneIndex) const { BEY_CORE_ASSERT(boneIndex < m_BoneNames.size(), "bone index out of range in Skeleton::GetBoneName()!"); return eastl::string_view(m_BoneNames[boneIndex].c_str(), m_BoneNames[boneIndex].size()); }
