...
namespace Beyond {
	...
	{
		BEY_CORE_ASSERT(parentBoneIndices.size() == boneNames.size());
		BEY_CORE_ASSERT(boneTranslations.size()  == boneNames.size());
		BEY_CORE_ASSERT(boneRotations.size()     == boneNames.size());
		BEY_CORE_ASSERT(boneScales.size()        == boneNames.size());
	...
	Beyond::Ref<Beyond::MeshSource> SkeletonAsset::GetMeshSource() const
	{
		return m_MeshSource;
	}

	const Beyond::Skeleton& SkeletonAsset::GetSkeleton() const
	{
		BEY_CORE_ASSERT(m_MeshSource && m_MeshSource->HasSkeleton());
