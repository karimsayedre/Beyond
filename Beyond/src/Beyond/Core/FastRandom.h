...
namespace Beyond
{
	...
	*/
	struct FastRandom
	{
		...
		inline glm::vec4 GetVec4InRange(float low, float high) noexcept
		{
			return { GetFloat32InRange(low, high), GetFloat32InRange(low, high), GetFloat32InRange(low, high), GetFloat32InRange(low, high) };
		}
