...
#include "Beyond/Embed/BeyondIcon.embed"

	static void GLFWErrorCallback(int error, const char* description)
	{
		BEY_CORE_ERROR_TAG("GLFW", "GLFW Error ({0}): {1}", error, description);
	}
	...
	void Window::Init()
	{
		...
		BEY_CORE_INFO_TAG("GLFW", "Creating window {0} ({1}, {2})", m_Specification.Title, m_Specification.Width, m_Specification.Height);

		if (!s_GLFWInitialized)
		{
			// TODO: glfwTerminate on system shutdown
			int success = glfwInit();
			BEY_CORE_ASSERT(success, "Could not intialize GLFW!");
		...
		// Set icon
		{
			...
			{
				const std::string iconPathStr = m_Specification.IconPath.string();
			...
			{
				// Use embedded Beyond icon
				icon.pixels = stbi_load_from_memory(g_BeyondIconPNG, sizeof(g_BeyondIconPNG), &icon.width, &icon.height, &channels, 4);
				glfwSetWindowIcon(m_Window, 1, &icon);
				stbi_image_free(icon.pixels);
			}
		}
		...
		if (isRawMouseMotionSupported)
			...
			BEY_CORE_WARN_TAG("Platform", "Raw mouse motion not supported.");
	...
	void Window::SetTitle(const eastl::string& title)
	{
		m_Data.Title = title;
