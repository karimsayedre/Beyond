...
namespace Beyond {
	...
	static Beyond::AllocationStats s_GlobalStats;
...
#ifndef BEY_DIST
			if (!found)
				BEY_CORE_WARN_TAG("Memory", "Memory block {0} not present in alloc map", memory);
#endif
		}
...
#if BEY_TRACK_MEMORY && BEY_PLATFORM_WINDOWS

_NODISCARD _Ret_notnull_ _Post_writable_byte_size_(size) _VCRT_ALLOCATOR
void* __CRTDECL operator new(size_t size)
{
	return Beyond::Allocator::Allocate(size);
}
...
{
	return Beyond::Allocator::Allocate(size, desc);
}
...
{
	return Beyond::Allocator::Allocate(size, file, line);
}
...
_NODISCARD _Ret_notnull_ _Post_writable_byte_size_(size) _VCRT_ALLOCATOR
void* __cdecl operator new[](size_t size, char const* file, int line, unsigned int type, char const* function, int blockType)
{
	return Beyond::Allocator::Allocate(size, file, line);
}

_NODISCARD _Ret_notnull_ _Post_writable_byte_size_(size) _VCRT_ALLOCATOR
void* __cdecl operator new[](unsigned __int64 size, unsigned __int64 alignment, unsigned __int64 offset, char const* file, int line, unsigned int type, char const* function, int blockType)
{
	return Beyond::Allocator::Allocate(size, file, line);
}
...
{
	return Beyond::Allocator::Free(memory);
}
...
inline int Vsnprintf8(char* pDestination, size_t n, const char* format, char* args)
{
