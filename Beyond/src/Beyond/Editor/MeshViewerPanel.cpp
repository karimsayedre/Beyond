...
#include "Beyond/Asset/MeshSerializer.h"
...
namespace Beyond {
	...
	void MeshViewerPanel::SetAsset(const Ref<Asset>& asset)
	{
		...
		sceneData->m_Mesh = Ref<Mesh>::Create(sceneData->m_MeshAsset, path);
	...
	void MeshViewerPanel::RenderMeshTab(ImGuiID dockspaceID, const std::shared_ptr<MeshScene>& sceneData)
	{
		...
		eastl::string toolBarName = fmt::eastl_format("##{}-{}", meshTabName, "toolbar");
		eastl::string viewportPanelName = fmt::eastl_format("Viewport##{}", meshTabName);
		eastl::string propertiesPanelName = fmt::eastl_format("Properties##{}", meshTabName);
	...
	void MeshViewerPanel::DrawMeshNode(const Ref<MeshSource>& meshAsset, const Ref<Mesh>& mesh)
	{
		...
		if (ImGui::Button("Create Mesh"))
		{
			...
			eastl::string filename = fmt::format("{0}.hmesh", meshPath.stem().string());
	...
	void MeshViewerPanel::MeshNodeHierarchy(const Ref<MeshSource>& meshAsset, Ref<Mesh> mesh, uint32_t nodeIndex, const glm::mat4& parentTransform, uint32_t level)
	{
		...
		BEY_CORE_ASSERT(meshAsset->m_NodeMap.find(node) != meshAsset->m_NodeMap.end());
