	...
	void MaterialEditor::Render()
	{
		...
		// Flags
		{
			if (UI::PropertyGridHeader("Properties"))
			{
				...
				//{
				//	transparent = blend;
				//	m_MaterialAsset->SetBlending(blend);
				//	//changed = true;
				//}
				if (UI::Property("Translucency", translucent))
				{

					m_MaterialAsset->SetTranslucency(translucent);
					//changed = true;
				}
				if (UI::Property("Two-Sided", twoSided))
				{
					m_MaterialAsset->SetTwoSided(twoSided);
					//changed = true;
				}
				if (UI::Property("DepthTested", depthTested))
				{
					m_MaterialAsset->SetIsDepthTested(depthTested);
					//changed = true;
				}
		...
		if (ImGui::CollapsingHeader("Albedo", nullptr, ImGuiTreeNodeFlags_DefaultOpen))
		{
			...
			auto& albedoColor = material->GetVector4("u_MaterialUniforms.AlbedoColor");
			Ref<Texture2D> albedoMap = material->GetBindlessTexture2D("u_MaterialUniforms.AlbedoTexIndex");
			...
			if (ImGui::BeginDragDropTarget())
			{
				...
				{
					material->Set("u_MaterialUniforms.AlbedoTexIndex", albedoMap);
				}
			...
			UI::ColorEdit4("##Albedo", glm::value_ptr(albedoColor), ImGuiColorEditFlags_NoInputs);
			...
			//nextRowCursorPos = ImGui::GetCursorPos();
			ImGui::SameLine(0, 20.0f);
			properCursorPos = ImGui::GetCursorPos();
			...
			float& ior = material->GetFloat("u_MaterialUniforms.IOR");
			//ImGui::SameLine(0, 20.0f);
			ImGui::SetNextItemWidth(100.0f);

			properCursorPos.y += 25;

			ImGui::SetCursorPos(properCursorPos);
			if (UI::DragFloat("##IOR", &ior, 0.01f, 1.0f, 50.0f))
			{
				material->Set("u_MaterialUniforms.IOR", ior);
				needsSerialize = true;
			}
			ImGui::SameLine();
			ImGui::TextUnformatted("IOR");
		}
		...
		if (!transparent)
		{
			// Textures ------------------------------------------------------------------------------
			{
				...
				{
					ImGui::PushStyleVar(ImGuiStyleVar_FramePadding, ImVec2(10, 10));
					bool useNormalMap = material->GetInt("u_MaterialUniforms.UseNormalMap");
					Ref<Texture2D> normalMap = material->GetBindlessTexture2D("u_MaterialUniforms.NormalTexIndex");
					...
					if (ImGui::BeginDragDropTarget())
					{
						...
						{
							material->Set("u_MaterialUniforms.NormalTexIndex", normalMap);
			...
			}
			{
				...
				if (ImGui::CollapsingHeader("Metalness", nullptr, ImGuiTreeNodeFlags_DefaultOpen))
				{
					...
					Ref<Texture2D> metalnessMap = material->GetBindlessTexture2D("u_MaterialUniforms.MetalnessTexIndex");
					...
					if (ImGui::BeginDragDropTarget())
					{
						...
						{
							material->Set("u_MaterialUniforms.MetalnessTexIndex", metalnessMap);
						}
						ImGui::EndDragDropTarget();
					}
			...
			}
			{
				...
				if (ImGui::CollapsingHeader("Roughness", nullptr, ImGuiTreeNodeFlags_DefaultOpen))
				{
					...
					Ref<Texture2D> roughnessMap = material->GetBindlessTexture2D("u_MaterialUniforms.RoughnessTexIndex");
					...
					if (ImGui::BeginDragDropTarget())
					{
						...
						{
							material->Set("u_MaterialUniforms.RoughnessTexIndex", roughnessMap);
						}
						ImGui::EndDragDropTarget();
					}
	...
	void SoundConfigEditor::Render()
	{
...

		{
			...
			if (spatializationOpen)
			{
				...
				const auto& attenModStr = std::vector<eastl::string>{ getTextForModel(AttModel::None),
