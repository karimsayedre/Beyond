...
namespace Beyond {
	...
	public:
		...
		// Textures
		inline static Ref<Texture2D> BeyondLogoTexture = nullptr;
		...
		static void Init()
		{
			...
			SceneFileIcon = LoadTexture("Beyond.png", "SceneFileIcon", spec);
			...
			BeyondLogoTexture = LoadTexture("BeyondLogo_Light.png");
	...
	private:
		...
		static Ref<Texture2D> LoadTexture(const std::filesystem::path& relativePath, const eastl::string& name, TextureSpecification specification)
		{
			...
			{
				BEY_CORE_FATAL("Failed to load icon {0}! The file doesn't exist.", path.string());
