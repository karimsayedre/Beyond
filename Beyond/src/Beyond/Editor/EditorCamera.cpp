...
namespace Beyond {
	...
	EditorCamera::EditorCamera(const float degFov, const float width, const float height, const float nearP, const float farP)
		: Camera(glm::perspectiveFov(glm::radians(degFov), width, height, farP, nearP), glm::perspectiveFov(glm::radians(degFov), width, height, nearP, farP)), m_FocalPoint(0.0f), m_VerticalFOV(glm::radians(degFov)), m_NearClip(nearP), m_FarClip(farP)  // NOLINT(readability-suspicious-call-argument)
	{
		Init();
	}
	...
	{
		m_FocalPoint = { 1.91, 6.5f, 23.55f };
		constexpr glm::vec3 position = { -17.33f, 5.93f, 27.54f };
		m_Distance = glm::distance(position, m_FocalPoint);

		glm::vec3 forward = glm::normalize(m_FocalPoint - position);
	...
	void EditorCamera::Focus(const glm::vec3& focusPoint)
	{
		...
		{
			m_Distance -= m_Distance - m_MinFocusDistance; //-V765
	...
	bool EditorCamera::IsMoving() const
	{
		if (m_IsActive)
		{
			if (m_IsMoving || m_PitchDelta < -0.0001f || m_PitchDelta > 0.0001f || m_YawDelta < -0.0001f || m_YawDelta > 0.0001f ||
				glm::any(glm::greaterThanEqual(m_PositionDelta, glm::vec3(0.0001f))) ||
				glm::any(glm::lessThanEqual(m_PositionDelta, glm::vec3(-0.0001f))))
				return true;
		}
		return false;
	}
	...
	{
		return { glm::vec3(-m_Pitch - m_PitchDelta, -m_Yaw - m_YawDelta, 0.0f) };
	}
}
