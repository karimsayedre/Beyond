...
#include "Beyond/Editor/FontAwesome.h"
...
namespace Beyond {
	...
	{
		BEY_CORE_ASSERT(s_Instance == nullptr);
		s_Instance = this;
	...
	bool EditorConsolePanel::OnImGuiRender(bool& isOpen)
	{
		bool changed = false;
	...
	void EditorConsolePanel::RenderMenu(const ImVec2& size)
	{
		...
		const eastl::string clearOnPlayText = fmt::eastl_format("{} Clear on Play", m_ClearOnPlay ? BEY_ICON_CHECK : BEY_ICON_TIMES);
...

		{
			...
			if (UI::ColoredButton(BEY_ICON_INFO_CIRCLE, GetToolbarButtonColor(m_MessageFilters & (int16_t)ConsoleMessageFlags::Info), textColor, buttonSize))
			...
			if (UI::ColoredButton(BEY_ICON_EXCLAMATION_TRIANGLE, GetToolbarButtonColor(m_MessageFilters & (int16_t)ConsoleMessageFlags::Warning), textColor, buttonSize))
			...
			if (UI::ColoredButton(BEY_ICON_EXCLAMATION_CIRCLE, GetToolbarButtonColor(m_MessageFilters & (int16_t)ConsoleMessageFlags::Error), textColor, buttonSize))
