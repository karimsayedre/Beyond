...
namespace Beyond {
	...
	public:
		...
		bool IsInconsistentPrimitive(GetOtherFunc func)
		{
			...
			Entity firstEntity = m_Context->GetEntityWithUUID(entities[0]);
			const TPrimitive& first = (TPrimitive)func(firstEntity.GetComponent<TComponent>());
		...
		void DrawComponent(const eastl::string& name, UIFunction uiFunction, Ref<Texture2D> icon = nullptr)
		{
	...
	private:
		...
		void DrawEntityNode(Entity entity, const eastl::string& searchFilter = {});
		...
		bool TagSearchRecursive(Entity entity, eastl::string_view searchFilter, uint32_t maxSearchDepth, uint32_t currentDepth = 1);
	...
	inline void SceneHierarchyPanel::EditGraphInput<glm::vec3>(Identifier id, choc::value::ValueView& value, const std::vector<UUID>& entities)
	{
		...
		{
			BEY_CORE_ASSERT(false, "value is not glm::vec3");
		}
