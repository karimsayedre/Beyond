...
#pragma endregion
	...
	bool NodeGraphEditorBase::DrawPinPropertyEdit(PinPropertyContext& context)
	{
		...
		switch ((EPinType)pin->GetType())
		{
			...
			default:
				BEY_CORE_ASSERT(false, "Unhandled pin type");
				break;
		}
	...
	void NodeGraphEditorBase::DrawDeferredComboBoxes(PinPropertyContext& pinContext)
	{
		ed::Suspend();
		{
			...
			/// Enum combo box
			{
				...
				{
					bool changed = false;
					BEY_CORE_ASSERT(pin->EnumTokens.has_value() && pin->EnumTokens.value() != nullptr);
	...
	void IONodeGraphEditor::DrawPropertyDetails(EPropertyType graphPropertyType, std::string_view propertyName)
	{
		...
		// Type & Value
		{
			...
			if (selected == -1)
			{
				BEY_CORE_ERROR("Invalid type of Graph Input!");
				selected = 0;
			}
	...
	void IONodeGraphEditor::DrawGraphIO()
	{
		...
		//	{
		//		// TODO: add output
		//		BEY_CORE_WARN("Add Output to Graph not implemented.");
		//	}
		//};
		//
		//// OUTPUTS list
