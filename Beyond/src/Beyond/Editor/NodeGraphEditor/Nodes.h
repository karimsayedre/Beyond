		...
		public:
			...
			[[nodiscard]] static Node* SpawnNodeStatic(std::string_view category, std::string_view type)
			{
				...
				if (cat != Registry.end())
				{
					...
					{
						BEY_CORE_ERROR("SpawnNodeStatic() - Category {0} does not contain node type {1}", category, type);
						return nullptr;
					}
				}

				BEY_CORE_ERROR("SpawnNodeStatic() - Category {0} does not exist", category);
				return nullptr;
//				return T::SpawnNodeStatic();
			...
			static const std::vector<Token>* GetEnumTokens(std::string_view nodeName, std::string_view memberName)
			{
				...
				BEY_CORE_ASSERT(memberName.find("::") == std::string_view::npos);
