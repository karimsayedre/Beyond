...
namespace Beyond::AnimationGraph {
	...
	struct Types final
	{
		...
		using PinDescr = Beyond::PinDescr<EType, TValueType, Color, EPinType>;
		...
		struct Node : Beyond::Node
		{
			using Beyond::Node::Node;
			...
			int GetTypeID() const override {
				if (TypeID == ENodeType::None)
				{
					...
					if (Category == "StateMachine")
					{
						...
						else                               BEY_CORE_ASSERT(false, "Unknown node type");
