...
#include "Beyond/Animation/AnimationGraphFactory.h"
...
namespace AG = Beyond::AnimationGraph;
...
namespace Beyond {
	...
	namespace AnimationGraph {
		...
		public:
			...
			{
				BEY_CORE_ASSERT(m_Options.Model);
			}
			...
			bool Parse()
			{
				...
				{
					BEY_CONSOLE_LOG_ERROR("Failed to construct Animation Graph Prototype!\n{}", e.what());
					return false;
				}
			}
				...
				if (!m_Options.RootNode || m_Options.Model->GetAllNodes().contains(m_Options.RootNode->ID))
				{
					...
					for (const auto node : nodes)
					{
						...
						if (node->Type == NodeType::Subroutine)
						{
							...
							{
								BEY_CORE_ASSERT(false, "Failed to parse subroutine! (this should not happen - check PreValidateGraph)");
							}
						}
					}
				}
			...
			void ParseConnections()
			{
				...
				if (!m_Options.RootNode || m_Options.Model->GetAllNodes().contains(m_Options.RootNode->ID))
				{
					...
					for (const auto& link : links)
					{
						...
						if (m_Options.Model->IsLocalVariableNode(sourceNode))
						{
							...
							*/
							if (auto* localVarSourcePin = m_Options.Model->FindLocalVariableSource(sourceNode->Name))
							{
								...
								if (!sourceNode)
								{
									BEY_CORE_ERROR("Failed to find source node while parsing Local Variable.");
									continue;
								}
							...
							else // Local variable plug connection
							{
								...
								else
								{
									...
									BEY_CORE_ERROR("We can't have default plugs for Local Variables of flow type!");
								}
						...
						{
							BEY_CORE_ASSERT(destNode->Inputs.size() >= 2);
							BEY_CORE_ASSERT(destNode->Inputs[1]->GetType() == Types::EPinType::String);
						...
						{
							BEY_CORE_ASSERT(FindNodeByID(sourceNode->ID));
							BEY_CORE_ASSERT(FindNodeByID(sourceNode->ID));
							BEY_CORE_ASSERT(FindNodeByID(destNode->ID));
		...
		Scope<Prototype> ConstructPrototype(GeneratorOptions options)
		{
			...
			if (!errors.empty())
			{
				...
				BEY_CONSOLE_LOG_ERROR("Failed to construct Animation Graph Prototype! " + choc::text::joinStrings(errors, "\n"));
				return nullptr;
			}
	...
	{
		AnimationGraphNodeEditorModel::Deserialize();
	...
	{
		BEY_CORE_INFO_TAG("Animation", "SaveGraphState() node = {0}: {1}", m_RootNodeId, data);
	...
	{
		BEY_CORE_INFO_TAG("Animation", "LoadGraphState() node = {0}: {1}", m_RootNodeId, m_AnimationGraph->m_GraphState.at(m_RootNodeId));
	...
	void AnimationGraphNodeEditorModel::OnGraphPropertyNameChanged(EPropertyType type, const std::string& oldName, const std::string& newName)
	{
		...
		{
			BEY_CORE_ASSERT(!node->Outputs.empty() || !node->Inputs.empty());
	...
	Node* AnimationGraphNodeEditorModel::SpawnLocalVariableNode(const std::string& variableName, bool getter)
	{
		...
		if (!getter)
		{
			for (const auto& node : GetNodes())
			{
				...
				{
					BEY_CONSOLE_LOG_TRACE("Setter node for the variable '{}' already exists!", variableName);
	...
	std::string AnimationGraphNodeEditorModel::GetNodeError(const Node* node)
	{
		...
		// State or blend-space is well defined if it has a sub graph, and there are no ill-defined nodes in the sub-graph
		if (node->Type == NodeType::Subroutine)
		{
			if (!GetAllNodes().contains(node->ID) || GetAllNodes().at(node->ID).size() <= (node->GetTypeID() == AG::Types::State? 1 : 0))
				return fmt::format("{} sub-graph has not been defined", node->Name);
		...
		// Transition is not well defined if it always returns false
		if (node->GetTypeID() == AG::Types::Transition)
		{
			if (!GetAllNodes().contains(node->ID) || GetAllNodes().at(node->ID).empty())
				return "Transition graph has not been defined";
			...
			{
				if (outputNode->Inputs[0]->Value.getBool() == false)
					return "Transition graph always returns false";
			}
		...
		for (auto input : node->Inputs)
		{
			...
			if (input->GetType() == AG::Types::EPinType::AnimationAsset)
			{
				...
				if (!AssetManager::IsAssetHandleValid(assetHandle) && !GetNodeConnectedToPin(input->ID))
					return fmt::format("{} asset is not valid", input->Name);
			}

			if ((input->Name == "Pose") && !GetNodeConnectedToPin(input->ID))
				return fmt::format("{} input is not connected", input->Name);

			if (input->GetType() == AG::Types::EPinType::Bone)
			{
				BEY_CORE_ASSERT(input->Storage != StorageKind::Array, "Found bone pin that is an array.  This needs to be handled!");

				auto selectedBone = input->Value["Value"].getString();
				uint32_t boneIndex = FindBoneIndex(selectedBone);
				if (boneIndex == Skeleton::NullIndex)
					return fmt::format("{} '{}' not found in skeleton", input->Name, selectedBone);
			}
		}
	...
	{
		BEY_CORE_INFO_TAG("Animation", "Compiling graph...");
		...
		if (onCompile)
		{
			if (onCompile(m_AnimationGraph.As<Asset>()))
			{
				...
				BEY_CORE_INFO_TAG("Animation", "Graph has been compiled.");
				if (onCompiledSuccessfully)
					onCompiledSuccessfully();

				BEY_CONSOLE_LOG_TRACE("Animation graph successfully compiled.");

			}
			else
			{
				BEY_CONSOLE_LOG_ERROR("Failed to compile animation graph!");
			}
		}
		else
		{
	...
	{
		BEY_CORE_ASSERT(m_CurrentPath.size() > 0 && m_CurrentPath.back()->GetTypeID() == AG::Types::StateMachine);
