...
#include "Beyond/Editor/NodeGraphEditor/NodeGraphEditorContext.h"
#include "Beyond/Vendor/imgui-node-editor/widgets.h"
...
namespace Beyond {
	...
	{
		auto* drawList = ImGui::GetWindowDrawList();
		std::string coords = fmt::format("({:.2f}, {:.2f})", blendSpacePos.x, blendSpacePos.y);
	...
	AnimationGraphEditor::AnimationGraphEditor() : IONodeGraphEditor("Animation Graph Editor")
	{
		...
		onDragDropOntoCanvas = [&]() {
			...
			auto data = ImGui::AcceptDragDropPayload("asset_payload", ImGuiDragDropFlags_AcceptBeforeDelivery);
			if (data)
			{
				...
				if (ImGui::Begin("Subgraph", nullptr, ImGuiWindowFlags_NoCollapse))
				{
					if (!m_FirstFrame)
					{
						...
						if (m_SelectedTransitionNodes.size() == 1)
						{
							...
							BEY_CORE_ASSERT(selectedTransition);
							...
							BEY_CORE_ASSERT(startNode && endNode);
							{
								UI::ScopedFont largeFont(ImGui::GetIO().Fonts->Fonts[1]);
								UI::ScopedColour darkText(ImGuiCol_Text, Colors::Theme::textDarker);
	...
	void AnimationGraphEditor::OnClose()
	{
		...
		BEY_CORE_ASSERT(ed::GetCurrentEditor() == m_Editor);
	...
	{
		BEY_CORE_ASSERT(node->Type != NodeType::Comment);
		auto model = static_cast<AnimationGraphNodeEditorModel*>(GetModel());
	...
	void AnimationGraphEditor::DrawNodeForStateMachine(Node* node, NodeBuilder& builder, PinPropertyContext& pinContext)
	{
		...
		BEY_CORE_ASSERT((node->GetTypeID() == AG::Types::ENodeType::State) || (node->GetTypeID() == AG::Types::QuickState) || (node->GetTypeID() == AG::Types::ENodeType::StateMachine) || (node->GetTypeID() == AG::Types::ENodeType::Transition));
	...
	void AnimationGraphEditor::DrawNodeForBlendSpace(Node* node, NodeBuilder& builder, PinPropertyContext& pinContext)
	{
		...
		//BEY_CORE_ASSERT(node->GetTypeID() == AG::Types::ENodeType::BlendSpaceVertex);
	...
	void AnimationGraphEditor::DrawBlendSpaceTriangles()
	{
		...
		for (auto triangle : triangulation.triangles)
		{
			...
			std::string coords = fmt::format("({:.2f}, {:.2f}, {:.2f})", u, v, w);
			ImVec2 coordsSize = ImGui::CalcTextSize(coords.c_str());
		...
		for (const auto& [subgraph, links] : allLinks)
		{
			...
			for (const auto& link : links)
			{
				...
				if (oldToNewNodeIDMap.contains(startNodeID) && oldToNewNodeIDMap.contains(endNodeID))
				{
					...
					BEY_CORE_ASSERT(newStartPin && newEndPin);
	...
	void AnimationGraphEditor::DrawNodeContextMenu(Node* node)
	{
		...
		if (selectedNodes.empty() && m_SelectedTransitionNodes.empty() && m_SelectedBlendSpaceVertices.empty())
			return;

		if (ImGui::MenuItem("Delete"))
			DeleteSelection();

		if(selectedNodes.empty() && m_SelectedBlendSpaceVertices.empty())
			return;
		...
		UI::Widgets::SearchWidget(searchString, "Search Assets...", &grabFocus);
		{
			...
			if (ImGui::BeginChild("##new_node_scroll_list", ImVec2(0.0f, 350.0f)))
			{
				for (const auto& [path, metadata] : Project::GetEditorAssetManager()->GetAssetRegistry())
				{
					...
					const auto& assetName = metadata.FilePath.stem().string();
					if (!searchString.empty() && !UI::IsMatchingSearch(assetName, eastl::string_view(searchString.c_str(), searchString.size())))
						continue;
