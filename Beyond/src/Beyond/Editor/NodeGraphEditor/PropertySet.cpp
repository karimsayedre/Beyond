...
//}
	...
	choc::value::Value ParseCustomValueType(const choc::value::ValueView& value)
	{
		if (value.isVoid())
		{
			BEY_CORE_WARN("Failed to deserialize custom value type, value is not an object.");
	...
	void ParseCustomObject(PropertySet& ps, std::string_view name, const choc::value::ValueView& value)
	{
		if (value["Value"].isArray())
		{
			...
			BEY_CORE_ASSERT(type == "AssetHandle", "The only custom type is supported for now is \"AssetHandle\".");
			BEY_CORE_ASSERT(inArraySize > 0);
			...
			BEY_CORE_ASSERT(valueArray.size() == inArraySize)
	...
	{
		BEY_CORE_ASSERT(false, "Should use ..ExternalValue() serialization instead.");
	...
	std::string PropertySet::ToJSON() const { BEY_CORE_ASSERT(false, "Should use ..ExternalValue() serialization instead."); return PropertySetToString(properties.get()); }
#endif
