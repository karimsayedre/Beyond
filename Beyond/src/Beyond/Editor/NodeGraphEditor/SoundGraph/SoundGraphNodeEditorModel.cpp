...
#include "Beyond/Audio/SoundGraph/SoundGraph.h"
#include "Beyond/Audio/SoundGraph/SoundGraphSource.h"
#include "Beyond/Audio/SoundGraph/GraphGeneration.h" 
...
namespace Beyond {
	...
	void NodeDump(const Ref<SG::SoundGraph>& soundGraph, UUID nodeID, std::string_view dumpIdentifier)
	{
		...
		{
			BEY_CORE_WARN("======================");
			BEY_CORE_WARN("NODE DUMP");
			BEY_CORE_WARN(dumpIdentifier);
			BEY_CORE_TRACE("----------------------");
			BEY_CORE_TRACE("{}", node->dbgName);
			for (const auto& [id, value] : node->Ins)
			{
				BEY_CORE_TRACE("[{0}] {1}", id, choc::json::toString(value));
			}
			BEY_CORE_TRACE("----------------------");
		}
	}
	...
	{
		BEY_CORE_INFO_TAG("Audio", "Compiling graph...");
		...
		if (onCompile)
		{
			if (onCompile(m_GraphAsset.As<Asset>()))
			{
				...
				BEY_CORE_INFO_TAG("Audio", "Graph has been compiled.");
				if (onCompiledSuccessfully)
					onCompiledSuccessfully();

				BEY_CONSOLE_LOG_TRACE("Sound graph successfully compiled.");
			...
			{
				BEY_CONSOLE_LOG_ERROR("Failed to compile sound graph!");
			}
		}
		else
		{
	...
	bool SoundGraphNodeEditorModel::Sort()
	{
		...
		if (!sortEndpointNode)
		{
			...
			BEY_CORE_ERROR("Failed to find endpoint node to sort the graph!");
		...
		auto correctSort = [&](Node* context, Node* previous, auto& correctSort) -> void
		{
			...
			if (IsLocalVariableNode(context))
			{
				...
				BEY_CORE_ASSERT(localVarSortIndex != Node::UndefinedSortIndex);
