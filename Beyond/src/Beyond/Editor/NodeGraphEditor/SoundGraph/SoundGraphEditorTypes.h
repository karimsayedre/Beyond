...
namespace Beyond::SoundGraph {
	...
	public:
		...
		using SGPinDescr = Beyond::PinDescr<EType, TValueType, Color, ESGPinType>;
		...
		static ESGPinType GetPinTypeForValue(const choc::value::ValueView value)
		{
			...
			if (type.isObject())
			{
				...
				else if (Beyond::Utils::IsAssetHandle<AssetType::Audio>(type))
				{
					pinType = ESGPinType::AudioAsset;
				}
				else if (value.hasObjectMember("Type"))
				{
					BEY_CORE_WARN("Found pin with custom type {}, need to handle it!", value["Type"].getString());
				}
			}
			...
			{
				pinType = ESGPinType::AudioAsset;
				BEY_CORE_ASSERT(false, "Shouldn't be using Int64 Value for AudioAsset type in SoundGraph.");
			}
			else BEY_CORE_ASSERT(false);
