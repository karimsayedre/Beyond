...
namespace Beyond {
	...
	void PanelManager::OnImGuiRender()
	{
		...
		for (auto& panelMap : m_Panels)
		{
			for (auto& [id, panelData] : panelMap)
			{
				...
				if (panelData.IsOpen)
				{
					changed |= panelData.Panel->OnImGuiRender(panelData.IsOpen);
	...
	void PanelManager::Deserialize()
	{
		...
		if (!data["Panels"])
		{
			BEY_CONSOLE_LOG_ERROR("Failed to load EditorLayout.yaml from {} because the file appears to be corrupted!", layoutPath.parent_path().string());
			return;
		}
