...
#include "Beyond/Script/ScriptProfiler.h"
...
namespace Beyond {
	...
	void ScriptEngineDebugPanel::OnProjectChanged(const Ref<Project>& project)
	{
		...
		const auto& beyondCoreAssembly = ScriptEngine::GetCoreAssemblyInfo();
		const auto& appAssembly = ScriptEngine::GetAppAssemblyInfo();
		s_LoadedAssembliesMetadata.push_back(beyondCoreAssembly->Metadata);
		s_LoadedAssembliesMetadata.push_back(appAssembly->Metadata);

		for (const auto& referencedAssemblyMetadata : beyondCoreAssembly->ReferencedAssemblies)
	...
	bool ScriptEngineDebugPanel::OnImGuiRender(bool& open)
	{
		bool changed = false;
		if (ImGui::Begin("Script Engine Debug", &open))
		{
			...
			if (UI::PropertyGridHeader("Memory"))
			{
				...
				{
					eastl::string generationName = fmt::eastl_format("Generation {0}", generation);
			...
			if (UI::PropertyGridHeader(fmt::eastl_format("Loaded Assemblies ({0})", s_LoadedAssembliesMetadata.size()), false))
			{
			...
			if (UI::PropertyGridHeader("Cache", false))
			{
				...
				{
					const eastl::string label = fmt::eastl_format("{0} ({1})", managedClass.FullName, managedClass.ID);
			...
			// NOTE: Disabled for the time since it doesn't fully work
			...
			//		eastl::string className = ScriptUtils::ResolveMonoClassName(klass);
			//		eastl::string objectName = fmt::format("{0} ({1})", (void*)obj, className);
