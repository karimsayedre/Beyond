...
namespace Beyond {
	...
	// NOTE: For now this just corresponds to what menu the panel will be listed in (in Editor)
	...
	public:
		...
		static bool HasAnyChanged()
		{
			return std::exchange(s_HasChanges, false);
	...
	public:
		...
		Ref<TPanel> AddPanel(PanelCategory category, const PanelData& panelData)
		{
			...
			{
				BEY_CORE_ERROR_TAG("PanelManager", "A panel with the id '{0}' has already been added.", panelData.ID);
				return nullptr;
			}
		...
		Ref<TPanel> GetPanel(const char* strID)
		{
			...
			BEY_CORE_ERROR_TAG("PanelManager", "Couldn't find panel with id '{0}'", strID);
			return nullptr;
		}

	private:
		std::array<std::unordered_map<uint32_t, PanelData>, (size_t)PanelCategory::_COUNT> m_Panels;
		inline static bool s_HasChanges = false;
	};
