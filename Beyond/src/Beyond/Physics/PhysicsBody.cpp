	...
	void PhysicsBody::CreateCollisionShapesForEntity(Entity entity, bool ignoreCompoundShapes)
	{
		...
		if (entity.HasComponent<MeshColliderComponent>())
		{
			...
			if (!meshCache.Exists(colliderAsset))
			{
				...
				{
					BEY_CORE_WARN_TAG("Physics", "Tried to add an Mesh Collider with an invalid collider asset. Please make sure your collider has been cooked.");
					return;
				}
			}
			...
			{
				BEY_CONSOLE_LOG_ERROR("Entity '{0}' ({1}) has a dynamic RigidBodyComponent along with a Complex MeshColliderComponent! This isn't allowed.", m_Entity.Name(), m_Entity.GetUUID());
				return;
			}
