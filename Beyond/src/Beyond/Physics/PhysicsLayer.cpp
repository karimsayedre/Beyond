...
namespace Beyond {
	...
	uint32_t PhysicsLayerManager::AddLayer(const eastl::string& name, bool setCollisions)
	{
	...
	void PhysicsLayerManager::RemoveLayer(uint32_t layerId)
	{
		...
		Utils::RemoveIf(s_LayerNames, [&](const eastl::string& name) { return name == layerInfo.Name; });
		Utils::RemoveIf(s_Layers, [&](const PhysicsLayer& layer) { return layer.LayerID == layerId; });
	}

	void PhysicsLayerManager::UpdateLayerName(uint32_t layerId, const eastl::string& newName)
	{
		...
		Utils::RemoveIf(s_LayerNames, [&](const eastl::string& name) { return name == layer.Name; });
	...
	PhysicsLayer& PhysicsLayerManager::GetLayer(const eastl::string& layerName)
	{
	...
	bool PhysicsLayerManager::IsLayerValid(const eastl::string& layerName)
	{
	...
	std::vector<eastl::string> PhysicsLayerManager::s_LayerNames;
