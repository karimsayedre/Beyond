...
namespace Beyond {
	...
	void JoltBody::SetGravityEnabled(bool isEnabled)
	{
		...
		BEY_CORE_VERIFY(lock.Succeeded());
	...
	void JoltBody::AddForce(const glm::vec3& force, const glm::vec3& location, EForceMode forceMode, bool forceWake /*= true*/)
	{
		...
		switch (forceMode)
		{
			...
			case EForceMode::VelocityChange:
			{
				// NOTE: Can't change the velocity at a specific point as far as I can tell...
				body.SetLinearVelocityClamped(body.GetLinearVelocity() + JoltUtils::ToJoltVector(force));
	...
	void JoltBody::AddRadialImpulse(const glm::vec3& origin, float radius, float strength, EFalloffMode falloff, bool velocityChange)
	{
		...
		BEY_CORE_VERIFY(readLock.Succeeded());
		...
		BEY_CORE_VERIFY(writeLock.Succeeded());
	...
	void JoltBody::CreateStaticBody(JPH::BodyInterface& bodyInterface)
	{
		...
		BEY_CORE_VERIFY(scene, "No scene active?");
		...
		{
			BEY_CORE_INFO_TAG("Physics", "Failed to create static PhysicsBody, no collision shape provided!");
			m_Shapes.clear();
			return;
		}
		...
		if (body == nullptr)
		{
			BEY_CORE_ERROR_TAG("Physics", "Failed to create PhysicsBody! This means we don't have enough space to store more RigidBodies!");
			return;
		}
	...
	void JoltBody::CreateDynamicBody(JPH::BodyInterface& bodyInterface)
	{
		...
		if (firstShape == nullptr)
		{
			BEY_CORE_INFO_TAG("Physics", "Failed to create dynamic PhysicsBody, no collision shape provided!");
			m_Shapes.clear();
			return;
		}
		...
		if (!PhysicsLayerManager::IsLayerValid(rigidBodyComponent.LayerID))
		{
			...
			BEY_CONSOLE_LOG_WARN("Entity '{}' has a RigidBodyComponent with an invalid layer set! Using the Default layer.", m_Entity.Name());
		}
