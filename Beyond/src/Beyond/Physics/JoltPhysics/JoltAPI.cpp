...
namespace Beyond {
	...
	static void JoltTraceCallback(const char* format, ...)
	{
		...
		BEY_CORE_TRACE_TAG("Physics", buffer);
	}
	...
	{
		BEY_CORE_FATAL_TAG("Physics", "{}:{}: ({}) {}", file, line, expression, message != nullptr ? message : "");
		return true;
	}
...
#define BEY_ENABLE_MALLOC_ALLOC 0

	void JoltAPI::Init()
	{
		BEY_CORE_VERIFY(!s_JoltData, "Can't initialize Jolt multiple times!");
...
#if BEY_ENABLE_MALLOC_ALLOC
		// NOTE: We shouldn't be using this allocator if we want the best performance
		...
		// NOTE: Just construct the thread pool for now, don't start the threads
	...
	const eastl::string& JoltAPI::GetLastErrorMessage() const { return s_JoltData->LastErrorMessage; }
