...
namespace Beyond {
	...
	{
		BEY_CORE_VERIFY(false, "Cannot remove shape from an ImmutableCompoundShape!");
	}
	...
	void JoltImmutableCompoundShape::Create()
	{
		...
		if (result.HasError())
		{
			BEY_CORE_ERROR_TAG("Physics", "Failed to create Immutable Compound Shape. Error: {}", result.GetError());
			return;
		}
	...
	void JoltMutableCompoundShape::Create()
	{
		...
		if (result.HasError())
		{
			BEY_CORE_ERROR_TAG("Physics", "Failed to create Mutable Compound Shape. Error: {}", result.GetError());
			return;
		}
		...
		: BoxShape(entity)
	{
		...
		if (result.HasError())
		{
			BEY_CORE_ERROR_TAG("Physics", "Failed to create BoxShape. Error: {}", result.GetError());
			return;
		}
		...
		: SphereShape(entity)
	{
		...
		if (result.HasError())
		{
			BEY_CORE_ERROR_TAG("Physics", "Failed to create SphereShape. Error: {}", result.GetError());
			return;
		}
		...
		: CapsuleShape(entity)
	{
		...
		if (result.HasError())
		{
			BEY_CORE_ERROR_TAG("Physics", "Failed to create CapsuleShape. Error: {}", result.GetError());
			return;
		}
		...
		: ConvexMeshShape(entity)
	{
		...
		BEY_CORE_ASSERT(meshData.Submeshes.size() > component.SubmeshIndex);
		...
		if (result.HasError())
		{
			BEY_CORE_ERROR_TAG("Physics", "Failed to construct ConvexMesh: {}", result.GetError());
			return;
		}
		...
		: TriangleMeshShape(entity)
	{
		...
		for (const auto& submeshData : meshData.Submeshes)
		{
			...
			if (result.HasError())
			{
				BEY_CORE_ERROR_TAG("Physics", "Failed to construct ConvexMesh!");
				return;
			}
