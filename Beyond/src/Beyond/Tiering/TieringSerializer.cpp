...
namespace Beyond {
	...
	bool TieringSerializer::Deserialize(Tiering::TieringSettings& outTieringSettings, const std::filesystem::path& filepath)
	{
		...
		if (tieringSettings["ShadowQuality"])
			outTieringSettings.RendererTS.ShadowQuality = Utils::ShadowQualitySettingFromString(tieringSettings["ShadowQuality"].as<std::string>().c_str());
		if (tieringSettings["ShadowResolution"])
			outTieringSettings.RendererTS.ShadowResolution = Utils::ShadowResolutionSettingFromString(tieringSettings["ShadowResolution"].as<std::string>().c_str());

		if (tieringSettings["AmbientOcclusionQuality"])
		{
			outTieringSettings.RendererTS.AOQuality = Utils::AmbientOcclusionQualitySettingFromString(tieringSettings["AmbientOcclusionQuality"].as<std::string>().c_str());
		}
		...
		if (tieringSettings["SSRQuality"])
			outTieringSettings.RendererTS.SSRQuality = Utils::SSRQualitySettingFromString(tieringSettings["SSRQuality"].as<std::string>().c_str());
