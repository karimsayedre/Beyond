	...
	void SceneRenderer::Init()
	{
...

		{
			...
			if (VulkanContext::GetCurrentDevice()->IsRaytracingSupported())
			{
				...
				{
					Ref<Shader> shader = Renderer::GetShaderLibrary()->Get("Pathtrace-GGX-comp");
					m_GGXCompMaterial  = Material::Create(shader, "Pathtrace-GGX-comp");

					ComputePassSpecification computePassSpecification;
					computePassSpecification.DebugName = "Pathtrace-GGX-comp";
					computePassSpecification.Pipeline  = PipelineCompute::Create(shader);
					m_GGXCompPathtrace				   = ComputePass::Create(computePassSpecification);

					m_GGXCompPathtrace->SetInput("u_Camera", m_UBSCamera);
					m_GGXCompPathtrace->SetInput("o_Image", m_RaytracingImage);
					m_GGXCompPathtrace->SetInput("o_AlbedoColor", m_AlbedoImage);
					m_GGXCompPathtrace->SetInput("io_AccumulatedColor", m_AccumulationImage);
					m_GGXCompPathtrace->SetInput("o_ViewNormalsLuminance", m_RaytracingNormalsImage);
					m_GGXCompPathtrace->SetInput("o_MetalnessRoughness", m_RaytracingMetalnessRoughnessImage);
					m_GGXCompPathtrace->SetInput("o_PrimaryHitT", m_RaytracingPrimaryHitT);

					m_GGXCompPathtrace->SetInput("objDescs", m_SBSObjectSpecs);
					m_GGXCompPathtrace->SetInput("materials", m_SBSMaterialBuffer);
					m_GGXCompPathtrace->SetInput("TLAS", m_SceneTLAS);

					m_GGXCompPathtrace->SetInput("Samplers", Renderer::GetBilinearSampler(), 0);
					m_GGXCompPathtrace->SetInput("Samplers", Renderer::GetPointSampler(), 1);
					m_GGXCompPathtrace->SetInput("Samplers", Renderer::GetAnisoSampler(), 2);

					m_GGXCompPathtrace->SetInput("u_PointLights", m_UBSPointLights);
					m_GGXCompPathtrace->SetInput("u_SpotLights", m_UBSSpotLights);
					m_GGXCompPathtrace->SetInput("u_SceneData", m_UBSScene);
					// m_GGXCompPathtrace->SetInput("s_VisiblePointLightIndicesBuffer", m_SBSVisiblePointLightIndicesBuffer);
					// m_GGXCompPathtrace->SetInput("s_VisibleSpotLightIndicesBuffer", m_SBSVisibleSpotLightIndicesBuffer);

					m_GGXCompPathtrace->SetInput("Bey_DebugImage", m_DebugImage);

					m_GGXCompPathtrace->SetInput("u_ScreenData", m_UBSScreenData);

					m_GGXCompPathtrace->SetInput("u_EnvRadianceTex", Renderer::GetBlackCubeTexture());
					m_GGXCompPathtrace->SetInput("u_EnvIrradianceTex", Renderer::GetBlackCubeTexture());

					// Set 3
					m_GGXCompPathtrace->SetInput("u_BRDFLUTTexture", Renderer::GetBRDFLutTexture());
					BEY_CORE_VERIFY(m_GGXCompPathtrace->Validate());
					m_GGXCompPathtrace->Bake();
...
#pragma endregion
	...
	void SceneRenderer::BeginScene(const SceneRendererCamera& camera, Timestep ts)
	{
		...
		if (VulkanContext::GetCurrentDevice()->IsRaytracingSupported())
		{
			m_GGXCompPathtrace->SetInput("u_EnvIrradianceTex", m_SceneData.SceneEnvironment->IrradianceMap);
			m_GGXCompPathtrace->SetInput("u_EnvRadianceTex", m_SceneData.SceneEnvironment->RadianceMap);
...
#pragma region SUBMIT
	...
	void SceneRenderer::SubmitMesh(Ref<Mesh> mesh, uint32_t submeshIndex, Ref<MaterialTable> materialTable, const glm::mat4& transform, const std::vector<glm::mat4>& boneTransforms, Ref<Material> overrideMaterial)
	{
		...
		auto& dc			= destDrawList[meshKey];
		{
			...
			SubmitToRaytracer(dc, material.Raw(), finalTransform);
		}
	...
	{
		BEY_PROFILE_FUNC();
		BEY_SCOPE_PERF("SceneRenderer::SubmitSelectedStaticMesh");
...
#pragma endregion
	...
	{
		BEY_PROFILE_FUNC();
		BEY_SCOPE_PERF("SceneRenderer::PreRender");

		uint32_t offset = 0;
		for (auto& [meshKey, transformMapData] : m_MeshTransformMap)
		{
			// Record the starting offset for this instance group.
			transformMapData.TransformIndex = offset;

			// Copy each transform to the GPU buffer.
			for (uint32_t i = 0; i < transformMapData.Transforms.size(); i++)
			{
				m_GPUTransformBuffers.Data[offset].PreviousMRow = std::exchange(m_GPUTransformBuffers.Data[offset].CurrentMRow, transformMapData.Transforms[i]);
				offset++;
			}
		}
		...
		if (offset > 0)
			m_SBSTransforms->Get()->SetData(m_GPUTransformBuffers.Data, static_cast<uint32_t>(offset * sizeof(TransformData)));
		...
		if (index > 0)
			m_SBSBoneTransforms->Get()->SetData(m_BoneTransformsData, static_cast<uint32_t>(index * sizeof(BoneTransforms)));
	}

	void SceneRenderer::BuildAccelerationStructures()
	{
		if (m_RaytracingSettings.Mode != RaytracingMode::None || m_DDGISettings.Enable)
		{
			m_CommandBuffers[eTLASBuild]->Begin();
			m_GPUTimeQueries.BuildAccelerationStructuresQuery = m_CommandBuffers[eTLASBuild]->BeginTimestampQuery();
			m_MainRaytracer->BuildTlas(m_CommandBuffers[eTLASBuild]);

			m_MainRaytracer->ClearInternalInstances();
			m_CommandBuffers[eTLASBuild]->EndTimestampQuery(m_GPUTimeQueries.BuildAccelerationStructuresQuery);
		...
		if (m_RaytracingSettings.Mode != RaytracingMode::None)
		{
			...
			if (m_RaytracingSettings.Mode == RaytracingMode::Pathtracing)
			{
				...
				}
				{
					...
					m_GPUTimeQueries.RaytracingQuery = raytracingCmdBuffer->BeginTimestampQuery();
					Renderer::BeginComputePass(raytracingCmdBuffer, m_GGXCompPathtrace);
					BeginGPUPerfMarker(raytracingCmdBuffer, "Ray tracing");
					...
					Renderer::DispatchCompute(raytracingCmdBuffer, m_GGXCompPathtrace, m_GGXCompMaterial, workGroups, Buffer(&pushConstant, sizeof(pushConstant)));

					EndGPUPerfMarker(raytracingCmdBuffer);
					Renderer::EndComputePass(raytracingCmdBuffer, m_GGXCompPathtrace);
					raytracingCmdBuffer->EndTimestampQuery(m_GPUTimeQueries.RaytracingQuery);
				}
			}
	...
	{
		if ((m_RaytracingSettings.Mode == RaytracingMode::None) && m_DDGISettings.ProbeVis && m_DDGISettings.Enable)
		{
	...
	{
		if (m_RaytracingSettings.Mode == RaytracingMode::None && !Renderer::GetDDGIVolumes().empty() && m_DDGISettings.Enable)
		{
	...
	void SceneRenderer::FlushDrawList()
	{
		if (m_ResourcesCreated && m_RenderWidth > 0 && m_RenderHeight > 0)
		{
			...
			// GTAO
			if (m_Options.EnableGTAO && !m_DDGISettings.ProbeVis && m_RaytracingSettings.Mode == RaytracingMode::None)
			{
				GTAOCompute();
				GTAODenoiseCompute();
				AOComposite();
