...
namespace Beyond
{
	...
	struct BoneInfluence
	{
		...
		void AddBoneData(uint32_t boneInfoIndex, float weight)
		{
			...
			{
				BEY_CORE_WARN("Vertex bone weight is out of range. We will clamp it to [0, 1] (BoneID={0}, Weight={1})", boneInfoIndex, weight);
			...
			if (weight > 0.0f)
			{
				...
				BEY_CORE_WARN("Vertex has more than four bones affecting it, extra bone influences will be discarded (BoneID={0}, Weight={1})", boneInfoIndex, weight);
			}
		}
	...
	public:
		...
		eastl::string NodeName, MeshName;
		bool IsRigged = false;
	...
	public:
		MeshSource() = default;
		MeshSource(const std::vector<Vertex>& vertices, const std::vector<Index>& indices, const glm::mat4& transform, const std::string& name);
		MeshSource(const std::vector<Vertex>& vertices, const std::vector<Index>& indices, const std::string& name, const std::vector<Submesh>& submeshes);
		...
		const Skeleton& GetSkeleton() const { BEY_CORE_ASSERT(m_Skeleton, "Attempted to access null skeleton!"); return *m_Skeleton; }
		bool IsCompatibleSkeleton(const uint32_t animationIndex, const Skeleton& skeleton) const;

		std::vector<eastl::string> GetAnimationNames() const;
		...
		uint32_t GetTriangleCountInSubmesh(uint32_t submeshIndex) const { return (uint32_t)m_TriangleCache.at(submeshIndex).size(); }
		...
		bool IsReady() const
		{
			if (m_IndexBuffer && m_VertexBuffer && m_IsReady)
				return m_IndexBuffer->IsReady() && m_VertexBuffer->IsReady();
			else return false;
		}
	private:
		...
		std::vector<eastl::string> m_AnimationNames;
	...
	{
	public:
		explicit Mesh(Ref<MeshSource> meshSource, const std::string& name);
		Mesh(Ref<MeshSource> meshSource, const std::string& name, const std::vector<uint32_t>& submeshes);
		...
		const MeshSource* GetMeshSourceRaw() const { return m_MeshSource.Raw(); }
		...
		bool IsReady() const { return m_MeshSource->IsReady() && m_IsReady; }
	private:
	...
	{
	public:
		explicit StaticMesh(Ref<MeshSource> meshSource, const std::string& name);
		StaticMesh(Ref<MeshSource> meshSource, const std::string& name, const std::vector<uint32_t>& submeshes);
