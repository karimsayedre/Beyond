...
#include "Beyond/Renderer/UI/MSDFData.h"
...
namespace Beyond {
	...
	void Renderer2D::Init()
	{
		if (m_Specification.SwapChainTarget)
			...
			m_RenderCommandBuffer = RenderCommandBuffer::Create(0, false, "Renderer2D");

		uint32_t framesInFlight = Renderer::GetConfig().FramesInFlight;
		m_UBSCamera = UniformBufferSet::Create(sizeof(UBCamera), "Renderer2D Camera", framesInFlight);
		...
		Ref<Framebuffer> framebuffer = Framebuffer::Create(framebufferSpec);
		{
			...
			quadSpec.Pipeline = RasterPipeline::Create(pipelineSpecification);
			m_QuadPass = RenderPass::Create(quadSpec);
			m_QuadPass->SetInput("u_Camera", m_UBSCamera);
			BEY_CORE_VERIFY(m_QuadPass->Validate());
			m_QuadPass->Bake();
			...
			for (uint32_t i = 0; i < framesInFlight; i++)
			{
				...
				m_QuadVertexBuffers[0][i] = VertexBuffer::Create(allocationSize, "2D Quad");
...

			{
				...
				m_QuadIndexBuffer = IndexBuffer::Create(quadIndices, "2D Quad", allocationSize);
				m_MemoryStats.TotalAllocated += allocationSize;
			}
			hdelete[] quadIndices;
		}
		...
		// Lines
		{
...

			{
				...
				lineSpec.Pipeline = RasterPipeline::Create(pipelineSpecification);
				m_LinePass = RenderPass::Create(lineSpec);
				m_LinePass->SetInput("u_Camera", m_UBSCamera);
				BEY_CORE_VERIFY(m_LinePass->Validate());
				m_LinePass->Bake();
			}

			{
				...
				lineOnTopSpec.Pipeline = RasterPipeline::Create(pipelineSpecification);
				m_LineOnTopPass = RenderPass::Create(lineOnTopSpec);
				m_LineOnTopPass->SetInput("u_Camera", m_UBSCamera);
				BEY_CORE_VERIFY(m_LineOnTopPass->Validate());
			...
			for (uint32_t i = 0; i < framesInFlight; i++)
			{
				...
				m_LineVertexBuffers[0][i] = VertexBuffer::Create(allocationSize, "2D Line");
				m_MemoryStats.TotalAllocated += allocationSize;
...

			{
				...
				m_LineIndexBuffer = IndexBuffer::Create(lineIndices, "2D Line", allocationSize);
				m_MemoryStats.TotalAllocated += allocationSize;
			}
			hdelete[] lineIndices;
		}
		...
		// Text
		{
			...
			textSpec.Pipeline = RasterPipeline::Create(pipelineSpecification);
			m_TextPass = RenderPass::Create(textSpec);
			m_TextPass->SetInput("u_Camera", m_UBSCamera);
			BEY_CORE_VERIFY(m_TextPass->Validate());
			m_TextPass->Bake();
			...
			for (uint32_t i = 0; i < framesInFlight; i++)
			{
				...
				m_TextVertexBuffers[0][i] = VertexBuffer::Create(allocationSize, "2D Text");
				m_MemoryStats.TotalAllocated += allocationSize;
			...
			{
				uint64_t allocationSize = c_MaxIndices * sizeof(uint32_t);
				m_TextIndexBuffer = IndexBuffer::Create(textQuadIndices, "2D Text", allocationSize);
				m_MemoryStats.TotalAllocated += allocationSize;
			}
		...
		// Circles
		{
			...
			m_CirclePipeline = RasterPipeline::Create(pipelineSpecification);
			...
			{
				uint64_t allocationSize = c_MaxVertices * sizeof(QuadVertex);
				m_CircleVertexBuffers[0][i] = VertexBuffer::Create(allocationSize, "2D Circle");
				m_MemoryStats.TotalAllocated += allocationSize;
	...
	void Renderer2D::BeginScene(const glm::mat4& viewProj, const glm::mat4& view, bool depthTest)
	{
		...
		BEY_CORE_TRACE_TAG("Renderer", "Renderer2D::BeginScene frame {}", frameIndex);
	...
	void Renderer2D::EndScene()
	{
		...
		BEY_CORE_TRACE_TAG("Renderer", "Renderer2D::EndScene frame {}", frameIndex);
		uint32_t dataSize = 0;
		...
		for (uint32_t i = 0; i <= m_CircleBufferWriteIndex; i++)
		{
			...
			if (dataSize)
			{
				...
				Renderer::BeginRenderPass(m_RenderCommandBuffer, m_LinePass);
				Renderer::RenderGeometry(m_RenderCommandBuffer, m_LinePass->GetSpecification().RasterPipeline, m_LineMaterial, m_LineVertexBuffers[i][frameIndex], m_LineIndexBuffer, glm::mat4(1.0f), indexCount);
				Renderer::EndRenderPass(m_RenderCommandBuffer);
	...
	void Renderer2D::AddQuadBuffer()
	{
		...
		{
			uint64_t allocationSize = c_MaxVertices * sizeof(QuadVertex);
			newVertexBuffer[i] = VertexBuffer::Create(allocationSize, "2D Quad");
			m_MemoryStats.TotalAllocated += allocationSize;
	...
	void Renderer2D::AddLineBuffer()
	{
		...
		{
			uint64_t allocationSize = c_MaxLineVertices * sizeof(LineVertex);
			newVertexBuffer[i] = VertexBuffer::Create(allocationSize, "2D Line");
			m_MemoryStats.TotalAllocated += allocationSize;
	...
	void Renderer2D::AddTextBuffer()
	{
		...
		{
			uint64_t allocationSize = c_MaxVertices * sizeof(TextVertex);
			newVertexBuffer[i] = VertexBuffer::Create(allocationSize, "2D Text");
			m_MemoryStats.TotalAllocated += allocationSize;
	...
	void Renderer2D::AddCircleBuffer()
	{
		...
		for (uint32_t i = 0; i < framesInFlight; i++)
		{
			...
			newVertexBuffer[i] = VertexBuffer::Create(allocationSize, "2D Circle");
			m_MemoryStats.TotalAllocated += allocationSize;
	...
	std::u32string utf8_to_utf32(const std::string& utf8)
	{
		...
		simdutf::convert_utf8_to_utf32(utf8.c_str(), utf8.size(), utf32.data());
		return utf32;
	}
	...
	void Renderer2D::DrawString(const std::string& string, const Ref<Font>& font, const glm::mat4& transform, float maxWidth, const glm::vec4& color, float lineHeightOffset, float kerningOffset)
	{
		...
		std::u32string utf32string = utf8_to_utf32(string);

		Ref<Texture2D> fontAtlas = font->GetFontAtlas();
		BEY_CORE_ASSERT(fontAtlas);
