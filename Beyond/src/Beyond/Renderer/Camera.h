...
namespace Beyond
{
	...
	public:
		...
		{
			m_ProjectionMatrix = glm::perspectiveFov(radFov, width, height, farP, nearP);  // NOLINT(readability-suspicious-call-argument)
		...
		void SetOrthoProjectionMatrix(const float width, const float height, const float nearP, const float farP)
		{
			...
			m_ProjectionMatrix = glm::ortho(-width * 0.5f, width * 0.5f, -height * 0.5f, height * 0.5f, farP, nearP);  // NOLINT(readability-suspicious-call-argument)
