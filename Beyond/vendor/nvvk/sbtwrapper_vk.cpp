...
{
	Beyond::Renderer::SubmitResourceFree([buffers = m_buffer]() mutable
	{
		Beyond::VulkanAllocator allocator("SBT de-allocator");
		for (auto& b : buffers)
			if (b.buffer != VK_NULL_HANDLE)
				allocator.DestroyBuffer(b.buffer, b.memHandle);
	});
						...
						const std::vector<VkRayTracingPipelineCreateInfoKHR>& librariesInfo /*= {}*/)
{
	...
	VK_CHECK_RESULT(vkGetRayTracingShaderGroupHandlesKHR(m_device, rtPipeline, 0, totalGroupCount, sbtSize, shaderHandleStorage.data()));
	...
	auto findStride = [&](auto entry, auto& stride) {
		...
		for (auto& e : entry)
		{
			...
			uint32_t dataHandleSize = align_up(static_cast<uint32_t>(m_handleSize + e.second.size() * sizeof(uint8_t)), m_handleAlignment);
	...
	Beyond::VulkanAllocator allocator("SBT allocator");
	VkCommandBuffer cmdBuf = Beyond::VulkanContext::GetCurrentDevice()->CreateCommandBuffer(fmt::eastl_format("SBT creation"), true);
	for (uint32_t i = 0; i < 4; i++)
	{
		if (!stage[i].empty())
		{
			bufferCreateInfo.size = stage[i].size() * sizeof(stage[i][0]);
			m_buffer[i].memHandle = allocator.AllocateBuffer(bufferCreateInfo, VMA_MEMORY_USAGE_CPU_TO_GPU, m_buffer[i].buffer);
			auto* mappedData = allocator.MapMemory<uint8_t>(m_buffer[i].memHandle);
			std::memcpy(mappedData, stage[i].data(), stage[i].size() * sizeof stage[0][0]);
			allocator.UnmapMemory(m_buffer[i].memHandle);
			Beyond::VKUtils::SetDebugUtilsObjectName(m_device, VK_OBJECT_TYPE_BUFFER, fmt::eastl_format("SBTWrapper::m_buffer[{}].buffer", i), m_buffer[i].buffer);
		}
	}
	Beyond::VulkanContext::GetCurrentDevice()->FlushCommandBuffer(cmdBuf);
