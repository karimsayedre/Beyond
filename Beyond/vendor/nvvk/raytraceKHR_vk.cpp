...
#include "Beyond/Platform/Vulkan/VulkanBottomLevelGeometry.h"

//--------------------------------------------------------------------------------------------------
  // Initializing the allocator and querying the raytracing properties
  //
void nvvk::RaytracingBuilderKHR::setup(const VkDevice& device, Beyond::VulkanAllocator allocator, Beyond::Ref<Beyond::AccelerationStructureSet> asSet)
{
	m_AccelerationStructureSet = asSet;
	m_device = device;
	m_alloc = allocator;
}
...
{
	uint32_t frameIndex = Beyond::Renderer::RT_GetCurrentFrameIndex();

	//m_AccelerationStructureSet->RT_Release(frameIndex);
}

//--------------------------------------------------------------------------------------------------
// Returning the constructed top-level acceleration structure
//
VkAccelerationStructureKHR nvvk::RaytracingBuilderKHR::getAccelerationStructure() 
{
	uint32_t frameIndex = Beyond::Renderer::RT_GetCurrentFrameIndex();
	return m_AccelerationStructureSet->Get(frameIndex).As<Beyond::VulkanAccelerationStructure>()->GetTLAS().accel;
}

VkDeviceAddress nvvk::RaytracingBuilderKHR::getBlasDeviceAddress(uint32_t blasId)
{
	uint32_t frameIndex = Beyond::Renderer::RT_GetCurrentFrameIndex();
	BEY_CORE_ASSERT(size_t(blasId) < m_AccelerationStructureSet->Get(frameIndex).As<Beyond::VulkanAccelerationStructure>()->GetBLASes()->GetBuildInfo().size());

	auto blas = m_AccelerationStructureSet->Get(frameIndex).As<Beyond::VulkanAccelerationStructure>()->GetBLASes()->GetBuildInfo()[blasId].as.accel;
...
//
void nvvk::RaytracingBuilderKHR::RT_buildBlas(VkCommandBuffer commandBuffer, const std::vector<BlasInput>& input, VkBuildAccelerationStructureFlagsKHR flags)
{
	...
	nvvk::Buffer scratchBuffer;
	{
		...
		scratchBuffer.memHandle = m_alloc.AllocateBuffer(bci, VMA_MEMORY_USAGE_GPU_ONLY, scratchBuffer.buffer);
		Beyond::VKUtils::SetDebugUtilsObjectName(m_device, VK_OBJECT_TYPE_BUFFER, "Blas scratch buffer", scratchBuffer.buffer);
	}
	...
	for (uint32_t idx = 0; idx < nbBlas; idx++)
	{
		...
		if (batchSize >= batchLimit || idx == nbBlas - 1)
		{
			...
			device->FlushCommandBuffer(cmdBuf);  // Submit command buffer and call vkQueueWaitIdle
	...
	if (queryPool)
	{
		...
		BEY_CORE_TRACE("RT BLAS: reducing from: {} to: {} = {} ({}% smaller).", asTotalSize, compactSize, asTotalSize - compactSize, fractionSmaller * 100.f);
	}

	//uint32_t frameIndex = Beyond::Renderer::RT_GetCurrentFrameIndex();
	//auto& blas = m_AccelerationStructureSet->Get(frameIndex).As<Beyond::VulkanAccelerationStructure>()->GetBLASes();
...
// The array of BuildAccelerationStructure was created in RT_buildBlas and the vector of
											   ...
											   VkQueryPool                              queryPool)
{
	...
	for (const auto& idx : indices)
	{
		...
		bufferInfo.size = buildAs[idx].sizeInfo.accelerationStructureSize;
		bufferInfo.usage = VK_BUFFER_USAGE_ACCELERATION_STRUCTURE_STORAGE_BIT_KHR | VK_BUFFER_USAGE_SHADER_DEVICE_ADDRESS_BIT;
		bufferInfo.sharingMode = VK_SHARING_MODE_EXCLUSIVE;
		buildAs[idx].as.buffer.memHandle = m_alloc.AllocateBuffer(bufferInfo, VMA_MEMORY_USAGE_CPU_TO_GPU, buildAs[idx].as.buffer.buffer);
		Beyond::VKUtils::SetDebugUtilsObjectName(m_device, VK_OBJECT_TYPE_BUFFER, fmt::format("Blas Buffer Idx: {}", idx), buildAs[idx].as.buffer.buffer);
		...
		vkCreateAccelerationStructureKHR(m_device, &createInfo, nullptr, &buildAs[idx].as.accel);

		Beyond::VKUtils::SetDebugUtilsObjectName(m_device, VK_OBJECT_TYPE_ACCELERATION_STRUCTURE_KHR, fmt::format("Blas Idx: {}", idx), buildAs[idx].as.accel);
												...
												VkQueryPool                              queryPool)
{
	...
	for (auto idx : indices)
	{
		...
		bufferInfo.sharingMode = VK_SHARING_MODE_EXCLUSIVE;
		buildAs[idx].as.buffer.memHandle = m_alloc.AllocateBuffer(bufferInfo, VMA_MEMORY_USAGE_CPU_TO_GPU, buildAs[idx].as.buffer.buffer);
		Beyond::VKUtils::SetDebugUtilsObjectName(m_device, VK_OBJECT_TYPE_BUFFER, fmt::format("Compact Blas Buffer Idx: {}", idx), buildAs[idx].as.buffer.buffer);
		...
		Beyond::VKUtils::SetDebugUtilsObjectName(m_device, VK_OBJECT_TYPE_ACCELERATION_STRUCTURE_KHR, fmt::format("Compact Blas Idx: {}", idx), buildAs[idx].as.accel);
...
void nvvk::RaytracingBuilderKHR::destroyNonCompacted(std::vector<uint32_t> indices, std::vector<BuildAccelerationStructure>& buildAs)
{
	for (auto& i : indices)
	{
		m_alloc.DestroyAS(buildAs[i].cleanupAS);
	}
}
...
void nvvk::RaytracingBuilderKHR::buildTlas(
	...
	const std::vector<VkAccelerationStructureInstanceKHR>& instances,
										   ...
										   bool                                 update /*= false*/)
{
	buildTlas(commandBuffer, instances, flags, update, false);
}
...
void nvvk::RaytracingBuilderKHR::buildTlas(
	...
	const std::vector<VkAccelerationStructureMotionInstanceNV>& instances,
										   ...
										   bool                                 update /*= false*/)
{
	buildTlas(commandBuffer, instances, flags, update, true);
}
#endif
											   ...
											   bool                                 motion)
{
	...
	//for (uint32_t frame = 0; frame < Beyond::Renderer::GetConfig().FramesInFlight; frame++)
	{
		...
		if (update == false)
		{
			...
			bufferInfo.usage = VK_BUFFER_USAGE_ACCELERATION_STRUCTURE_STORAGE_BIT_KHR | VK_BUFFER_USAGE_SHADER_DEVICE_ADDRESS_BIT;
			bufferInfo.sharingMode = VK_SHARING_MODE_EXCLUSIVE;
			tlas.buffer.memHandle = m_alloc.AllocateBuffer(bufferInfo, VMA_MEMORY_USAGE_CPU_TO_GPU, tlas.buffer.buffer);
			Beyond::VKUtils::SetDebugUtilsObjectName(m_device, VK_OBJECT_TYPE_BUFFER, fmt::format("Tlas Buffer"), tlas.buffer.buffer);

			createInfo.buffer = tlas.buffer.buffer;
			vkCreateAccelerationStructureKHR(m_device, &createInfo, nullptr, &tlas.accel);

			Beyond::VKUtils::SetDebugUtilsObjectName(m_device, VK_OBJECT_TYPE_ACCELERATION_STRUCTURE_KHR, fmt::format("Tlas"), tlas.accel);
			...
			as->SetDescriptorInfo(descASInfo);
		...
		// Allocate the scratch memory
		{
			...
			bci.usage = VK_BUFFER_USAGE_STORAGE_BUFFER_BIT | VK_BUFFER_USAGE_SHADER_DEVICE_ADDRESS_BIT | VK_BUFFER_USAGE_ACCELERATION_STRUCTURE_BUILD_INPUT_READ_ONLY_BIT_KHR;
			scratchBuffer.memHandle = m_alloc.AllocateBuffer(bci, VMA_MEMORY_USAGE_GPU_ONLY, scratchBuffer.buffer);
			Beyond::VKUtils::SetDebugUtilsObjectName(m_device, VK_OBJECT_TYPE_BUFFER, "Tlas scratch buffer", scratchBuffer.buffer);
		}
...
//
void nvvk::RaytracingBuilderKHR::updateBlas(VkCommandBuffer commandBuffer, uint32_t blasIdx, BlasInput& blas, VkBuildAccelerationStructureFlagsKHR flags)
{
	//uint32_t frameIndex = Beyond::Renderer::RT_GetCurrentFrameIndex();
	//auto as = m_AccelerationStructureSet->Get(frameIndex).As<Beyond::VulkanAccelerationStructure>();
	//assert(size_t(blasIdx) < as->GetBLASes().size());
	...
	//buildInfos.srcAccelerationStructure = as->GetBLASes()[blasIdx].accel;  // UPDATE
	//buildInfos.dstAccelerationStructure = as->GetBLASes()[blasIdx].accel;
	...
	//Beyond::VKUtils::SetDebugUtilsObjectName(m_device, VK_OBJECT_TYPE_BUFFER, "Update scratch buffer", scratchBuffer.buffer);
