...
namespace rtxgi
{
    ...
    struct DDGIVolumeDesc
    {
        ...
        bool            ClearProbes = false;                    // Note(Karim): My change.
    ...
    public:
		...
		void SetDesc(const DDGIVolumeDesc& desc) { m_desc = desc; }
        ...
        DDGIVolumeDesc& GetDesc() { return m_desc; } // Note(Karim): My change.

        DDGIVolumeDescGPU GetDescGPU() const;

        DDGIVolumeDescGPUPacked GetDescGPUPacked() const;

        const eastl::string GetName() const { return eastl::string(m_desc.name.c_str(), m_desc.name.size()); }
