 ...
 */
void AddPerfMarker(VkCommandBuffer cmdBuffer, uint8_t r, uint8_t g, uint8_t b, eastl::string name)
{
...
namespace rtxgi
{
    namespace vulkan
    {
        ...
        ERTXGIStatus UpdateDDGIVolumeProbes(VkCommandBuffer cmdBuffer, uint32_t numVolumes, DDGIVolume** volumes)
        {
            ...
            for (volumeIndex = 0; volumeIndex < numVolumes; volumeIndex++)
            {
                ...
                // Probe irradiance blending
                {
                    ...
                    {
                        eastl::string msg = "Irradiance, DDGIVolume[" + eastl::to_string(volume->GetIndex()) + "] - \"" + volume->GetName() + "\"";
                        AddPerfMarker(cmdBuffer, RTXGI_PERF_MARKER_GREEN, msg);
                    }
            ...
            for (volumeIndex = 0; volumeIndex < numVolumes; volumeIndex++)
            {
                ...
                // Probe distance blending
                {
                    if (bInsertPerfMarkers && volume->GetInsertPerfMarkers())
                    {
                        eastl::string msg = "Distance, DDGIVolume[" + eastl::to_string(volume->GetIndex()) + "] - \"" + volume->GetName() + "\"";
        ...
        ERTXGIStatus CalculateDDGIVolumeVariability(VkCommandBuffer cmdBuffer, uint32_t numVolumes, DDGIVolume** volumes)
        {
            ...
            for (volumeIndex = 0; volumeIndex < numVolumes; volumeIndex++)
            {
                ...
                // First pass reduction takes probe irradiance data and calculates variability, reduces as much as possible
                {
                    if (bInsertPerfMarkers && volume->GetInsertPerfMarkers())
                    {
                        eastl::string msg = "Reduction, DDGIVolume[" + eastl::to_string(volume->GetIndex()) + "] - \"" + volume->GetName() + "\"";
                        AddPerfMarker(cmdBuffer, RTXGI_PERF_MARKER_GREEN, msg.c_str());
                    }
                ...
                while (inputTexelsX > 1 || inputTexelsY > 1 || inputTexelsZ > 1)
                {
                    if (bInsertPerfMarkers && volume->GetInsertPerfMarkers())
                    {
                        eastl::string msg = "Extra Reduction, DDGIVolume[" + eastl::to_string(volume->GetIndex()) + "] - \"" + volume->GetName() + "\"";
                        AddPerfMarker(cmdBuffer, RTXGI_PERF_MARKER_GREEN, msg.c_str());
                    }
        ...
        ERTXGIStatus DDGIVolume::CreateManagedResources(const DDGIVolumeDesc& desc, const DDGIVolumeManagedResourcesDesc& managed, bool updateShaders)
        {
            bool deviceChanged = IsDeviceChanged(managed);

            // Create the descriptor set layout, pipeline layout, and pipelines
            if (deviceChanged || updateShaders)
            {
...
# if 1
    ...
    #if RTXGI_DDGI_RESOURCE_MANAGEMENT
        ERTXGIStatus DDGIVolume::Create(VkCommandBuffer cmdBuffer, bool updateShaders, const DDGIVolumeDesc& desc, const DDGIVolumeResources& resources)
    #else
        ...
        #if RTXGI_DDGI_RESOURCE_MANAGEMENT
            result = CreateManagedResources(desc, resources.managed, updateShaders);
            if (result != ERTXGIStatus::OK) return result;
        #else
        ...
        bool DDGIVolume::CreateDescriptorSet()
        {
            ...
            if (VKFAILED(result)) return false;
        #ifdef RTXGI_GFX_NAME_OBJECTS
            eastl::string name = "DDGIVolume[" + eastl::to_string(m_desc.index) + "], Descriptor Set";
        ...
        bool DDGIVolume::CreateLayouts()
        {
            ...
            if (VKFAILED(result)) return false;
        #ifdef RTXGI_GFX_NAME_OBJECTS
            eastl::string name = "DDGIVolume[" + eastl::to_string(m_desc.index) + "] Descriptor Set Layout";
        ...
        #ifdef RTXGI_GFX_NAME_OBJECTS
            name = "DDGIVolume[" + eastl::to_string(m_desc.index) + "] Pipeline Layout";
        ...
        {
            if (eastl::string(entryPoint).compare("") == 0) return false;
            ...
            if (VKFAILED(result)) return false;
        #ifdef RTXGI_GFX_NAME_OBJECTS
            eastl::string name = "DDGIVolume[" + eastl::to_string(m_desc.index) + "]," + debugName + " Shader Module";
            ...
            if (VKFAILED(result)) return false;
        #ifdef RTXGI_GFX_NAME_OBJECTS
            name = "DDGIVolume[" + eastl::to_string(m_desc.index) + "]," + debugName + " Pipeline";
        ...
        bool DDGIVolume::CreateProbeRayData(const DDGIVolumeDesc& desc)
        {
            ...
            if (!result) return false;
        #ifdef RTXGI_GFX_NAME_OBJECTS
            eastl::string name = "DDGIVolume[" + eastl::to_string(desc.index) + "], Probe Ray Data";
            eastl::string memory = name + " Memory";
            eastl::string view = name + " View";
        ...
        bool DDGIVolume::CreateProbeIrradiance(const DDGIVolumeDesc& desc)
        {
            ...
            if (!result) return false;
        #ifdef RTXGI_GFX_NAME_OBJECTS
            eastl::string name = "DDGIVolume[" + eastl::to_string(desc.index) + "], Probe Irradiance";
            eastl::string memory = name + " Memory";
            eastl::string view = name + " View";
        ...
        bool DDGIVolume::CreateProbeDistance(const DDGIVolumeDesc& desc)
        {
            ...
            if (!result) return false;
        #ifdef RTXGI_GFX_NAME_OBJECTS
            eastl::string name = "DDGIVolume[" + eastl::to_string(desc.index) + "], Probe Distance";
            eastl::string memory = name + " Memory";
            eastl::string view = name + " View";
        ...
        bool DDGIVolume::CreateProbeData(const DDGIVolumeDesc& desc)
        {
            ...
            if (!result) return false;
        #ifdef RTXGI_GFX_NAME_OBJECTS
            eastl::string name = "DDGIVolume[" + eastl::to_string(desc.index) + "], Probe Data";
            eastl::string memory = name + " Memory";
            eastl::string view = name + " View";
        ...
        bool DDGIVolume::CreateProbeVariability(const DDGIVolumeDesc& desc)
        {
            ...
            if (!result) return false;
        #ifdef RTXGI_GFX_NAME_OBJECTS
            eastl::string name = "DDGIVolume[" + eastl::to_string(desc.index) + "], Probe Variability";
            eastl::string memory = name + " Memory";
            eastl::string view = name + " View";
        ...
        bool DDGIVolume::CreateProbeVariabilityAverage(const DDGIVolumeDesc& desc)
        {
            ...
            if (!result) return false;
        #ifdef RTXGI_GFX_NAME_OBJECTS
            eastl::string name = "DDGIVolume[" + eastl::to_string(desc.index) + "], Probe Variability Average";
            eastl::string memory = name + " Memory";
            eastl::string view = name + " View";
        ...
        #ifdef RTXGI_GFX_NAME_OBJECTS
            name = "DDGIVolume[" + eastl::to_string(desc.index) + "], Probe Variability Readback";
            memory = name + " Memory";
