...
{
	// Standard PBR inputs
	vec4 albedoTexColor = texture(GetTex(u_MaterialUniforms.AlbedoTexIndex), Input.TexCoord);
	m_Params.Albedo = albedoTexColor * ToLinear(u_MaterialUniforms.AlbedoColor);
	float alpha = albedoTexColor.a;
	m_Params.Metalness = 0.0f;
	m_Params.Roughness = 0.0f;
	m_Params.Roughness = max(m_Params.Roughness.x, 0.05); // Minimum roughness of 0.05 to keep specular highlight
	...
	color = vec4(m_Params.Albedo.rgb, u_MaterialUniforms.AlbedoColor.a);
