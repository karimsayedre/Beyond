...
#define GTAO_REFLECTION_OCCLUSION (__BEY_REFLECTION_OCCLUSION_METHOD & BEY_REFLECTION_OCCLUSION_METHOD_GTAO)
#define HBAO_REFLECTION_OCCLUSION (__BEY_REFLECTION_OCCLUSION_METHOD & BEY_REFLECTION_OCCLUSION_METHOD_HBAO)
...
bool AdvanceRay(vec3 origin, vec3 direction, vec3 invDirection, vec2 currentMipPosition, vec2 currentMipResolutionInv, vec2 floorOffset, vec2 uvOffset, float surfaceZ, inout vec3 position,
...
float ValidateHit(vec3 origin, vec3 ray, vec3 originPosVS, vec3 rayDirVS, float roughness)
{
	...
	if (any(lessThan(ray.xy, vec2(0.0))) || any(greaterThan(ray.xy, vec2(1.0)))) return 0;
	...
	if (all(lessThan(manhattanDist, u_ScreenData.InvHalfResolution))) return 0;
...
void main()
{
	...
	const vec3 normalVS = texture(u_Normal, uv).xyz * 2.0 - 1.0;
	...
	imageStore(outColor, ivec2(base), vec4(roughnessFactor * totalColor.rgb, max(confidence - 0.5, -0.02)));
}
