...
void main()
{
	...
	TexIndex = a_TexIndex;
	gl_Position = u_Camera.u_ViewProjection * u_Renderer.Transform * vec4(a_Position, 1.0);
}
...
void main()
{
	vec4 bgColor = vec4(Input.Color.rgb, 0.0); // TODO: outlines
	vec4 fgColor = Input.Color;

	// NOTE: MSDF texture has no mips (only LOD 0), but in the future it might
