...
Payload TraceRay(RayDesc ray, uint bounceIndex)
{
#if BEY_SER
    payload = (Payload)0;
...
float3 TracePath(inout RayDesc ray, inout uint seed)
{
    ...
    for (int bounceIndex = 0; bounceIndex < MAX_BOUNCES; ++bounceIndex)
    {
        ...
        { // Reflection: Calculate the half-vector and reflected direction
            ...
            // bsdf = EvaluateBRDF(view, ray.Direction, material.Normal, H, material.roughness, material.metallic,
            // material.Albedo.rgb, F, isSpecular); float alpha = material.roughness * material.roughness;
...
[shader("anyhit")]
void main(inout Payload packedPayload, BuiltInTriangleIntersectionAttributes attrib)
{
    uint2 launchIndex = DispatchRaysIndex().xy;
    uint2 launchDimensions = DispatchRaysDimensions().xy;
...
[shader("miss")]
void main(inout Payload packedPayload)
{
