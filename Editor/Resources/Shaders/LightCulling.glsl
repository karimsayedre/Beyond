// ---------------------------------------
// -- Beyond Engine light culling shader --
// ---------------------------------------
//
// References:
...
#version 450 core
#pragma stage : comp
#define BEY_MEMORY_ACESS writeonly
...
void main()
{
	...
	for (uint i = 0; i < passCount; i++)
	{
		...
		if (lightIndex >= u_PointLights.LightCount) break;
	...
	for (uint i = 0; i < passCount; i++)
	{
		// Get the lightIndex to test for this thread / pass. If the index is >= light count, then this thread can stop testing lights
		uint lightIndex = i * threadCount + gl_LocalInvocationIndex;
		if (lightIndex >= u_SpotLights.LightCount) break;
	...
	if (gl_LocalInvocationIndex == 0)
	{
		...
		//debugPrintfEXT("asdfasdfasdfasdfasdfergioerhjtiuehtiuehtvvvvg%u", visiblePointLightCount);
		//debugPrintfEXT("asdfasdfasdfasdfasdfasdfasdfasdfasdf: %u", visibleSpotLightCount);
