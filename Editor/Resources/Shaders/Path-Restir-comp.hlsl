...
PbrMaterial TraceRay(RaytracingAccelerationStructure TLAS, RayDesc ray, uint bounceIndex, inout uint seed)
{
	...
	while (rayQuery.Proceed())
	{
		...
		{
			if (!AnyHitRayQuery(rayQuery, seed)) continue;
	...
	{
		return GetMaterialParams(ray, rayQuery.CommittedObjectToWorld3x4(), rayQuery.CommittedTriangleBarycentrics(), rayQuery.CommittedRayT(), rayQuery.CommittedInstanceID(), rayQuery.CommittedPrimitiveIndex(), launchIndex);
	}
	else 
	{
...
float3 TracePath(inout RayDesc ray, inout uint seed)
{
	...
	[loop] for (int bounceIndex = 0; bounceIndex < MAX_BOUNCES; ++bounceIndex)
	{
		...
		PbrMaterial payload = TraceRay(TLAS, ray, bounceIndex, seed);

		const float3 view = -ray.Direction;

		isInside = dot(payload.Ng, view) < 0.0;

		[branch] if (bounceIndex == 0)
		{
			o_ViewNormalsLuminance[launchIndex] = float4(mul((float3x3)(u_Camera.ViewMatrix), normalize(payload.N)) * 0.5 + 0.5, 1.0);
			o_MetalnessRoughness[launchIndex] = float4(payload.metallic, payload.roughness.r, 0.0, 1.0);
			o_AlbedoColor[launchIndex] = float4(payload.baseColor, payload.opacity);
		}
		o_PrimaryHitT[launchIndex] = float4(payload.HitT, 0.0, 0.0, 0.0);

		float NdotV = max(dot(payload.N, view), 0.0);
		float3 F0 = lerp(Fdielectric.xxx, payload.baseColor.rgb, payload.metallic);

		// Miss, exit loop
		[branch] if (payload.HitT < 0.f)
		{
			color += EnvironmentSample(F0, ray.Direction, bounceIndex, payload) * throughput;
			DebugImage[launchIndex] = float4(color, 1.0);
			return color;
		}

		bool thin_walled = (payload.thickness == 0);
		if (isInside && !thin_walled)
		{
			const float3 abs_coeff = AbsorptionCoefficient(payload.attenuationDistance, payload.attenuationColor);
		...
		color += payload.emissive * throughput;
		color += DirectLighting(seed, F0, payload, view, 1, TLAS) * throughput;
		...
		throughput *= min(payload.baseColor.rgb, 0.9.xxx);
		...
		float3 offsetDir = dot(ray.Direction, payload.Ng) > 0 ? payload.Ng : -payload.Ng;
		ray.Origin = offsetRay(payload.WorldPosition, offsetDir);
		// ray.Origin = payload.WorldPosition + ray.Direction * ray.TMin;
	}
	return color;
}
