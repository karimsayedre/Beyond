...
// [[vk::binding(9, 2)]] RWTexture2D<float4> DebugImage;
...
VK_BINDING(2, 2) RWTexture2D<float4>                         GBufferA;
    ...
    out float3 sampleDirection)
{
    ...
    uint type = RTXGI_DDGI_VISUALIZE_PROBE_IRRADIANCE; // TODO: Should get it from the push constant?
...
void main()
{
    ...
    uint2 LaunchDimensions = DispatchRaysDimensions().xy;
                    #ifdef HGSDF
    // DebugImage[DispatchRaysIndex().xy] = float4(0.0.xxx, 1.0);
#endif
    ...
    if (payload.hitT > 0.f)
    {
        ...
        float depth = GBufferB[LaunchIndex].r;

        float linearDepth = LinearizeDepth(depth);

        if(linearDepth < 0.f || payload.hitT < linearDepth)
        {
...
void main()
{
    ...
    RaytracingAccelerationStructure DDGIProbeVisTLAS = TLAS(DDGIPROBEVIS_TLAS_INDEX);
