...
[shader("raygeneration")] void main()
{
	...
	{
		// Store the ray miss
		//float3 skyColor = u_SkyboxTexture.SampleLevel(GetBilinearWrapSampler(), payload.shadingNormal, u_SceneData.EnvironmentMapIntensity).rgb * u_SceneData.EnvironmentMapLod;
		DDGIStoreProbeRayMiss(RayData, outputCoords, volume, /*skyColor*/ 0.0f); // TODO: Fix this.
		return;
	}
	...
	Payload rayPayload = {float4(payload.albedo, payload.opacity), 0.0, payload.worldPosition, payload.metallic, payload.normal, payload.roughness, payload.hitT, 1.0, 0};
	float3 diffuse = DirectLighting(asuint(probeRayDirection.x), F0, rayPayload, view, NdotV, 2, SceneTLAS); // TODO: NdotD??
	...
	float3 radiance = diffuse + ((min(payload.albedo, float3(maxAlbedo, maxAlbedo, maxAlbedo)) / RTXGI_PI) * irradiance);
...
#include <rtxgi/ddgi/DDGIVolumeDescGPU.h>
        ...
        [shader("closesthit")] void main(inout DDGIPackedPayload packedPayload, BuiltInTriangleIntersectionAttributes attrib)
{
	...
	if (material.AlbedoTexIndex > 4)
	{
		...
		float4 bco = GetTex2D(material.AlbedoTexIndex).SampleLevel(GetBilinearWrapSampler(), vertex.TexCoord, numLevels / 2.f);
	...
	if (material.NormalTexIndex > 4)
	{
		...
		payload.shadingNormal = GetTex2D(material.NormalTexIndex).SampleLevel(GetBilinearWrapSampler(), vertex.TexCoord, numLevels / 2.f).xyz;
...
// ---[ Miss Shader ]---
[shader("miss")] void main(inout DDGIPackedPayload packedPayload) { packedPayload.hitT = -1.f; }