...
function linkAppReferences(linkScriptCore)
	local beyondDir = os.getenv("BEYOND_DIR")
	local monoLibsPath
    local monoLibsFacadesPath
	local is_windows = os.istarget('windows')

	if is_windows then
		monoLibsPath = path.join(beyondDir, "Editor", "mono", "lib", "mono", "4.5"):gsub("/", "\\")
		monoLibsFacadesPath = path.join(monoLibsPath, "Facades"):gsub("/", "\\")
	else
		monoLibsPath = path.join(beyondDir, "Editor", "mono", "linux", "lib", "mono", "4.5")
