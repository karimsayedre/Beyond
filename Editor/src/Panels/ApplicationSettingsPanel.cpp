...
namespace Beyond {
	...
	bool ApplicationSettingsPanel::OnImGuiRender(bool& isOpen)
	{
		bool changed = false;
		if (!isOpen)
			return changed;
		...
		if (ImGui::Begin("Application Settings", &isOpen))
		{
				...
				ImGui::GetContentRegionAvail().y - 8.0f)))
			{
				...
				{
					BEY_CORE_ERROR_TAG("ApplicationSettingsPanel", "Invalid page selected!");
				}
