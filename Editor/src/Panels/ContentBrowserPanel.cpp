...
namespace Beyond {
	...
	bool ContentBrowserPanel::OnImGuiRender(bool& isOpen)
	{
		bool changed = false;
				...
				if (ImGui::BeginDragDropTarget())
				{
					...
					if (data)
					{
						...
						for (size_t i = 0; i < count; i++)
						{
							...
							if (!entity)
							{
								BEY_CORE_ERROR_TAG("ContentBrowser", "Failed to find Entity with ID {0} in current Scene context!", entityID);
								continue;
							}
	...
	void ContentBrowserPanel::PasteCopiedAssets()
	{
		...
		for (AssetHandle copiedAsset : m_CopiedAssets)
		{
			...
			if (item->GetType() == ContentBrowserItem::ItemType::Asset)
			{
				...
				BEY_CORE_VERIFY(!std::filesystem::exists(filepath));
