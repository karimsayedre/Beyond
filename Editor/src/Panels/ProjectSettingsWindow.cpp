...
#include "Beyond/Audio/DSP/Reverb/Reverb.h"
...
namespace Beyond {
	...
	bool ProjectSettingsWindow::OnImGuiRender(bool& isOpen)
	{
		bool changed = false;
	...
	void ProjectSettingsWindow::RenderPhysicsSettings()
	{
		...
		if (UI::PropertyGridHeader("Physics", false))
		{
...

			{
				...
				{
					int counter = 0;
					auto checkID = [&counter](auto checkID) -> eastl::string
					{
						++counter;
						const eastl::string counterStr = [&counter] {
							if (counter < 10)
								return "0" + fmt::to_eastl_string(counter);
							else
								return eastl::to_string(counter);
						}();

						eastl::string idstr = "NewLayer_" + counterStr;
				...
				{
					eastl::string name = GetUniqueName();
