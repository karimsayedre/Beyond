...
#include "Beyond/Asset/AnimationAssetSerializer.h"
#include "Beyond/Audio/AudioEngine.h"
#include "Beyond/Audio/AudioEvents/AudioCommandRegistry.h"
#include "Beyond/Audio/Editor/AudioEventsEditor.h"

#include "Beyond/Core/Events/EditorEvents.h"

#include "Beyond/Editor/AssetEditorPanel.h"
#include "Beyond/Editor/EditorApplicationSettings.h"
#include "Beyond/Editor/NodeGraphEditor/AnimationGraph/AnimationGraphAsset.h"
#include "Beyond/Editor/NodeGraphEditor/AnimationGraph/AnimationGraphNodeEditorModel.h"
#include "Beyond/Editor/ScriptEngineDebugPanel.h"
...
#include "Beyond/Project/ProjectSerializer.h"
...
#define PHYSICS_CAPTURES_PANEL_ID "PhysicsCapturesPanel"
	...
	void EditorLayer::OnAttach()
	{
		...
		// Should we ever want to actually show editor layer panels in Beyond::Runtime
		...
		if (!m_UserPreferences->StartupProject.empty())
			...
			BEY_CORE_VERIFY(false, "No project provided!");
	...
	void EditorLayer::BuildShaderPack()
	{
		...
		auto path = std::filesystem::absolute("Resources/ShaderPack.hsp").string();
		std::string formattedMessage = fmt::format("Successfully created shader pack. File Path: {}", path);
		const char* view = "Successfully created shader pack. File Path: {0}";
		UI::ShowSimpleMessageBox<BEY_MESSAGE_BOX_OK_BUTTON>(
			"Created ShaderPack!",
			fmt::runtime(view),
			formattedMessage
		);
	...
	void EditorLayer::BuildAssetPack()
	{
		...
		BEY_CONSOLE_LOG_INFO("Building Asset Pack...");
	...
	void EditorLayer::OnSceneTransition(AssetHandle scene)
	{
		...
		{
			BEY_CORE_ERROR("Could not deserialize scene {0}", scene);
		}
	}

	void EditorLayer::UpdateWindowTitle(const eastl::string& sceneName)
	{
		BEY_CORE_VERIFY(RendererAPI::Current() == RendererAPIType::Vulkan, "Only Vulkan is supported!");
		const std::string rendererAPI = "Vulkan";
		const eastl::string title = fmt::eastl_format("{0} ({1}) - Editor - {2} ({3}) Renderer: {4}", sceneName, Project::GetActive()->GetConfig().Name, Application::GetPlatformName(), Application::GetConfigurationName(), rendererAPI);
				...
				if (ImGui::BeginMenu("File"))
				{
					...
					if (ImGui::MenuItem("Build Sound Bank"))
					{
						if (!MiniAudioEngine::BuildSoundBank())
							UI::ShowSimpleMessageBox<BEY_MESSAGE_BOX_OK_BUTTON>(":[", "Failed to build Sound Bank.");
					}
	...
	float EditorLayer::UI_DrawTitlebar()
	{
		...
		// Logo
		{
			const int logoWidth =  int((float)EditorResources::BeyondLogoTexture->GetWidth() / 12.0f);
			const int logoHeight = int((float) EditorResources::BeyondLogoTexture->GetHeight() / 12.0f);
			const ImVec2 logoOffset(6.0f + windowPadding.x, 6.0f + windowPadding.y);
			const ImVec2 logoRectStart = { ImGui::GetItemRectMin().x + logoOffset.x, ImGui::GetItemRectMin().y + logoOffset.y };
			const ImVec2 logoRectMax = { logoRectStart.x + logoWidth, logoRectStart.y + logoHeight };
			drawList->AddImage(UI::GetTextureID(EditorResources::BeyondLogoTexture), logoRectStart, logoRectMax);
		}
		...
		// Draw the settings popup
		{
			...
			if (ImGui::BeginPopup("ViewportSettingsPanel", ImGuiWindowFlags_NoMove))
			{
				...
				if (beginSection("Scene Camera"))
				{
					...
					float fov = glm::degrees(m_EditorCamera.GetVerticalFOV());
					if (drag("FOV", fov, 0.1f, 1.0f, 170.0f))
						m_EditorCamera.SetFOV(glm::radians(fov));
	...
	bool EditorLayer::UI_DrawGizmos()
	{
		BEY_PROFILE_FUNC();
		bool changed = false;
		...
		bool snap = Input::IsKeyDown(BEY_KEY_LEFT_CONTROL);
			...
			for (uint64_t i = 0; i < count; i++)
			{
				...
				if (asset)
				{
					...
					else if (asset->GetAssetType() == AssetType::StaticMesh)
					{
						...
						const auto& entityName = assetData.FilePath.stem().string();
						Entity entity = m_EditorScene->CreateEntity(eastl::string(entityName.c_str(), entityName.size()));
	...
	void EditorLayer::OnUpdate(Timestep ts)
	{
		...
		if (((Input::IsKeyDown(Beyond::KeyCode::LeftAlt) && (Input::IsMouseButtonDown(MouseButton::Left) || (Input::IsMouseButtonDown(MouseButton::Middle)))) || Input::IsMouseButtonDown(MouseButton::Right)) && !m_StartedCameraClickInViewport && m_ViewportPanelFocused && m_ViewportPanelMouseOver)
			m_StartedCameraClickInViewport = true;

		if (!Input::IsMouseButtonDown(MouseButton::Right) && !(Input::IsKeyDown(Beyond::KeyCode::LeftAlt) && (Input::IsMouseButtonDown(MouseButton::Left) || (Input::IsMouseButtonDown(MouseButton::Middle)))))
		...
		if (m_DrawOnTopBoundingBoxes && m_ShowBoundingBoxes)
		{
			if (m_ShowBoundingBoxSelectedMeshOnly)
			{
				...
				for (const auto& entityID : selectedEntities)
				{
					...
					{
						auto mesh = AssetManager::GetAsset<Mesh>(entity.GetComponent<MeshComponent>().MeshAssetHandle);
					...
					{
						auto mesh = AssetManager::GetAsset<StaticMesh>(entity.GetComponent<StaticMeshComponent>().StaticMeshAssetHandle);
	...
	void EditorLayer::CreateProject(std::filesystem::path projectPath)
	{
		...
		std::filesystem::path beyondRootDirectory = std::filesystem::absolute("./Resources").parent_path().string();
		std::string beyondRootDirectoryString = beyondRootDirectory.string();

		if (beyondRootDirectory.stem().string() == "Editor")
			beyondRootDirectoryString = beyondRootDirectory.parent_path().string();

		std::replace(beyondRootDirectoryString.begin(), beyondRootDirectoryString.end(), '\\', '/');
		...
		{
			std::filesystem::path originalFilePath = beyondRootDirectoryString + "/Editor/Resources/Meshes/Default";
			std::filesystem::path targetPath = projectPath / "Assets" / "Meshes" / "Default" / "Source";
			BEY_CORE_VERIFY(std::filesystem::exists(originalFilePath));
	...
	{
		std::filesystem::path filepath = FileSystem::OpenFileDialog({ { "Beyond Project", "hproj" } });
	...
	void EditorLayer::OpenProject(const std::filesystem::path& filepath)
	{
		...
		{
			BEY_CORE_ERROR("Tried to open a project that doesn't exist. Project path: {0}", filepath);
			memset(s_OpenProjectFilePathBuffer, 0, MAX_PROJECT_FILEPATH_LENGTH);
			return;
		}
		...
		{
			BEY_CONSOLE_LOG_WARN("No C# assembly has been provided in the Project Settings, or it wasn't found. Please make sure to build the Visual Studio Solution for this project if you want to use C# scripts!");
			std::string path = appAssemblyPath.string();
			if (path.empty())
				path = "<empty>";
			BEY_CONSOLE_LOG_WARN("App Assembly Path = {}", path);
		}
	...
	void EditorLayer::CloseProject(bool unloadProject)
	{
		...
		BEY_CORE_ASSERT(m_EditorScene->GetRefCount() == 1, "Scene will not be destroyed after project is closed - something is still holding scene refs!");
	...
	void EditorLayer::NewScene(const eastl::string& name)
	{
		SelectionManager::DeselectAll();
	...
	{
		std::filesystem::path filepath = FileSystem::OpenFileDialog({ { "Beyond Scene", "hscene" } });
		if (!filepath.empty())
	...
	bool EditorLayer::OpenScene(const std::filesystem::path& filepath, const bool checkAutoSave)
	{
		if (!FileSystem::Exists(filepath))
		{
			BEY_CORE_ERROR("Tried loading a non-existing scene: {0}", filepath);
			return false;
		}
		...
		const auto& title = filepath.filename().string();
		UpdateWindowTitle(eastl::string(title.c_str(), title.size()));
		m_PanelManager->SetSceneContext(m_EditorScene);
		ScriptEngine::SetSceneContext(m_EditorScene, m_ViewportRenderer);
		MiniAudioEngine::SetSceneContext(m_EditorScene);
		AssetEditorPanel::SetSceneContext(m_EditorScene);
	...
	{
		std::filesystem::path filepath = FileSystem::SaveFileDialog({ { "Beyond Scene (*.hscene)", "*.hscene" } });
		...
		UpdateWindowTitle(path.filename().string().c_str());
		m_SceneFilePath = filepath.string();
		std::replace(m_SceneFilePath.begin(), m_SceneFilePath.end(), '\\', '/');
	}
	...
	void EditorLayer::UI_ShowWelcomePopup()
	{
		...
		{
			ImGui::Text("Welcome to Beyond!");
			ImGui::Separator();
			ImGui::TextWrapped("Beyond is an early-stage interactive application and rendering engine for Windows.");
			ImGui::TextWrapped("Please report bugs by opening an issue on the Beyond-dev GitLab.");
	...
	{
		UI::ShowMessageBox("About Beyond", []()
		{
			UI::Fonts::PushFont("Large");
			ImGui::Text("Beyond Engine");
			UI::Fonts::PopFont();
		...
		if (m_AssetPackFuture.valid())
		{
			...
			if (status == std::future_status::ready)
			{
				try
				{
					...
					BEY_CONSOLE_LOG_INFO(m_AssetPackBuildMessage);
				}
				catch (...)
				{
					m_AssetPackBuildMessage = "Asset pack build failed!";
					BEY_CONSOLE_LOG_ERROR(m_AssetPackBuildMessage);
				}
	...
	void EditorLayer::UI_ShowCreateNewMeshPopup()
	{
		...
		BEY_CORE_ASSERT(m_CreateNewMeshPopupData.MeshToCreate);
		...
		{
			ImGui::TextWrapped("This file must be converted into Beyond assets before it can be added to the scene.");
			...
			if (ImGui::Button("Create"))
			{
				...
				if (doImportMesh)
				{
					if (!m_CreateNewMeshPopupData.MeshToCreate->GetSubmeshes().empty())
					{
						...
						if (meshType == MeshType::Static)
						{
							Ref<StaticMesh> mesh = Project::GetEditorAssetManager()->CreateNewAsset<StaticMesh>(path.filename().string(), path.parent_path().string(), m_CreateNewMeshPopupData.MeshToCreate, path.string());
							...
							if (entity)
							{
								...
								mc.StaticMeshAssetHandle = mesh->Handle;
							}
							else
							{
								const auto& meshMetadata = Project::GetEditorAssetManager()->GetMetadata(mesh->Handle);
								const auto& entityName = meshMetadata.FilePath.stem().string();
								entity = m_EditorScene->CreateEntity(eastl::string(entityName.c_str(), entityName.size()));
						...
						{
							Ref<Mesh> mesh = Project::GetEditorAssetManager()->CreateNewAsset<Mesh>(path.filename().string(), path.parent_path().string(), m_CreateNewMeshPopupData.MeshToCreate, path.string());
							...
							if (entity)
							{
								...
								mc.MeshAssetHandle = mesh->Handle;
							}
							else
							{
				...
				for (size_t i = 0; i < doImportAnimations.size(); ++i)
				{
					...
					{
						BEY_CORE_ASSERT(m_CreateNewMeshPopupData.MeshToCreate->GetAnimationNames().size() > i);
						...
						{
							auto skeletonAsset = AssetManager::GetAsset<SkeletonAsset>(skeletonAssets[i]);
							BEY_CORE_ASSERT(skeletonAsset && skeletonAsset->IsValid());
		...
		if (ImGui::Begin("Statistics"))
		{
			...
			if (ImGui::BeginTabBar("StatisticsTabs", tab_bar_flags))
			{
				...
				if (ImGui::BeginTabItem("Renderer"))
				{
					...
					ImGui::Text("FPS: %.2fms\n", app.GetFPS());
					//ImGui::Text("FPS: %.2fms\n", 1.0 / app.GetTimestep().GetMilliseconds() * 1000);
					...
					}
					{
						...
						ImGui::Text("Samplers: %d", renderResourceAllocationCounts.Samplers.load());
					}
					ImGui::Separator();
				...
				if (ImGui::BeginTabItem("Performance"))
				{
					...
					std::vector<std::pair<const char*, float>> sortedStats;
					sortedStats.reserve(perFrameData.size());

					for (const auto& [name, time] : perFrameData)
					{
						sortedStats.emplace_back(name, time.Time);
					}

					//std::ranges::sort(sortedStats, [](const auto& a, const auto& b) { return a.second > b.second; });

					for (const auto& [name, time] : sortedStats)
					{
						ImGui::Text("%s: %.3fms\n", name, time);
					}
					ImGui::EndTabItem();
				...
				if (ImGui::BeginTabItem("Memory"))
				{
					...
					for (auto& entry : sortedEntries)
					{
						...
						{
							std::string_view tag = slash;
							tag = tag.substr(tag.find_last_of("\\") + 1);

							ImGui::TextColored(ImVec4(0.3f, 0.4f, 0.9f, 1.0f), "%s: %s", tag.data(), usageStr.c_str());
						}
						else
						{
...
#else
					ImGui::TextColored(ImVec4(0.9f, 0.35f, 0.3f, 1.0f), "Memory is not being tracked because BEY_TRACK_MEMORY is not defined!");
#endif
	...
	bool EditorLayer::OnImGuiRender()
	{
		BEY_PROFILE_FUNC();
		bool changed = false;
		...
		ImGui::Begin("Viewport");
		{
			...
			ImVec2 viewportSize = ImVec2(contentRegionMax.x - cursorPos.x, contentRegionMax.y - cursorPos.y);
			...
			// NOTE: This currently just subtracts 0 because I removed the toolbar window, but I'll keep it in just in case
	...
	bool EditorLayer::OnKeyPressedEvent(KeyPressedEvent& e)
	{
		if (UI::IsWindowFocused("Viewport") || UI::IsWindowFocused("Scene Hierarchy"))
		{
			...
			switch (e.GetKeyCode())
			{
				...
				case KeyCode::Delete:
				{
					// NOTE: Intentional copy since DeleteEntity modifies the selection map
		...
		if (Input::IsKeyDown(BEY_KEY_LEFT_CONTROL) && !Input::IsMouseButtonDown(MouseButton::Right))
		{
			switch (e.GetKeyCode())
			{
				case KeyCode::B:
						...
						if (entity.HasComponent<PrefabComponent>())
						{
							...
							if (!AssetManager::IsAssetHandleValid(prefabID))
							{
								UI::ShowMessageBox("Cannot duplicate an invalid prefab Entity!", []()
								{
									...
									UI::Fonts::PopFont();
									UI::Fonts::PushFont("Medium");
									ImGui::TextWrapped("and navigate to \"Working withing Beyond\" -> \"Fixing Broken Prefabs\"");
									UI::Fonts::PopFont();
			...
			if (Input::IsKeyDown(BEY_KEY_LEFT_SHIFT))
			{
				switch (e.GetKeyCode())
				{
					case KeyCode::S:
		...
		if (m_SceneState == SceneState::Play && Input::IsKeyDown(BEY_KEY_LEFT_ALT))
		{
	...
	bool EditorLayer::OnMouseButtonPressed(MouseButtonPressedEvent& e)
	{
		...
		if (mouseX > -1.0f && mouseX < 1.0f && mouseY > -1.0f && mouseY < 1.0f)
		{
			...
			for (auto e : meshEntities)
			{
				...
				auto mesh = AssetManager::GetAsset<Mesh>(mc.MeshAssetHandle);
				if (!mesh || mesh->IsFlagSet(AssetFlag::Missing) && !mesh->IsFlagSet(AssetFlag::StillLoading))
					continue;
			...
			for (auto e : staticMeshEntities)
			{
				...
				auto staticMesh = AssetManager::GetAsset<StaticMesh>(smc.StaticMeshAssetHandle);
				if (!staticMesh || staticMesh->IsFlagSet(AssetFlag::Missing) && !staticMesh->IsFlagSet(AssetFlag::StillLoading))
					continue;
