...
namespace Beyond {
	...
	void RuntimeLayer::OnAttach()
	{
		...
		spec.NumShadowCascades = 2; 
		spec.EnableEdgeOutlineEffect = false; 
		...
		BEY_CORE_VERIFY(m_SwapChainRP->Validate());
		m_SwapChainRP->Bake();
	...
	void RuntimeLayer::OnDetach()
	{
		...
		BEY_CORE_VERIFY(m_RuntimeScene->GetRefCount() == 1);
	...
	void RuntimeLayer::UpdateWindowTitle(const eastl::string& sceneName)
	{
	...
	{
		BEY_CORE_VERIFY(false); // Not supported in runtime
	...
	void RuntimeLayer::OnEvent(Event& e)
	{
		...
		dispatcher.Dispatch<KeyPressedEvent>(BEY_BIND_EVENT_FN(RuntimeLayer::OnKeyPressedEvent));
		dispatcher.Dispatch<MouseButtonPressedEvent>(BEY_BIND_EVENT_FN(RuntimeLayer::OnMouseButtonPressed));
	}
